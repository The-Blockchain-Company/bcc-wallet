...
instance NextWallet "sophie" where
    type MnemonicSize "sophie" = 15
...
icaMnemonics = unsafeMkMnemonic <$>
    ...
    , [ "title", "prijen", "loan", "peasant", "reduce"
...
--
-- >>> genMnemonics 100 >>= genColeFaucets "cole-faucets.yaml"
genColeFaucets :: FilePath -> [Mnemonic 12] -> IO [[Text]]
genColeFaucets = genFaucet encodeAddress genAddresses
  where
    ...
    genAddresses mw =
        let
            ...
            accXPrv =
                Cole.deriveAccountPrivateKey pwd rootXPrv (liftIndex @'Hardened minBound)
            addrXPrv =
...
--
-- >>> genMnemonics 100 >>= genSophieFaucets "sophie-faucets.yaml"
genSophieFaucets :: FilePath -> [Mnemonic 15] -> IO [[Text]]
genSophieFaucets =
    genFaucet encodeAddressHex (genSophieAddresses . SomeMnemonic)

-- | Generate faucet addresses and mnemonics to a file.
--
-- >>> genMnemonics 100 >>= genMAFaucets "ma-faucets.yaml"
genMAFaucets :: FilePath -> [Mnemonic 24] -> IO [[Text]]
genMAFaucets = genFaucet encodeAddressHex (genSophieAddresses . SomeMnemonic)

encodeAddressHex :: Address -> Text
encodeAddressHex = T.decodeUtf8 . convertToBase Base16 . unAddress

genSophieAddresses :: SomeMnemonic -> [Address]
genSophieAddresses mw =
    let
        (seed, pwd) =
            (mw, mempty)
        rootXPrv =
...
-- | A special Sophie Wallet with 200 UTxOs where 100 of them are 1 BCC
...
sophieIntegrationTestFunds :: [(Address, Coin)]
sophieIntegrationTestFunds = mconcat
  ...
  where
    ...
    mirWallets = (,defaultAmt) . head . genSophieAddresses . SomeMnemonic
        <$> mirMnemonics
...
jenAssetScripts :: [(TokenPolicyId, (String, String))]
jenAssetScripts = map (first (unsafeFromText . T.pack))
...
jenIntegrationTestAssets
    :: Coin -- ^ Amount of bcc in each bundle
    -> [(Address, (TokenBundle, [(String, String)]))]
jenIntegrationTestAssets tips = maMnemonics >>= take 3
    . flip zip (cycle jenTokenBundles)
    . genSophieAddresses
    . SomeMnemonic
  where
    jenTokenBundles = zipWith mint [simple, fruit, combined] jenAssetScripts
