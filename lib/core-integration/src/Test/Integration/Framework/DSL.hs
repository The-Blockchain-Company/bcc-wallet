...
import qualified Bcc.Wallet.Primitive.AddressDerivation.Cole as Cole
import qualified Bcc.Wallet.Primitive.AddressDerivation.Icarus as Icarus
...
import qualified Bcc.Wallet.Primitive.Types.TokenQuantity as TokenQuantity
...
minUTxOValue e
    | e >= ApiAurum = 999_978 -- From 34482 entropic per word
    | otherwise   = 1_000_000

-- | Parameter in test cluster sophie genesis.
--
...
computeApiCoinSelectionFee selection
  ...
  where
    ...
    balanceOfDeposits
        ...
        & getSum
    quantityToInteger :: Quantity "entropic" Natural -> Integer
...
isValidDerivationPath purpose path =
    ...
    ( [ ApiT $ DerivationIndex $ getIndex purpose
      , ApiT $ DerivationIndex $ getIndex coinTypeBcc
...
waitAllTxsInLedger ctx w = eventually "waitAllTxsInLedger: all txs in ledger" $ do
    let ep = Link.listTransactions @'Sophie w
...
-- NOTE
-- This __90s__ is mostly justified by the parameters in the sophie
...
emptyIcarusWallet ctx = do
    ...
    emptyColeWalletWith ctx "icarus"
        ("Icarus Wallet", mnemonic, fixturePassphrase)

emptyIcarusWalletMws
    :: (MonadIO m, MonadUnliftIO m)
    => Context
    -> ResourceT m (ApiColeWallet, Mnemonic 15)
emptyIcarusWalletMws ctx = do
    mnemonic <- liftIO $ entropyToMnemonic <$> genEntropy
    (,mnemonic) <$> emptyColeWalletWith ctx "icarus"
...
emptyRandomWalletWithPasswd ctx rawPwd = do
    ...
    let key = T.decodeUtf8
            $ hex
            $ Cole.getKey
            $ Cole.generateKeyFromSeed seed pwd
    pwdH <- liftIO $ T.decodeUtf8 . hex <$> encryptPasswordWithScrypt pwd
    emptyColeWalletFromXPrvWith ctx "random" ("Random Wallet", key, pwdH)
  where
...
postWallet' ctx headers payload = snd <$> allocate create (free . snd)
  where
    create =
        request @ApiWallet ctx (Link.postWallet @'Sophie) headers payload

    free (Right w) = void $ request @Aeson.Value ctx
        (Link.deleteWallet @'Sophie w) Default Empty
    free (Left _) = return ()
...
postColeWallet
    ...
    => Context
    -> Payload
    -> ResourceT m (HTTP.Status, Either RequestException ApiColeWallet)
postColeWallet ctx payload = snd <$> allocate create (free . snd)
  where
    create =
        request @ApiColeWallet ctx (Link.postWallet @'Cole) Default payload
...
emptyColeWalletWith
    ...
    -> (Text, [Text], Text)
    -> ResourceT m ApiColeWallet
emptyColeWalletWith ctx style (name, mnemonic, pass) = do
    let payload = Json [aesonQQ| {
            "name": #{name},
...
emptyColeWalletFromXPrvWith
    ...
    -> (Text, Text, Text)
    -> ResourceT m ApiColeWallet
emptyColeWalletFromXPrvWith ctx style (name, key, passHash) = do
    let payload = Json [aesonQQ| {
            "name": #{name},
...
rewardWallet ctx = do
    ...
    eventually "MIR wallet: wallet is 100% synced " $ do
        rg <- request @ApiWallet ctx (Link.getWallet @'Sophie w) Default Empty
        verify rg
...
fixtureMultiAssetRandomWallet ctx = do
    ...
    rL <- request @[ApiAddress n] ctx (Link.listAddresses @'Cole wB) Default Empty
    let addrs = getFromResponse id rL
    let destination = (addrs !! 1) ^. #id
    payload <- mkTxPayloadMA @n destination 0 [val] fixturePassphrase

    -- send assets to Cole wallet
    rtx <- request @(ApiTransaction n) ctx
        (Link.createTransactionOld @'Sophie wMA) Default payload
    expectResponseCode HTTP.status202 rtx

    eventually "Cole wallet has assets" $ do
        rb <- request @ApiColeWallet ctx
            (Link.getWallet @'Cole wB) Default Empty
        verify rb
...
fixtureWallet
    ...
    => Context
    -> ResourceT m ApiWallet
fixtureWallet = fmap fst . fixtureWalletWithMnemonics (Proxy @"sophie")
  ...
  where
    create = do
        ...
        r <- request @ApiWallet ctx
            (Link.postWallet @'Sophie) Default payload
        expectResponseCode HTTP.status201 r
        let w = getFromResponse id r
    ...
    checkBalance w = do
        r <- request @ApiWallet ctx
            (Link.getWallet @'Sophie w) Default Empty
...
fixtureRandomWalletAddrs
    ...
    => Context
    -> ResourceT m (ApiColeWallet, [Address])
fixtureRandomWalletAddrs =
...
-- faucet and they "only" have 'faucetUtxoAmt = 100_000 Bcc' in each.
--
-- This function makes no attempt at ensuring the request is valid, so be
-- careful.
--
-- TODO: Remove duplication between Sophie / Cole fixtures.
fixtureRandomWalletWith
    :: forall (n :: NetworkDiscriminant) m.
        ( EncodeAddress n
        , DecodeAddress n
...
fixtureRandomWalletWith ctx coins0 = do
    ...
    dest <- emptyColeWalletWith ctx "random"
        ("Random Wallet", mnemonicToText @12 mws, fixturePassphrase)
    let addrs = randomAddresses @n mws
    liftIO $ mapM_ (moveColeCoins @n ctx src (dest, addrs)) (groupsOf 10 coins0)
    void $ request @() ctx
        (Link.deleteWallet @'Cole src) Default Empty
    r <- request @ApiColeWallet ctx
        (Link.getWallet @'Cole dest) Default Empty
    expectResponseCode HTTP.status200 r
...
fixtureIcarusWalletWith ctx coins0 = do
    ...
    mws  <- liftIO $ entropyToMnemonic <$> genEntropy
    dest <- emptyColeWalletWith ctx "icarus"
...
-- | Restore a legacy wallet (Cole or Icarus)
fixtureLegacyWallet
    :: forall m. (MonadIO m, MonadUnliftIO m)
    => Context
    -> String
...
fixtureLegacyWallet ctx style mnemonics = snd <$> allocate create free
  where
    create = do
        let payload = Json [aesonQQ| {
                "name": "Faucet Cole Wallet",
                "mnemonic_sentence": #{mnemonics},
                "passphrase": #{fixturePassphrase},
                "style": #{style}
                } |]
        r <- request @ApiColeWallet ctx (Link.postWallet @'Cole) Default payload
        expectResponseCode HTTP.status201 r
        let w = getFromResponse id r
        liftIO $ race (threadDelay sixtySeconds) (checkBalance w) >>= \case
            Left _ ->
                expectationFailure'
                    "fixtureColeWallet: waited too long for initial transaction"
            Right a ->
                return a
    free w = do
        void $ request @() ctx
...
fixtureWalletWith ctx coins0 = do
    ...
    liftIO $ void $ request @() ctx
        (Link.deleteWallet @'Sophie src) Default Empty

    waitForTxImmutability ctx

    r <- request @ApiWallet ctx
        (Link.getWallet @'Sophie dest) Default Empty
    expectResponseCode HTTP.status200 r
    pure (getFromResponse id r)
  where
    ...
    moveCoins src dest coins = do
        ...
        addrs <- fmap (view #id) . getFromResponse id
            <$> request @[ApiAddress n] ctx
                    (Link.listAddresses @'Sophie dest) Default Empty
        ...
        request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie src) Default payload
        ...
        eventually "balance available = balance total" $ do
            ...
            ra <- request @ApiWallet ctx
                (Link.getWallet @'Sophie src) Default Empty
...
moveColeCoins
    ...
    -> ApiColeWallet
        -- ^ Source Wallet
    -> (ApiColeWallet, [Address])
        -- ^ Destination wallet
    -> [Natural]
        -- ^ Coins to move
    -> IO ()
moveColeCoins ctx src (dest, addrs) coins = do
    balance <- getFromResponse (#balance . #available . #getQuantity)
        <$> request @ApiColeWallet ctx (Link.getWallet @'Cole dest) Default Empty
    ...
    request @(ApiTransaction n) ctx
        (Link.createTransactionOld @'Cole src) Default payload
        >>= expectResponseCode HTTP.status202
    eventually "balance available = balance total" $ do
        rb <- request @ApiColeWallet ctx
            (Link.getWallet @'Cole dest) Default Empty
        expectField (#balance . #available)
            (`shouldBe` Quantity (sum (balance:coins))) rb
        ra <- request @ApiColeWallet ctx
            (Link.getWallet @'Cole src) Default Empty
        getFromResponse (#balance . #available) ra
...
arbitraryStake = Just $ bcc 10_000_000_000
...
randomAddresses mw =
    let
        ...
        rootXPrv =
            Cole.generateKeyFromSeed seed pwd
        accXPrv =
            Cole.deriveAccountPrivateKey pwd rootXPrv minBound
        addrXPrv =
            Cole.deriveAddressPrivateKey pwd accXPrv
    in
...
-- | Generate an infinite list of addresses for sophie wallets.
--
-- To be typically used as:
--
-- >>> take 1 (sophieAddresses @n)
-- [addr]
sophieAddresses
    :: forall (n :: NetworkDiscriminant). (PaymentAddress n SophieKey)
    => Mnemonic 15
    -> [Address]
sophieAddresses mw =
    let
        (seed, pwd) =
            (SomeMnemonic mw, mempty)
        rootXPrv =
            Sophie.generateKeyFromSeed (seed, Nothing) pwd
        accXPrv =
            deriveAccountPrivateKey pwd rootXPrv minBound
        addrXPrv =
            deriveAddressPrivateKey pwd accXPrv UtxoExternal
...
listAddresses ctx w = do
    let link = Link.listAddresses @'Sophie w
...
getWallet
    ...
    -> m ApiWallet
getWallet ctx w = do
    let link = Link.getWallet @'Sophie w
...
listTransactions ctx w mStart mEnd mOrder = do
    ...
    return txs
  where
    path = Link.listTransactions' @'Sophie w
        Nothing
        (Iso8601Time <$> mStart)
        (Iso8601Time <$> mEnd)
        mOrder
...
deleteAllWallets ctx = do
    ...
    respCole <-
        request @[ApiColeWallet] ctx ("GET", "v2/cole-wallets") Default Empty
    forM_ (wallets (snd respCole)) $ \wal -> do
        let endpoint = "v2/cole-wallets" </> wal ^. walletId
        d <- request @Value ctx ("DELETE", endpoint) None Empty
        expectResponseCode HTTP.status204 d
 where
...
-- | Calls 'GET /cole-wallets' and filters the response. This allows tests to
-- be written for a parallel setting.
listFilteredColeWallets
    :: (MonadIO m, MonadUnliftIO m)
    => Set Text -- ^ Set of walletIds to include
    -> Context
    -> m (HTTP.Status, Either RequestException [ApiColeWallet])
listFilteredColeWallets include ctx = do
    (s, mwallets) <- request @[ApiColeWallet] ctx
        (Link.listWallets @'Cole) Default Empty
    return (s, filter (\w -> (w ^. walletId) `Set.member` include) <$> mwallets)
...
--
-- >>> Link.getWallet @Sophie myWallet
-- ( "GET", "v2/wallets/2512a00e9653fe49a44a5886202e24d77eeb998f" )
--
-- >>> withPathParam 0 (<> "suffix") $ Link.getWallet @Sophie myWallet
-- ( "GET", "v2/wallets/2512a00e9653fe49a44a5886202e24d77eeb998fsuffix" )
--
-- >>> withPathParam 1 (const "suffix") $ Link.joinStakePool @Sophie myPool myWallet
...
commandName :: String
commandName = "bcc-wallet"

-- | Run a command using the 'bcc-wallet' executable.
bccWalletCLI
    :: forall r m. (CmdResult r, MonadIO m)
    => [String]
    -> m r
bccWalletCLI = liftIO . command [EchoStderr False] commandName

generateMnemonicsViaCLI
    :: forall r m. (CmdResult r, MonadIO m)
    => [String]
    -> m r
generateMnemonicsViaCLI args = bccWalletCLI
...
deleteWalletViaCLI
    ...
    -> String
    -> m r
deleteWalletViaCLI ctx walId = bccWalletCLI
...
getWalletViaCLI
    ...
    -> String
    -> m r
getWalletViaCLI ctx walId = bccWalletCLI
...
getWalletUtxoStatisticsViaCLI
    ...
    -> String
    -> m r
getWalletUtxoStatisticsViaCLI ctx walId = bccWalletCLI
...
getWalletUtxoSnapshotViaCLI
    ...
    -> String
    -> m r
getWalletUtxoSnapshotViaCLI ctx walId = bccWalletCLI
    [ "wallet"
    , "utxo-snapshot"
    , "--port"
...
importAddressViaCLI
    ...
    -> m r
        -- ^ (ExitCode, StdOut, StdErr)
importAddressViaCLI ctx args = bccWalletCLI $
...
listAddressesViaCLI
    ...
    -> [String]
    -> m r
listAddressesViaCLI ctx args = bccWalletCLI $
...
listStakePoolsViaCLI
    ...
    => s
    -> m r
listStakePoolsViaCLI ctx = bccWalletCLI
...
listWalletsViaCLI
    ...
    => s
    -> m r
listWalletsViaCLI ctx = bccWalletCLI
...
updateWalletNameViaCLI
    ...
    -> [String]
    -> m r
updateWalletNameViaCLI ctx args = bccWalletCLI
...
postTransactionFeeViaCLI
    ...
    -> [String]
    -> m r
postTransactionFeeViaCLI ctx args = bccWalletCLI $ join
        [ ["transaction", "fees"]
        , ["--port", show (ctx ^. typed @(Port "wallet"))]
        , args
        ]
...
listTransactionsViaCLI
    ...
    -> [String]
    -> m r
listTransactionsViaCLI ctx args = bccWalletCLI $ join
    [ ["transaction", "list"]
    , ["--port", show (ctx ^. typed @(Port "wallet"))]
    , args
    ]
...
postExternalTransactionViaCLI
    ...
    -> [String]
    -> m r
postExternalTransactionViaCLI ctx args = bccWalletCLI $ join
    [ ["transaction", "submit"]
    , ["--port", show (ctx ^. typed @(Port "wallet"))]
    , args
    ]
...
deleteTransactionViaCLI
    ...
    -> String
    -> m r
deleteTransactionViaCLI ctx wid tid = bccWalletCLI $ join
    [ ["transaction", "forget"]
...
getTransactionViaCLI
    ...
    -> String
    -> m r
getTransactionViaCLI ctx wid tid = bccWalletCLI $ join
    [ ["transaction", "get"]
    , ["--port", show (ctx ^. typed @(Port "wallet")), wid, tid]
    ]
...
rootPrvKeyFromMnemonics mnemonics pass =
    T.decodeUtf8 $ hex $ getKey $ Cole.generateKeyFromSeed seed
...
pubKeyFromMnemonics mnemonics =
 ...
 where
     ...
     rootXPrv = Sophie.generateKeyFromSeed (seed, Nothing) mempty
...
getTTLSlots ctx dt = liftIO $ do
    ...
    let slotLen = 0.2 -- fixme: this is the value from cole genesis
