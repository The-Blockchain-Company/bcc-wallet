...
specMnemonicCole = [ "squirrel", "material", "silly", "twice", "direct", "slush",
...
steveToken = ApiAssetMetadata
    ...
    (Just (ApiT (unsafeFromText "https://tbco.io/stevetoken")))
...
errMsg403MinUTxOValue =
    "Some outputs have bcc values that are too small. There's a \
    \minimum bcc value specified by the protocol that each output must satisfy. \
    \I'll handle that minimum value myself when you do not explicitly specify \
    \an bcc value for an output. Otherwise, you must specify enough bcc."
...
errMsg403Fee :: String
errMsg403Fee =
    "I am unable to finalize the transaction, as there is not enough bcc \
    \available to pay for the fee and also pay for the minimum bcc quantities \
    \of all change outputs."

errMsg403NotAColeWallet :: String
errMsg403NotAColeWallet =
...
errMsg403NotASophieWallet :: String
errMsg403NotASophieWallet =
    "It is regrettable but you've just attempted an operation that is invalid \
    \for this type of wallet. Only new 'Sophie' wallets can do something with \
...
--
-- See https://The-Blockchain-Company.atlassian.net/browse/ADP-1005
--
