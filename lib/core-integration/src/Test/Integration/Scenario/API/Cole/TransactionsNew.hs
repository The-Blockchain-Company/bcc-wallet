...
module Test.Integration.Scenario.API.Cole.TransactionsNew
    ( spec
    ) where
...
spec = describe "NEW_COLE_TRANSACTIONS" $ do

    describe "COLE_TRANS_NEW_CREATE_01a - Empty payload is not allowed" $
        forM_ [ (fixtureRandomWallet, "Cole fixture wallet")
              ...
              \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
                ...
                liftIO $ pendingWith "Cole fixture wallet returns 500"

                wa <- srcFixture ctx
                let emptyPayload = Json [json|{}|]

                rTx <- request @(ApiConstructTransaction n) ctx
                    (Link.createUnsignedTransaction @'Cole wa)
    ...
    describe "COLE_TRANS_NEW_CREATE_01c - No payload is bad request" $
        forM_ [ (fixtureRandomWallet, "Cole fixture wallet")
              , (fixtureIcarusWallet, "Icarus fixture wallet")] $
              \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
                ...
                rTx <- request @(ApiConstructTransaction n) ctx
                    (Link.createUnsignedTransaction @'Cole wa) Default Empty
                verify rTx
                    [ expectResponseCode HTTP.status400
                    ]

    describe "COLE_TRANS_NEW_CREATE_04 - Single Output Transaction" $
        forM_ [(fixtureRandomWallet, "Cole wallet"),
              (fixtureIcarusWallet, "Icarus wallet")] $
              \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do

            liftIO $ pendingWith "Cole wallet returns 500, Icarus wallet returns 403 (invalid_wallet_type)"


            (wCole, wSophie) <- (,) <$> srcFixture ctx <*> emptyWallet ctx
            ...
            rTx <- request @(ApiConstructTransaction n) ctx
                (Link.createUnsignedTransaction @'Cole wCole) Default payload
            verify rTx
                [ expectSuccess
    ...
    describe "COLE_TRANS_NEW_CREATE_04c - Can't cover fee" $
        forM_ [(fixtureRandomWalletWith @n, "Cole wallet"),
               (fixtureIcarusWalletWith @n, "Icarus wallet")] $
               \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do

            liftIO $ pendingWith "Cole wallet returns 500, Icarus wallet as expected"
            ...
            rTx <- request @(ApiConstructTransaction n) ctx
                (Link.createUnsignedTransaction @'Cole wa) Default payload
            verify rTx
                [ expectResponseCode HTTP.status403
                , expectErrorMessage errMsg403Fee
                ]

    describe "COLE_TRANS_NEW_CREATE_04d - Not enough money" $
        forM_ [(fixtureRandomWalletWith @n, "Cole wallet"),
               (fixtureIcarusWalletWith @n, "Icarus wallet")] $
               \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do
    ...
    describe "COLE_TRANS_NEW_CREATE_04d - Not enough money emptyWallet" $
        forM_ [(emptyRandomWallet, "Empty Cole wallet"),
    ...
    describe "COLE_TRANS_NEW_CREATE_04e - Multiple Output Tx to single wallet" $
        forM_ [(fixtureRandomWallet, "Cole wallet"),
              (fixtureIcarusWallet, "Icarus wallet")] $
              \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do
    ...
    describe "COLE_TRANS_NEW_ASSETS_CREATE_01a - Multi-asset tx with Bcc" $
        ...
        wa <- srcFixture ctx
        wb <- emptyWallet ctx
        ra <- request @ApiColeWallet ctx (Link.getWallet @'Cole wa) Default Empty
        let (_, Right wal) = ra
    ...
    describe "COLE_TRANS_NEW_ASSETS_CREATE_01b - Multi-asset tx with not enough Bcc" $
        forM_ [(fixtureMultiAssetRandomWallet @n, "Cole wallet"),
              (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")] $
              \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do
    ...
    describe "COLE_TRANS_NEW_ASSETS_CREATE_01c - Multi-asset tx without Bcc" $
        forM_ [(fixtureMultiAssetRandomWallet @n, "Cole wallet"),
              (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")] $
              \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do
    ...
    describe "COLE_TRANS_NEW_ASSETS_CREATE_01d - Multi-asset tx with not enough assets" $
        forM_ [(fixtureMultiAssetRandomWallet @n, "Cole wallet"),
              (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")] $
              \(srcFixture,name) -> it name $ \ctx -> runResourceT $ do
