...
module Test.Integration.Scenario.API.Cole.Migrations
    ( spec
    ) where
...
spec = describe "COLE_MIGRATIONS" $ do

    it "COLE_CREATE_MIGRATION_PLAN_01 - \
        \Can create a migration plan."
        $ \ctx -> forM_ [fixtureRandomWallet, fixtureIcarusWallet]
        $ \fixtureColeWallet -> runResourceT $ do
            sourceWallet <- fixtureColeWallet ctx
            ...
            let ep = Link.createMigrationPlan @'Cole sourceWallet
    ...
    it "COLE_CREATE_MIGRATION_PLAN_02 - \
        \Cannot create plan for empty wallet."
        $ \ctx -> forM_ [emptyRandomWallet, emptyIcarusWallet]
        $ \emptyColeWallet -> runResourceT $ do
            sourceWallet <- emptyColeWallet ctx
            targetWallet <- emptyWallet ctx
            targetAddresses <- listAddresses @n ctx targetWallet
            let targetAddressIds = targetAddresses <&>
                    (\(ApiTypes.ApiAddress addrId _ _) -> addrId)
    ...
    it "COLE_CREATE_MIGRATION_PLAN_03 - \
        \Cannot create plan for Sophie wallet using Cole endpoint."
        $ \ctx -> runResourceT $ do
    ...
    it "COLE_CREATE_MIGRATION_PLAN_04 - \
        ...
        $ \ctx -> runResourceT $ do
            ...
            let payloadRestore = Json [json| {
                    "name": "Dust Cole Wallet",
                    "mnemonic_sentence": #{mnemonicSentence},
                    "passphrase": #{fixturePassphrase},
                    "style": "random"
                    } |]
            sourceWallet <- unsafeResponse <$>
                postColeWallet ctx payloadRestore
            eventually "sourceWallet contains dust" $
                request @ApiColeWallet ctx
                    (Link.getWallet @'Cole sourceWallet) Default Empty
                    >>= flip verify
    ...
    it "COLE_CREATE_MIGRATION_PLAN_05 - \
    ...
    Hspec.it "COLE_MIGRATE_02 - \
        ...
        --
        --  - 100 coins are each worth 1 entropic, and are expected to be
        --    treated as dust.
        --  - 100 coins are each worth 10,000,000,000 entropic.
        --
        ...
        sourceWallet <- unsafeResponse <$> postColeWallet ctx
            (Json [json|{
                "name": "Big Cole Wallet",
                "mnemonic_sentence": #{sourceWalletMnemonic},
                "passphrase": #{fixturePassphrase},
                "style": "random"
            }|])
        sourceBalance <- eventually "Source wallet balance is correct." $ do
            response <- request @ApiColeWallet ctx
                (Link.getWallet @'Cole sourceWallet) Default Empty
            verify response
                [ expectField (#balance . #available . #getQuantity)
        ...
        responsePlan <- request @(ApiWalletMigrationPlan n) ctx
            (Link.createMigrationPlan @'Cole sourceWallet) Default
            (Json [json|{addresses: #{targetAddressIds}}|])
        verify responsePlan
            [ expectResponseCode HTTP.status202
            , expectField
        ...
        responseFinalSourceBalance <- request @ApiColeWallet ctx
            (Link.getWallet @'Cole sourceWallet) Default Empty
    ...
    it "COLE_MIGRATE_03 - \
        ...
        $ \emptyColeWallet -> runResourceT $ do
            ...
            let targetAddressIds = targetAddresses <&>
                    (\(ApiTypes.ApiAddress addrId _ _) -> addrId)
            let ep = Link.migrateWallet @'Cole sourceWallet
    ...
    Hspec.it "COLE_MIGRATE_04 - \
        \Actual fee for migration is identical to predicted fee."
        $ \ctx -> forM_ [fixtureRandomWallet, fixtureIcarusWallet]
        $ \fixtureColeWallet -> runResourceT @IO $ do
            ...
            let endpointPlan = (Link.createMigrationPlan @'Cole sourceWallet)
            ...
            -- Perform a migration:
            let endpointMigrate = Link.migrateWallet @'Cole sourceWallet
            responseMigrate <-
    ...
    it "COLE_MIGRATE_05 - \
        ...
        $ \fixtureColeWallet -> runResourceT $ do
            ...
            -- Restore a Cole wallet with funds, to act as a source wallet:
            sourceWallet <- fixtureColeWallet ctx
            ...
            response <- request @[ApiTransaction n] ctx
                (Link.migrateWallet @'Cole sourceWallet)
                Default
                (Json [json|
    ...
    describe "COLE_MIGRATE_06 - \
        ...
        $ forM_
            ...
            it ("From wallet type: " ++ walType) $ \ctx -> runResourceT $ do
                ...
                addrCole <- liftIO $ encodeAddress @n
                    . head
                    . randomAddresses @n
                ...
                response <- request @[ApiTransaction n] ctx
                    (Link.migrateWallet @'Cole sourceWallet) Default
                    (Json [json|
                        { passphrase: #{fixturePassphrase}
                        , addresses:
                            [ #{addrSophie}
    ...
    Hspec.it "COLE_MIGRATE_08 - \
        \It's not possible to migrate a wallet whose total balance is less \
        \than the minimum bcc quantity for an output."
        $ \ctx -> runResourceT @IO $ do
            ...
            eventually "Source wallet balance is correct." $ do
                request @ApiColeWallet ctx
                    (Link.getWallet @'Cole sourceWallet)
                    Default
                    Empty >>= flip verify
                    [ expectField (#balance . #available)
            ...
            responseSourceDistribution <- request @ApiUtxoStatistics ctx
                (Link.getUTxOsStatistics @'Cole sourceWallet) Default Empty
  ...
  where
    ...
    migrateWallet ctx src targets = do
      ...
      where
        endpointCreateMigrationPlan =
            Link.createMigrationPlan @'Cole src
        endpointMigrateWallet =
            Link.migrateWallet @'Cole src
        endpointListTxs =
            Link.listTransactions @'Cole src
        endpointForget =
            Link.deleteTransaction @'Cole src
    ...
    testAddressCycling sourceWalletType mkSourceWallet targetAddressCount = do
        ...
        it title $ \ctx -> runResourceT $ do
            ...
            let balanceLeftover = getFromResponse
                    (#balanceLeftover . #bcc . #getQuantity) response0
