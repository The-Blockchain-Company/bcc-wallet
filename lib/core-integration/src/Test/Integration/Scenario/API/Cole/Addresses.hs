...
module Test.Integration.Scenario.API.Cole.Addresses
    ( spec
    ) where
...
scenario_ADDRESS_LIST_01
    ...
    => (Context -> ResourceT IO ApiColeWallet)
    -> Int
    -> SpecWith Context
scenario_ADDRESS_LIST_01 fixture derPathSize = it title $ \ctx -> runResourceT $ do
    w <- fixture ctx
    r <- request @[ApiAddress n] ctx (Link.listAddresses @'Cole w) Default Empty
...
scenario_ADDRESS_LIST_02 fixture = it title $ \ctx -> runResourceT $ do
    ...
    rUsed <- request @[ApiAddress n] ctx
        (Link.listAddresses' @'Cole w (Just Used)) Default Empty
    verify rUsed
    ...
    rUnused <- request @[ApiAddress n] ctx
        (Link.listAddresses' @'Cole w (Just Unused)) Default Empty
...
scenario_ADDRESS_LIST_04 fixture = it title $ \ctx -> runResourceT $ do
    w <- fixture ctx
    _ <- request @() ctx (Link.deleteWallet @'Cole w) Default Empty
    r <- request @[ApiAddress n] ctx (Link.listAddresses @'Cole w) Default Empty
    verify r
...
scenario_ADDRESS_CREATE_02 = it title $ \ctx -> runResourceT $ do
    ...
    verify r
        ...
        , expectErrorMessage errMsg403NotAColeWallet
        ]
  where
...
scenario_ADDRESS_CREATE_04 = it title $ \ctx -> runResourceT $ do
    ...
    rL <- request @[ApiAddress n] ctx (Link.listAddresses @'Cole w) Default Empty
    verify rL
        [ expectResponseCode HTTP.status200
...
scenario_ADDRESS_IMPORT_01
    ...
    => (Context -> ResourceT IO (ApiColeWallet, Mnemonic 12))
    -> SpecWith Context
scenario_ADDRESS_IMPORT_01 fixture = it title $ \ctx -> runResourceT $ do
    ...
    -- Import it
    r1 <- request @[ApiAddress n] ctx (Link.listAddresses @'Cole w) Default Empty
    verify r1
...
scenario_ADDRESS_IMPORT_02
    ...
    => (Context -> ResourceT IO (ApiColeWallet, Mnemonic 15))
    -> SpecWith Context
