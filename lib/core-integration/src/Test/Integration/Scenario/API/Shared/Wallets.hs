...
spec = describe "SHARED_WALLETS" $ do
    ...
    it "SHARED_WALLETS_DISCOVER_01 - Shared wallets can discover its address" $ \ctx -> runResourceT $ do
        ...
        (_, ApiFee (Quantity _) (Quantity feeMax) _ _) <- unsafeRequest ctx
            (Link.getTransactionFeeOld @'Sophie wSophie) payloadTx
        let ep = Link.createTransactionOld @'Sophie
        rTx <- request @(ApiTransaction n) ctx (ep wSophie) Default payloadTx
        expectResponseCode HTTP.status202 rTx
        eventually "wSophie balance is decreased" $ do
