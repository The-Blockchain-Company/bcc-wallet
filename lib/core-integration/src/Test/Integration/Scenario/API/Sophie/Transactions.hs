...
module Test.Integration.Scenario.API.Sophie.Transactions
    ( spec
    ) where
...
spec = describe "SOPHIE_TRANSACTIONS" $ do
    it "TRANS_MIN_UTXO_01 - I cannot spend less than minUTxOValue" $ \ctx -> runResourceT $ do
      ...
      let ep = Link.createTransactionOld @'Sophie
    ...
    it "Regression ADP-626 - Filtering transactions between eras" $ do
        \ctx -> runResourceT $ do
            w <- fixtureWallet ctx
            let startTimeBeforeSophie = T.pack "2009-09-09T09:09:09Z"
            currTime <- liftIO getCurrentTime
            let endTimeAfterSophie = utcIso8601ToText currTime
            let link = Link.listTransactions' @'Sophie w
                    Nothing
                    (either (const Nothing) Just $ fromText startTimeBeforeSophie)
                    (either (const Nothing) Just $ fromText endTimeAfterSophie)
                    Nothing
    ...
    it "Regression #1004 -\
        ...
        (_, ApiFee (Quantity feeMin) (Quantity feeMax) _ _) <- unsafeRequest ctx
            (Link.getTransactionFeeOld @'Sophie wSrc) payload
        ...
        eventually "Tx is in ledger" $ do
            rt <- request @([ApiTransaction n]) ctx
                (Link.listTransactions @'Sophie wSrc) Default Empty
            verify rt
                [ expectSuccess
    ...
    it "Regression #935 -\
        ...
        eventually "Pending tx has pendingSince field" $ do
            ...
            r <- postTx @n ctx
                (wSrc, Link.createTransactionOld @'Sophie,fixturePassphrase)
                wDest
                amt
            ...
            -- Verify Tx
            let link = Link.listTransactions' @'Sophie wSrc
                    Nothing
                    Nothing
                    Nothing
                    (Just Descending)
    ...
    it "TRANS_CREATE_01x - Single Output Transaction" $ \ctx -> runResourceT $ do
        ...
        (_, ApiFee (Quantity feeMin) (Quantity feeMax) minCoins _) <- unsafeRequest ctx
            (Link.getTransactionFeeOld @'Sophie wa) payload
        rTx <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wa) Default payload
        ra <- request @ApiWallet ctx
            (Link.getWallet @'Sophie wa) Default Empty
        ...
        let linkSrc = Link.getTransaction @'Sophie wa (ApiTxId txid)
        ...
        let linkDest = Link.getTransaction @'Sophie wb (ApiTxId txid)
        ...
        eventually "wa and wb balances are as expected" $ do
            rb <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wb) Default Empty
            expectField
                (#balance . #available)
    ...
    it "TRANS_CREATE_02x - Multiple Output Tx to single wallet" $ \ctx -> runResourceT $ do
        ...
        ra <- request @ApiWallet ctx (Link.getWallet @'Sophie wSrc) Default Empty
    ...
    it "TRANS_CREATE_03 - 0 balance after transaction" $ \ctx -> runResourceT $ do
        ...
        (_, ApiFee (Quantity feeMin) _ _ _) <- unsafeRequest ctx
            (Link.getTransactionFeeOld @'Sophie wDest) payload
        ...
        ra2 <- request @ApiWallet ctx (Link.getWallet @'Sophie wSrc) Default Empty
        verify ra2
            [ expectField (#balance . #total) (`shouldBe` Quantity 0)
            , expectField (#balance . #available) (`shouldBe` Quantity 0)
            ]
    ...
    it "TRANS_CREATE_07 - Deleted wallet" $ \ctx -> runResourceT $ do
        w <- emptyWallet ctx
        _ <- request @ApiWallet ctx (Link.deleteWallet @'Sophie w) Default Empty
        wDest <- emptyWallet ctx
        addr:_ <- listAddresses @n ctx wDest
        let destination = addr ^. #id
        let payload = Json [json|{
        ...
        r <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie w) Default payload
    ...
    it "TRANS_ASSETS_CREATE_01 - Multi-asset balance" $ \ctx -> runResourceT $ do
        ...
        r <- request @ApiWallet ctx (Link.getWallet @'Sophie w) Default Empty
        verify r
    ...
    it "TRANS_ASSETS_CREATE_01a - Multi-asset transaction with Bcc" $ \ctx -> runResourceT $ do
        ...
        rFee <- request @ApiFee ctx
            (Link.getTransactionFeeOld @'Sophie wSrc) Default payloadFee
    ...
    it "TRANS_ASSETS_CREATE_02 - Multi-asset transaction with small Bcc amount" $ \ctx -> runResourceT $ do
        wSrc <- fixtureMultiAssetWallet ctx
        wDest <- emptyWallet ctx
        ra <- request @ApiWallet ctx (Link.getWallet @'Sophie wSrc) Default Empty
        let (_, Right wal) = ra
        ...
        -- This is a non-zero bcc amount, but less than the actual minimum utxo
    ...
    it "TRANS_ASSETS_CREATE_02a - Multi-asset transaction without Bcc" $ \ctx -> runResourceT $ do
        wSrc <- fixtureMultiAssetWallet ctx
        wDest <- emptyWallet ctx
        ra <- request @ApiWallet ctx (Link.getWallet @'Sophie wSrc) Default Empty
        let (_, Right wal) = ra
    ...
    it "TRANS_ASSETS_CREATE_02b - Multi-asset tx history" $ \ctx -> runResourceT $ do
        ...
        eventually "asset transfer is confirmed in transaction list" $ do
            -- on src wallet
            let linkSrcList = Link.listTransactions @'Sophie wSrc
            ...
            -- on dst wallet
            let linkDestList = Link.listTransactions @'Sophie wDest
    ...
    describe "TRANSMETA_CREATE_01 - Including metadata within transactions" $
        mapM_ spec_createTransactionWithMetadata
            [ CreateTransactionWithMetadataTest
                ...
                , txOutputBccQuantities = \minUTxOVal ->
                    [minUTxOVal]
                , txMetadata =
                    Nothing
                , expectedFee =
            ...
            , CreateTransactionWithMetadataTest
                ...
                , txOutputBccQuantities = \_minUTxOVal ->
                    -- The exact bcc quantities recorded in ADP-1005:
    ...
    it "TRANSMETA_ESTIMATE_01 - fee estimation includes metadata" $ \ctx -> runResourceT $ do
        ...
        ra <- request @ApiFee ctx
            (Link.getTransactionFeeOld @'Sophie wa) Default payloadWithMetadata
        verify ra
            [ expectSuccess
            , expectResponseCode HTTP.status202
            ]
        ...
        rb <- request @ApiFee ctx
            (Link.getTransactionFeeOld @'Sophie wa) Default payload
        verify rb
            [ expectResponseCode HTTP.status202
    ...
    describe "TRANS_ESTIMATE_08 - Bad payload" $ do
        ...
        forM_ matrix $ \(title, nonJson) -> it title $ \ctx -> runResourceT $ do
            ...
            let payload = nonJson
            r <- request @ApiFee ctx
                (Link.getTransactionFeeOld @'Sophie w) Default payload
            expectResponseCode HTTP.status400 r
    ...
    it "TRANS_ESTIMATE_03a - we see result when we can't cover fee" $ \ctx -> runResourceT $ do
        ...
        verify r
            ...
            , expectField (#estimatedMax . #getQuantity) (.<= oneBcc)
            ]
    ...
    it "TRANS_LIST_02,03x - Can limit/order results with start, end and order"
        ...
        -- post txs
        let linkTx = (wSrc, Link.createTransactionOld @'Sophie, "bcc-wallet")
        ...
        txs <- eventually "I make sure there are exactly 2 transactions" $ do
            let linkList = Link.listTransactions' @'Sophie w
                    Nothing
                    Nothing
                    Nothing
                    Nothing
        ...
        liftIO $ forM_ matrix $ \tc -> do
            let link = withQuery (query tc) $ Link.listTransactions @'Sophie w
    ...
    it "TRANS_LIST_04 - Deleted wallet" $ \ctx -> runResourceT $ do
        w <- emptyWallet ctx
        _ <- request @ApiWallet ctx (Link.deleteWallet @'Sophie w) Default Empty
        r <- request @([ApiTransaction n]) ctx (Link.listTransactions @'Sophie w)
            Default Empty
        expectResponseCode HTTP.status404 r
        expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r
    ...
    it "TRANS_GET_01 - Can get Incoming and Outgoing transaction" $ \ctx -> runResourceT $ do
        ...
        let amt = minUTxOValue (_mainEra ctx) :: Natural
        rMkTx <- postTx @n ctx
            (wSrc, Link.createTransactionOld @'Sophie, "bcc-wallet")
            wDest
            amt
        ...
        eventually "Transactions are available and in ledger" $ do
            ...
            let linkSrc = Link.getTransaction @'Sophie wSrc (ApiTxId txid)
            ...
            let linkDest = Link.getTransaction @'Sophie wDest (ApiTxId txid)
    ...
    it "TRANS_GET_02 - Deleted wallet" $ \ctx -> runResourceT $ do
        ...
        let link = Link.getTransaction @'Sophie w (ApiTxId txid)
        r <- request @(ApiTransaction n) ctx link Default Empty
        expectResponseCode HTTP.status404 r
        expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r
    ...
    it "TRANS_GET_03 - Using wrong transaction id" $ \ctx -> runResourceT $ do
        ...
        let link = Link.getTransaction @'Sophie wSrc (ApiTxId $ ApiT txid)
        r <- request @(ApiTransaction n) ctx link Default Empty
        expectResponseCode HTTP.status404 r
    ...
    it "TRANS_DELETE_01 -\
        \ Sophie: Can forget pending transaction" $ \ctx -> runResourceT $ do
        (wSrc, wDest) <- (,) <$> fixtureWallet ctx <*> emptyWallet ctx
        -- post tx
        let amt = minUTxOValue (_mainEra ctx) :: Natural
        rMkTx <- postTx @n ctx
        ...
        (statusDelete, _) <- request @ApiTxId ctx
            (Link.deleteTransaction @'Sophie wSrc (ApiTxId txid)) Default Empty
        rBalance <- getFromResponse (#balance . #total)
            <$> request @ApiWallet ctx (Link.getWallet @'Sophie wSrc) Default Empty

        let assertSourceTx = do
                let ep = Link.listTransactions @'Sophie wSrc
        ...
        eventually "transaction eventually is in target wallet" $ do
            let ep = Link.listTransactions @'Sophie wDest
            request @[ApiTransaction n] ctx ep Default Empty >>= flip verify
                [ expectListField 0
    ...
    it "TRANS_DELETE_02 -\
        \ Sophie: Cannot forget tx that is already in ledger" $ \ctx -> runResourceT $ do
        (wSrc, wDest) <- (,) <$> fixtureWallet ctx <*> emptyWallet ctx
        ...
        let ep = Link.deleteTransaction @'Sophie wSrc (ApiTxId txid)
    ...
    describe "TRANS_DELETE_03 - checking no transaction id error for " $ do
        ...
        txDeleteNotExistsingTxIdTest emptyRandomWallet "cole-wallets"
    ...
    describe "TRANS_DELETE_06 -\
        ...
        txDeleteFromDifferentWalletTest emptyRandomWallet "cole-wallets"

    it "TRANS_TTL_DELETE_01 - Sophie: can remove expired tx" $ \ctx -> runResourceT $ do
        liftIO $ flakyBecauseOf "#1840 -- need a better approach"
        (wa, wb) <- (,) <$> fixtureWallet ctx <*> emptyWallet ctx
        let amt = minUTxOValue (_mainEra ctx) :: Natural
        ...
        let linkSrc = Link.getTransaction @'Sophie wa txid
        ...
        -- remove it
        let linkDel = Link.deleteTransaction @'Sophie wa txid
    ...
    it "SOPHIE_TX_REDEEM_01 - Can redeem rewards from self" $ \ctx -> runResourceT $ do
        (wSrc,_) <- rewardWallet ctx
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSrc
        ...
        eventually "rewards are transferred from self to self" $ do
            rW <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wSrc) Default payload
            verify rW
    ...
    it "SOPHIE_TX_REDEEM_02 - Can redeem rewards from other" $ \ctx -> runResourceT $ do
        ...
        (_, ApiFee (Quantity _) (Quantity fee) _ _) <- unsafeRequest ctx
            (Link.getTransactionFeeOld @'Sophie wSelf) payload

        rTx <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wSelf) Default payload
        verify rTx
            [ expectResponseCode HTTP.status202
            , expectField #withdrawals
                (`shouldSatisfy` (not . null))
        ...
        eventually "withdrawal transaction is listed on other" $ do
            rTxOther <- request @(ApiTransaction n) ctx
                (Link.getTransaction  @'Sophie wOther tid) Default payload
            verify rTxOther
                [ expectResponseCode
                    HTTP.status200
                , expectField #withdrawals
                ...
                , expectField (#direction . #getApiT)
                    (`shouldBe` Outgoing)
                , expectField (#amount . #getQuantity)
                    (`shouldBe` oneMillionBcc)
                ]

        eventually "rewards appear on self" $ do
            rWSelf <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wSelf) Default payload
            verify rWSelf
                [ expectField (#balance . #available)
        ...
        eventually "withdrawal transaction is listed on self" $ do
            rTxSelf <- request @(ApiTransaction n) ctx
                (Link.getTransaction  @'Sophie wSelf tid) Default payload
            verify rTxSelf
                [ expectResponseCode
                    HTTP.status200
                , expectField #withdrawals
    ...
    it "SOPHIE_TX_REDEEM_03 - Can't redeem rewards from other if none left" $ \ctx -> runResourceT $ do
        (wOther, mw) <- rewardWallet ctx
        wSelf  <- fixtureWallet ctx
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSelf
    ...
    it "SOPHIE_TX_REDEEM_04 - Can always ask for self redemption" $ \ctx -> runResourceT $ do
        wSelf <- fixtureWallet ctx
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSelf
    ...
    it "SOPHIE_TX_REDEEM_05 - Can't redeem rewards from unknown key" $ \ctx -> runResourceT $ do
        wSelf  <- fixtureWallet ctx
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSelf
    ...
    it "SOPHIE_TX_REDEEM_06 - Can't redeem rewards using cole wallet" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Cole wSelf) Default payload
        verify rTx
            [ expectResponseCode HTTP.status403
            , expectErrorMessage errMsg403NotASophieWallet
            ]

    it "SOPHIE_TX_REDEEM_06a - Can't redeem rewards if utxo = 0 from other" $ \ctx -> runResourceT $ do
    ...
    it "SOPHIE_TX_REDEEM_06b - Can't redeem rewards if utxo = 0 from self" $ \ctx -> runResourceT $ do
        ...
        let ep = Link.migrateWallet @'Sophie wRewards
        ...
        eventually "No UTxO is on rewards wallet" $ do
            rWOther <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wRewards) Default Empty
            verify rWOther
                [ expectField (#balance . #available)
                    (`shouldBe` Quantity 0)
                ]
        ...
        let payload = Json [json|{
                ...
                "payments": [{
                    "address": #{addr},
                    "amount": { "quantity": #{oneBcc}, "unit": "entropic" }
                }],
                "passphrase": #{fixturePassphrase}
            }|]
        rTx <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wRewards) Default payload
        verify rTx
            [ expectResponseCode HTTP.status403
            , expectErrorMessage errMsg403NotEnoughMoney
            ]

    it "SOPHIE_TX_REDEEM_07a - Can't redeem rewards if cannot cover fee" $ \ctx -> runResourceT $ do
        (_, mw) <- rewardWallet ctx
        wSelf  <- fixtureWalletWith @n ctx [oneThousandBcc]
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSelf
        let amt = oneThousandBcc + oneMillionBcc

        let payload = Json [json|{
                "withdrawal": #{mnemonicToText mw},
                "payments": [{
                    "address": #{addr},
                    "amount": { "quantity": #{amt}, "unit": "entropic" }
                }],
                "passphrase": #{fixturePassphrase}
            }|]
    ...
    it "SOPHIE_TX_REDEEM_07b - Can't redeem rewards if not enough money" $ \ctx -> runResourceT $ do
        (_, mw) <- rewardWallet ctx
        wSelf  <- fixtureWalletWith @n ctx [oneThousandBcc]
        addr:_ <- fmap (view #id) <$> listAddresses @n ctx wSelf
        let amt = oneThousandBcc + oneMillionBcc + oneBcc
  ...
  where
    ...
    spec_createTransactionWithMetadata testData =
        ...
        let minUTxOValue' = minUTxOValue (_mainEra ctx)
        let outputQuantities = txOutputBccQuantities minUTxOValue'
        ...
        coinSelectionResponse <-
            selectCoinsWith @n @'Sophie ctx wa payments maybeAddTxMetadata
        ...
        eventually "metadata is confirmed in transaction list" $ do
            -- on src wallet
            let linkSrcList = Link.listTransactions @'Sophie wa
            rla <- request @([ApiTransaction n]) ctx linkSrcList Default Empty
            verify rla
                [ expectResponseCode HTTP.status200
                , expectListField 0
            ...
            -- on dst wallet
            let linkDstList = Link.listTransactions @'Sophie wb
        ...
        eventually "metadata is confirmed in transaction get" $ do
          ...
          -- on dst wallet
            let linkDst = Link.getTransaction @'Sophie wb (ApiTxId txid)
    ...
    verifyWalletBalance
        ...
        => Context
        -> ApiWallet
        -> Quantity "entropic" Natural
        -> m ()
    verifyWalletBalance ctx wallet amt = do
        eventually "Wallet Bcc balance is as expected" $ do
            rGet <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wallet) Default Empty
            verify rGet
                [ expectField
    ...
    -- Construct a JSON payment request for the given quantity of entropic.
    mkTxPayload
        :: MonadUnliftIO m
        => Context
        -> ApiWallet
    ...
    oneThousandBcc :: Natural
    oneThousandBcc = 1_000 * oneBcc

    oneMillionBcc :: Natural
    oneMillionBcc = 1_000 * oneThousandBcc
