...
module Test.Integration.Scenario.API.Sophie.Addresses
    ( spec
    ) where
...
spec = describe "SOPHIE_ADDRESSES" $ do
    it "COLE_ADDRESS_LIST - Cole wallet on Sophie ep" $ \ctx -> runResourceT $ do
    ...
    it "ADDRESS_LIST_02 - Can filter used and unused addresses" $ \ctx -> runResourceT $ do
        ...
        rUsed <- request @[ApiAddress n] ctx
            (Link.listAddresses' @'Sophie w (Just Used)) Default Empty
        ...
        rUnused <- request @[ApiAddress n] ctx
            (Link.listAddresses' @'Sophie w (Just Unused)) Default Empty
    ...
    -- TODO
    -- MOVE TO test/unit/Bcc/Wallet/ApiSpec.hs
    describe "ADDRESS_LIST_02 - Invalid filters are bad requests" $ do
        ...
        forM_ filters $ \fil -> it fil $ \ctx -> runResourceT $ do
            w <- emptyWallet ctx
            let link = withQuery fil $ Link.listAddresses @'Sophie w
    ...
    it "ADDRESS_LIST_03 - Generates new address pool gap" $ \ctx -> runResourceT $ do
        ...
        r <- request @[ApiAddress n] ctx
            (Link.listAddresses @'Sophie wDest) Default Empty
        verify r
    ...
    it "ADDRESS_LIST_06 - Used change addresses are listed after a transaction is no longer pending" $ \ctx -> runResourceT @IO $ do
        ...
        (_, rtx) <- unsafeRequest @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wA) payload
        ...
        eventually "Transaction from A -> B is discovered on B" $ do
            request @(ApiTransaction n) ctx
                (Link.getTransaction @'Sophie wA rtx) Default Empty
                >>= expectField #status (`shouldBe` ApiT InLedger)
            request @(ApiTransaction n) ctx
                (Link.getTransaction @'Sophie wB rtx) Default Empty
                >>= expectField #status (`shouldBe` ApiT InLedger)
    ...
    --- $ bcc-address script hash "$(cat script.txt)" \
    --- | bcc-address address payment --from-script --network-tag mainnet
    ...
    --- $ bcc-address script hash "$(cat script.txt)" \
    --- | bcc-address address stake --from-script --network-tag mainnet
    ...
    --- $ bcc-address script hash "$(cat script1.txt)" \
    --- | bcc-address address payment --from-script --network-tag mainnet \
    --- | bcc-address address delegation --from-script $(bcc-address script hash "$(cat script2.txt)")
    ...
    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie \
    --- > | bcc-address key child 1852H/1815H/0H/0/0 \
    --- > | bcc-address key public --without-chain-code
    --- xpub1lqglg77z6kajsdz4739q22c0zm0yhuy567z6xk2vc0z5ucjtkwps75l8wa
    -- which can be translated in bcc-addresses
    ...
    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie \
    --- > | bcc-address key child 1852H/1815H/0H/0/0 \
    --- > | bcc-address key public --with-chain-code \
    --- > | bcc-address address payment --from-key --network-tag mainnet
    ...
    --- east student silly already breeze enact seat trade few way online skin grass humble electric
    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie \
    --- > | bcc-address key child 1852H/1815H/0H/2/0 \
    --- > | bcc-address key public --without-chain-code
    ...
    --- > | bcc-address key child 1852H/1815H/0H/2/0 \
    --- > | bcc-address key public --with-chain-code \
    --- > | bcc-address address stake --from-key --network-tag mainnet
    ...
    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie \
    --- > | bcc-address key child 1852H/1815H/0H/2/0 \
    --- > | bcc-address key public --with-chain-code > stake.xpub

    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie \
    --- > | bcc-address key child 1852H/1815H/0H/0/0 \
    --- > | bcc-address key public --with-chain-code \
    --- > | bcc-address address payment --from-key --network-tag mainnet \
    --- > | bcc-address address delegation --from-key $(cat stake.xpub)
    ...
    --- > | bcc-address key public --with-chain-code \
    --- > | bcc-address address payment --from-key --network-tag mainnet \
    --- > | bcc-address address delegation --from-script $(bcc-address script hash "$(cat script3.txt)")
    ...
    it "ANY_ADDRESS_POST_12 - Delegating addresses API roundtrip" $ \ctx -> runResourceT $ do
        ...
        generatedAddresses <- forM indices $ \index -> do
            let paymentPath = Link.getWalletKey @'Sophie w UtxoExternal (DerivationIndex index) Nothing
            (_, paymentKey) <- unsafeRequest @ApiVerificationKeySophie ctx paymentPath Empty

            let stakePath = Link.getWalletKey @'Sophie w MutableAccount (DerivationIndex 0) Nothing
            (_, stakeKey) <- unsafeRequest @ApiVerificationKeySophie ctx stakePath Empty
    ...
    it "POST_ACCOUNT_01 - Can retrieve account public keys" $ \ctx -> runResourceT $ do
        ...
        let endpoint = Link.postAccountKey @'Sophie w (DerivationIndex 0)
        let payload = Json [json|{
                "passphrase": #{fixturePassphrase},
                "format": "extended"
            }|]
        ...
        accountPublicKeys <- forM indices $ \index -> do
            let accountPath = Link.postAccountKey @'Sophie w (DerivationIndex $ 2147483648 + index)
    ...
    it "POST_ACCOUNT_02 - Can get account public key using purpose" $ \ctx -> runResourceT $ do
        let initPoolGap = 10
        w <- emptyWalletWith ctx ("Wallet", fixturePassphrase, initPoolGap)
        let accountPath = Link.postAccountKey @'Sophie w (DerivationIndex $ 2147483648 + 1)
        let payload1 = Json [json|{
                "passphrase": #{fixturePassphrase},
                "format": "extended"
            }|]
