...
module Test.Integration.Scenario.API.Sophie.StakePools
    ( spec
    ) where
...
spec = describe "SOPHIE_STAKE_POOLS" $ do
    ...
    it "STAKE_POOLS_JOIN_01rewards - \
        ...
        eventually "Wallet has joined pool and deposit info persists" $ do
            rJoin' <- request @(ApiTransaction n) ctx
                (Link.getTransaction @'Sophie src
        ...
        let link = Link.getTransaction @'Sophie src (ApiTxId txId)
        ...
        (previousBalance, walletRewards) <-
            liftIO $ eventually "Wallet gets rewards" $ do
                r <- request @ApiWallet ctx (Link.getWallet @'Sophie src)
                    Default Empty
                verify r
                    [ expectField
                        (#balance . #reward)
        ...
        eventually "Wallet has not consumed rewards" $ do
          ...
          request @ApiWallet ctx (Link.getWallet @'Sophie src) Default Empty
              >>= flip verify
                  [ expectField
        ...
        rw1 <- request @[ApiTransaction n] ctx
            (Link.listTransactions' @'Sophie src (Just 1)
        ...
        eventually "There's one incoming transaction with correct amount" $ do
            request @[ApiTransaction n] ctx (Link.listTransactions @'Sophie dest)
        ...
        eventually "Certificates are inserted after quiting a pool" $ do
            let epg = Link.getTransaction @'Sophie src txid
            ...
            let epl = Link.listTransactions @'Sophie src
    ...
    it "STAKE_POOLS_JOIN_01 - Can rejoin another stakepool" $ \ctx -> runResourceT $ do
        ...
        request @ApiWallet ctx (Link.getWallet @'Sophie w) Default Empty
            >>= flip verify
    ...
    describe "STAKE_POOLS_JOIN_UNSIGNED_01" $ do
        it "Can join a pool that's not retiring" $ \ctx -> runResourceT $ do
            ...
            liftIO $ joinStakePoolUnsigned
                @n @'Sophie ctx w nonRetiringPoolId >>= \o -> do
                verify o
                    [ expectResponseCode HTTP.status200
                    , expectField #inputs
    ...
    describe "STAKE_POOLS_JOIN_UNSIGNED_02"
        $ it "Can join a pool that's retiring" $ \ctx -> runResourceT $ do
            ...
            -- Join Pool
            w <- fixtureWallet ctx
            liftIO $ joinStakePoolUnsigned @n @'Sophie ctx w retiringPoolId >>= \o -> do
                verify o
                    [ expectResponseCode HTTP.status200
                    , expectField #inputs
                        (`shouldSatisfy` (not . null))
    ...
    describe "STAKE_POOLS_JOIN_UNSIGNED_03"
        $ it "Cannot join a pool that's retired" $ \ctx -> runResourceT $ do
            ...
            w <- fixtureWallet ctx
            r <- liftIO $ joinStakePoolUnsigned @n @'Sophie ctx w (ApiT retiredPoolId)
            expectResponseCode HTTP.status404 r
            expectErrorMessage (errMsg404NoSuchPool (toText retiredPoolId)) r
    ...
    describe "STAKE_POOLS_JOIN_UNSIGNED_04"
        $ it "Cannot join a pool that's never existed" $ \ctx -> runResourceT $ do
            ...
            r <- liftIO $ joinStakePoolUnsigned
                @n @'Sophie ctx w (ApiT non_existing_pool_id)
            expectResponseCode HTTP.status404 r
            expectErrorMessage
    ...
    describe "STAKE_POOLS_QUIT_UNSIGNED_01"
        $ it "Join/quit when already joined a pool" $ \ctx -> runResourceT $ do
            ...
            eventually "Wallet is delegating to p1" $ do
                request @ApiWallet ctx (Link.getWallet @'Sophie w)
                    Default Empty >>= flip verify
                    [ expectField #delegation (`shouldBe` delegating pool1 [])
                    ]
            ...
            liftIO $ joinStakePoolUnsigned @n @'Sophie ctx w pool1 >>= \o -> do
                verify o
                    [ expectResponseCode HTTP.status403
            ...
            liftIO $ joinStakePoolUnsigned @n @'Sophie ctx w pool2 >>= \o -> do
                verify o
                    [ expectResponseCode HTTP.status200
                    , expectField #inputs
                        (`shouldSatisfy` (not . null))
            ...
            liftIO $ quitStakePoolUnsigned @n @'Sophie ctx w >>= \o -> do
                verify o
                    [ expectResponseCode HTTP.status200
                    , expectField #inputs
                        (`shouldSatisfy` (not . null))
    ...
    describe "STAKE_POOLS_QUIT_UNSIGNED_02"
        $ it "Cannot quit if not delegating" $ \ctx -> runResourceT $ do
            ...
            quitStakePoolUnsigned @n @'Sophie ctx w >>= \r -> do
    ...
    describe "STAKE_POOLS_LIST_01 - List stake pools" $ do
        ...
        it "eventually has correct margin, cost and pledge" $ \ctx -> runResourceT $ do
            eventually "pool worker finds the certificate" $ do
                r <- listPools ctx arbitraryStake
                expectResponseCode HTTP.status200 r
                let oneMillionBcc = 1_000_000_000_000
                ...
                setOf pools' (view #pledge)
                    `shouldBe`
                    Set.fromList
                        [ Quantity oneMillionBcc
                        , Quantity $ 2 * oneMillionBcc
                        ]
    ...
    it "STAKE_KEY_LIST_01 - Can list stake keys" $ \ctx -> runResourceT $ do
        ...
        -- fixtureWallets have funds on payment addresses, so their entire bcc
        ...
        request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie w)
            Default (Json payload)
            >>= flip verify
                [ expectResponseCode HTTP.status202 ]
...
-- in the 'operators' constant of 'Bcc.Wallet.Sophie.Launch'.
--
