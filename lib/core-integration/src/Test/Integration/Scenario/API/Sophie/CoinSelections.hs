...
module Test.Integration.Scenario.API.Sophie.CoinSelections
    ( spec
    ) where
...
spec = describe "SOPHIE_COIN_SELECTION" $ do
    ...
    it "WALLETS_COIN_SELECTION_01 - \
        ...
        \ctx -> runResourceT $ do
            ...
            selectCoins @_ @'Sophie ctx source (payment :| []) >>= flip verify
    ...
    it "WALLETS_COIN_SELECTION_02 - \
        ...
        \ctx -> runResourceT $ do
            ...
            selectCoins @_ @'Sophie ctx source payments >>= flip verify
                [ expectResponseCode HTTP.status200
    ...
    it "WALLETS_COIN_SELECTION_03 - \
        ...
        selectCoins @_ @'Sophie ctx w payments >>= flip verify
    ...
    describe "WALLETS_COIN_SELECTION_04 - HTTP headers" $ do
        ...
        forM_ matrix $ \(title, headers, expectations) -> it title $ \ctx -> runResourceT $ do
            ...
            r <- request @(ApiCoinSelection n) ctx
                (Link.selectCoins @'Sophie w) headers payload
    ...
    it "WALLETS_COIN_SELECTION_05a - can include metadata" $ \ctx -> runResourceT $ do
        ...
        selectCoinsWith @_ @'Sophie ctx source (payment :| []) transform >>= flip verify
            [ expectResponseCode HTTP.status200
    ...
    it "WALLETS_COIN_SELECTION_08 - \
        ...
        \ctx -> runResourceT $ do
            ...
            let nonBccQuantities = TokenMap.fromFlatList $
