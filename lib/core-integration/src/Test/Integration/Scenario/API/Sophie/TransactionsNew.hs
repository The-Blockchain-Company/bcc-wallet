...
module Test.Integration.Scenario.API.Sophie.TransactionsNew
    ( spec
    ) where
...
spec = describe "NEW_SOPHIE_TRANSACTIONS" $ do
    ...
    it "TRANS_NEW_CREATE_01b - Validity interval only is not allowed" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default validityInterval
        verify rTx
            [ expectResponseCode HTTP.status403
            , expectErrorMessage errMsg403InvalidConstructTx
            ]
    ...
    it "TRANS_NEW_CREATE_01c - No payload is bad request" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default Empty
        verify rTx
    ...
    it "TRANS_NEW_CREATE_02 - Only metadata" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default metadata
        verify rTx
    ...
    it "TRANS_NEW_CREATE_03a - Withdrawal from self" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default withdrawal
        verify rTx
            [ expectResponseCode HTTP.status202
    ...
    it "TRANS_NEW_CREATE_04a - Single Output Transaction" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default payload
        verify rTx
            [ expectSuccess
            , expectResponseCode HTTP.status202
    ...
    it "TRANS_NEW_ASSETS_CREATE_01a - Multi-asset tx with Bcc" $ \ctx -> runResourceT $ do

        liftIO $ pendingWith "Missing outputs on response - to be fixed in ADP-985"

        wa <- fixtureMultiAssetWallet ctx
        wb <- emptyWallet ctx
        ra <- request @ApiWallet ctx (Link.getWallet @'Sophie wa) Default Empty
        let (_, Right wal) = ra
    ...
    it "TRANS_NEW_ASSETS_CREATE_01b - Multi-asset tx with not enough Bcc" $ \ctx -> runResourceT $ do
        wa <- fixtureMultiAssetWallet ctx
        wb <- emptyWallet ctx
        ra <- request @ApiWallet ctx (Link.getWallet @'Sophie wa) Default Empty
        let (_, Right wal) = ra
        ...
        verify rTx
            [ expectResponseCode HTTP.status403
            , expectErrorMessage "Some outputs have bcc values that are too small."
            ]

    it "TRANS_NEW_ASSETS_CREATE_01c - Multi-asset tx without Bcc" $ \ctx -> runResourceT $ do
    ...
    it "TRANS_NEW_JOIN_01a - Can join stakepool" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.createUnsignedTransaction @'Sophie wa) Default delegation
        verify rTx
            [ expectResponseCode HTTP.status202
  ...
  -- end to end join pool and get rewards
    ...
    it "TRANS_NEW_BALANCE_01a - multiple-output transaction with all covering inputs present" $ \ctx -> runResourceT $ do
        ...
        let balancePayload = Json [json|{
              "transaction": { "cborHex" : #{serializedTxHex}, "description": "", "type": "Tx AurumEra" },
              "signatories": [],
              "inputs": [
                  { "txIn" : "0eaa33be8780935ca5a7c1e628a2d54402446f96236ca8f1770e07fa22ba8648#0"
                  , "txOut" :
                      { "value" : { "entropic": 100000000000 }
                      , "address": "addr1vx0d0kyppx3qls8laq5jvpq0qa52d0gahm8tsyj2jpg0lpg4ue9lt"
                      }
                  }]
          }|]
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.balanceTransaction @'Sophie wa) Default balancePayload
        verify rTx
            [ expectResponseCode HTTP.status403
    ...
    it "TRANS_NEW_BALANCE_01b - incorrect cbor - not hex-encoded" $ \ctx -> runResourceT $ do
        ...
        let balancePayload = Json [json|{
              "transaction": { "cborHex" : #{serializedTx}, "description": "", "type": "Tx AurumEra" },
              "signatories": [],
              "inputs": []
          }|]
        rTx <- request @(ApiConstructTransaction n) ctx
    ...
    it "TRANS_NEW_BALANCE_01c - incorrect cbor - hex-encoded but cannot deserialize into sealedTx" $ \ctx -> runResourceT $ do
        ...
        rTx <- request @(ApiConstructTransaction n) ctx
            (Link.balanceTransaction @'Sophie wa) Default balancePayload
        verify rTx
            [ expectErrorMessage "Error in $: cborHex seems to be not deserializing correctly due to DecoderErrorDeserialiseFailure 'Sophie Tx' (DeserialiseFailure 5 'expected bytes'" ]
    ...
    it "TRANS_NEW_BALANCE_01e - zerepoch with missing covering inputs wallet enough funds" $ \ctx -> runResourceT $ do
        ...
        let balancePayload = Json [json|{
              "transaction": { "cborHex" : #{serializedZerepochTx}, "description": "", "type": "Tx AurumEra" },
              "signatories": [],
              "inputs": [
  ...
  where
    ...
    serializedZerepochTx :: Text
    serializedZerepochTx =
