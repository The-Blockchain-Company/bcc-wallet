...
module Test.Integration.Scenario.API.Sophie.Wallets
    ( spec
    ) where
...
-- FIXME:
-- give ways to construct and deconstruct an 'XSignature' in bcc-addresses,
...
spec = describe "SOPHIE_WALLETS" $ do
    it "WALLETS_CREATE_01 - Create a wallet" $ \ctx -> runResourceT $ do
        ...
        eventually "Wallet state = Ready" $ do
            rg <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wid) Default Empty
    ...
    describe "OWASP_INJECTION_CREATE_WALLET_01 - \
        ...
        forM_ matrix $ \(nameIn, nameOut) -> it nameIn $ \ctx -> runResourceT $ do
            ...
            eventually "listed wallet's state = Ready" $ do
                r2 <- request @ApiWallet ctx (Link.getWallet @'Sophie wid) Default Empty
                verify r2
    ...
    it "WALLETS_CREATE_02 - Restored wallet preserves funds" $ \ctx -> runResourceT $ do
        ...
        -- delete wallet
        rDel <- request @ApiWallet ctx (Link.deleteWallet @'Sophie wDest) Default Empty
    ...
    describe "WALLETS_CREATE_08 - address_pool_gap" $ do
        ...
        -- creating/using such wallets. Therefore using max gap that is used in Klarity.
        -- Performance issues are addressed in ADP-442, ADP-436
        let maxKlarityGap = (1_000 :: Word32)
        let addrPoolBig = fromIntegral maxKlarityGap
        ...
        forM_ matrix $ \(title, addrPoolGap, expectations) -> it title $ \ctx -> runResourceT $ do
            ...
            rA <- request @[ApiAddress n] ctx
                (Link.listAddresses @'Sophie w) Default Empty
    ...
    describe "WALLETS_UPDATE_PASS_05,06 - Transaction after updating passphrase" $ do
        let oldPass = "bcc-wallet"
        let newPass = "bcc-wallet2"
        ...
        forM_ matrix $ \(title, pass, expectations) -> it title $ \ctx -> runResourceT $ do
            ...
            rup <- request @ApiWallet ctx
                   (Link.putWalletPassphrase @'Sophie wSrc) Default payloadUpdate
            expectResponseCode HTTP.status204 rup
            ...
            r <- request @(ApiTransaction n) ctx
                (Link.createTransactionOld @'Sophie wSrc) Default payloadTrans
            verify r expectations
    ...
    describe "WALLETS_UPDATE_PASS_07 - HTTP headers" $ do
        ...
        forM_ matrix $ \(title, headers, expectations) -> it title $ \ctx -> runResourceT $ do
            ...
            let payload = updatePassPayload fixturePassphrase "Passphrase"
            let endpoint = Link.putWalletPassphrase @'Sophie w
    ...
    it "WALLETS_UTXO_01 - Wallet's inactivity is reflected in utxo" $ \ctx -> runResourceT $ do
        ...
        rStat <- request @ApiUtxoStatistics ctx
                 (Link.getUTxOsStatistics @'Sophie w) Default Empty
    ...
    it "WALLET_UTXO_SNAPSHOT_01 - \
        ...
        \ctx -> runResourceT $ do
            ...
            rSnap <- request @ApiWalletUtxoSnapshot ctx
                (Link.getWalletUtxoSnapshot @'Sophie w) Default Empty
    ...
    it "WALLETS_UTXO_02 - Sending and receiving funds updates wallet's utxo." $ \ctx -> runResourceT $ do
        ...
        rTrans <- request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wSrc) Default (Json payload)
        expectResponseCode HTTP.status202 rTrans
        ...
        rStat1 <- request @ApiUtxoStatistics ctx
            (Link.getUTxOsStatistics @'Sophie wDest) Default Empty
    ...
    it "WALLETS_UTXO_03 - Deleted wallet is not available for utxo" $ \ctx -> runResourceT $ do
        w <- emptyWallet ctx
        _ <- request @ApiWallet ctx (Link.deleteWallet @'Sophie w)
            Default Empty
        r <- request @ApiUtxoStatistics ctx (Link.getUTxOsStatistics @'Sophie w)
            Default Empty
    ...
    describe "WALLETS_UTXO_04 - HTTP headers" $ do
        ...
        forM_ matrix $ \(title, headers, expectations) -> it title $ \ctx -> runResourceT $ do
            w <- emptyWallet ctx
            r <- request @ApiUtxoStatistics ctx (Link.getUTxOsStatistics @'Sophie w) headers Empty
            verify r expectations
    ...
    --- $ cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie > root.prv
    --- $ cat root.prv \
    --- > | bcc-address key child 1852H/1815H/0H/ROLE/INDEX \
    --- > | bcc-address key public \
        ...
        forM_ matrix $ \(role_, index, expected) ->
            counterexample (show role_ <> "/" <> show index) $ do
                let link = Link.getWalletKey @'Sophie (apiWal ^. id) role_ index Nothing
                rGet <- request @ApiVerificationKeySophie ctx link Default Empty
                verify rGet
                    [ expectResponseCode HTTP.status200
    ...
    it "WALLETS_GET_KEY_02 - invalid index for verification key" $ \ctx -> runResourceT $ do
        ...
        let link = Link.getWalletKey @'Sophie w UtxoExternal (DerivationIndex 2147483648) Nothing
        r <- request @ApiVerificationKeySophie ctx link Default Empty
    ...
    it "WALLETS_GET_KEY_03 - unknown wallet" $ \ctx -> runResourceT $ do
        ...
        let link = Link.getWalletKey @'Sophie w UtxoExternal (DerivationIndex 0) Nothing
        r <- request @ApiVerificationKeySophie ctx link Default Empty
    ...
    it "WALLETS_SIGNATURES_01 - can verify signature" $ \ctx -> runResourceT $ do
        ...
        rKey <- request @ApiVerificationKeySophie ctx
            (Link.getWalletKey @'Sophie w role_ index Nothing)
            Default
            Empty
        verify rKey
            [ expectResponseCode HTTP.status200
    ...
    it "COLE_WALLETS_UTXO -\
        \ Cannot show Cole wal utxo with sophie ep (404)" $ \ctx -> runResourceT $ do
    ...
    it "COLE_WALLETS_UPDATE_PASS -\
        \ Cannot update Cole wal with sophie ep (404)" $ \ctx -> runResourceT $ do
        w <- emptyRandomWallet ctx
        let payload = updatePassPayload fixturePassphrase "Secure passphrase2"
        let wid = w ^. walletId
        let endpoint =
    ...
    it "COLE_GET_02 - Cole ep does not show Sophie wallet" $ \ctx -> runResourceT $ do
        w <- emptyWallet ctx
    ...
    it "COLE_GET_03 - Sophie ep does not show Cole wallet" $ \ctx -> runResourceT $ do
        w <- emptyRandomWallet ctx
    ...
    it "COLE_LIST_02,03 -\
        \ Cole wallets listed only via Cole endpoints \\\
        \ Sophie wallets listed only via new endpoints" $ \ctx -> runResourceT $ do
        m1 <- liftIO $ genMnemonics M12
        m2 <- liftIO $ genMnemonics M12
        m3 <- liftIO $ genMnemonics M12
        r1 <- emptyColeWalletWith ctx "random" ("cole1", m1, fixturePassphrase)
        r2 <- emptyColeWalletWith ctx "random" ("cole2", m2, fixturePassphrase)
        r3 <- emptyColeWalletWith ctx "random" ("cole3", m3, fixturePassphrase)

        r4 <- emptyWalletWith ctx ("sophie1", fixturePassphrase, 20)
        r5 <- emptyWalletWith ctx ("sophie2", fixturePassphrase, 20)
        r6 <- emptyWalletWith ctx ("sophie3", fixturePassphrase, 20)
        ...
        verify rl
            ...
            , expectListSize 3
            , expectListField 0
                    (#name . #getApiT . #getWalletName) (`shouldBe` "cole1")
            , expectListField 1
                    (#name . #getApiT . #getWalletName) (`shouldBe` "cole2")
            , expectListField 2
                    (#name . #getApiT . #getWalletName) (`shouldBe` "cole3")
            ]
        --list only sophie
        ...
        verify rl2
            ...
            , expectListSize 3
            , expectListField 0
                    (#name . #getApiT . #getWalletName) (`shouldBe` "sophie1")
            , expectListField 1
                    (#name . #getApiT . #getWalletName) (`shouldBe` "sophie2")
            , expectListField 2
                    (#name . #getApiT . #getWalletName) (`shouldBe` "sophie3")
            ]

    it "COLE_LIST_04, DELETE_01 -\
        \ Deleted wallets cannot be listed" $ \ctx -> runResourceT $ do
        m1 <- liftIO $ genMnemonics M12
        m2 <- liftIO $ genMnemonics M12
        m3 <- liftIO $ genMnemonics M12
        _wb1   <- emptyColeWalletWith ctx "random" ("cole1", m1, fixturePassphrase)
        wb2 <- emptyColeWalletWith ctx "random" ("cole2", m2, fixturePassphrase)
        _wb3   <- emptyColeWalletWith ctx "random" ("cole3", m3, fixturePassphrase)

        _ws1 <- emptyWalletWith ctx ("sophie1", fixturePassphrase, 20)
        _ws2 <- emptyWalletWith ctx ("sophie2", fixturePassphrase, 20)
        ws3 <- emptyWalletWith ctx ("sophie3", fixturePassphrase, 20)
        ...
        -- delete
        _ <- request @ApiColeWallet ctx (Link.deleteWallet @'Cole wb2) Default Empty
        _ <- request @ApiWallet ctx (Link.deleteWallet @'Sophie ws3) Default Empty

        --list only cole
        rdl <- listFilteredColeWallets wids ctx
        verify rdl
            [ expectResponseCode HTTP.status200
            , expectListSize 2
            , expectListField 0
    ...
    it "COLE_DELETE_02 - Cole ep does not delete Sophie wallet" $ \ctx -> runResourceT $ do
        w <- emptyWallet ctx
        r <- request @ApiColeWallet ctx (Link.deleteWallet @'Cole w) Default Empty
        expectResponseCode HTTP.status404 r
        expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r

    it "COLE_DELETE_03 - Sophie ep does not delete Cole wallet" $ \ctx -> runResourceT $ do
        w <- emptyRandomWallet ctx
        r <- request @ApiColeWallet ctx (Link.deleteWallet @'Sophie w) Default Empty
        expectResponseCode HTTP.status404 r
        expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r

    it "NETWORK_SOPHIE - Wallet has the same tip as network/information" $ \ctx -> runResourceT $ do
