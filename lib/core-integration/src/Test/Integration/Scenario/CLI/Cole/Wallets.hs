...
module Test.Integration.Scenario.CLI.Cole.Wallets
    ( spec
    ) where
...
import Bcc.Wallet.Api.Types
    ( ApiColeWallet, ApiUtxoStatistics, DecodeAddress )
...
spec = describe "COLE_CLI_WALLETS" $ do

    describe "CLI_COLE_GET_04, CLI_COLE_DELETE_01, COLE_RESTORE_02, COLE_RESTORE_03 -\
        ...
        forM_ matrix $ \(style, genM) -> it style $ \ctx -> runResourceT $ do
            ...
            j <- expectValidJSON (Proxy @ApiColeWallet) out
            ...
            jr <- expectValidJSON (Proxy @ApiColeWallet) out3
    ...
    describe "CLI_COLE_RESTORE_01, CLI_COLE_GET_01, CLI_COLE_LIST_01 -\
        \Restore a wallet" $ do
        let scenarioSuccess style mnemonic ctx = runResourceT @IO $ do
                ...
                eventually "wallet is available and ready" $ do
                    ...
                    jg <- expectValidJSON (Proxy @ApiColeWallet) out2
                    verify jg $
                    ...
                    c3 `shouldBe` ExitSuccess
                    err3 `shouldBe` cmdOk
                    jl <- expectValidJSON (Proxy @[ApiColeWallet]) out3
    ...
    describe "CLI_COLE_RESTORE_06 - Passphrase" $ do
        ...
        forM_ matrix $ \(title, passphrase) -> it title $
            \ctx -> runResourceT @IO $ do
                ...
                (c, out, err) <- createWalletViaCLI ctx
                            ...
                            "\n" (T.unpack passphrase)
                T.unpack err `shouldContain` cmdOk
                _ <- expectValidJSON (Proxy @ApiColeWallet) out
                c `shouldBe` ExitSuccess

    it "CLI_COLE_UPDATE_NAME_01 - Update names of wallets" $ \ctx ->
        forM_ [ emptyRandomWallet, emptyIcarusWallet ] $
            \emptyColeWallet -> runResourceT @IO $ do
            wid <- fmap (T.unpack . view walletId) (emptyColeWallet ctx)
            let updatedName = "Name is updated"
            (Exit c, Stdout out, Stderr err) <-
                updateWalletNameViaCLI ctx [wid, updatedName]
            c `shouldBe` ExitSuccess
            err `shouldBe` cmdOk
            ju <- expectValidJSON (Proxy @ApiColeWallet) out
            expectCliField
                (#name . #getApiT . #getWalletName)
                (`shouldBe` T.pack updatedName) ju

    it "CLI_COLE_UPDATE_NAME_02 - When updated name too long" $ \ctx ->
        forM_ [ emptyRandomWallet, emptyIcarusWallet ] $
            \emptyColeWallet -> runResourceT @IO $ do
            wid <- fmap (T.unpack . view walletId) (emptyColeWallet ctx)
    ...
    it "CLI_COLE_UTXO_01 - Wallet's inactivity is reflected in utxo" $ \ctx ->
        forM_ [ emptyRandomWallet, emptyIcarusWallet ]
        $ \emptyColeWallet -> runResourceT @IO $ do
            wid <- fmap (T.unpack . view walletId) (emptyColeWallet ctx)
    ...
    it "CLI_COLE_UPDATE_PASS_02 - Old passphrase incorrect" $ \ctx ->
        forM_ [ emptyRandomWallet, emptyIcarusWallet ] $
        \emptyColeWallet -> runResourceT @IO $ do
            wid <- fmap (T.unpack . view walletId) (emptyColeWallet ctx)
            Stdout out <- getWalletViaCLI ctx wid
    ...
    describe "CLI_COLE_UPDATE_PASS_03 - Pass length incorrect" $ do
        let minLength = passphraseMinLength (Proxy @"raw")
        let maxLength = passphraseMaxLength (Proxy @"raw")
        ...
        forM_ matrix $ \(title, oldPass, newPass, errMsg) -> it title $ \ctx -> runResourceT @IO $ do
            forM_ [ emptyRandomWallet, emptyIcarusWallet ] $ \emptyColeWallet -> do
                wid <- fmap (T.unpack . view walletId) (emptyColeWallet ctx)
                Stdout out <- getWalletViaCLI ctx wid
                expectValidJSON (Proxy @ApiColeWallet) out
                    >>= flip verify [ expectCliField #passphrase (`shouldSatisfy` isJust) ]
