...
module Test.Integration.Scenario.CLI.Cole.Addresses
    ( spec
    ) where
...
import Test.Integration.Framework.TestData
    ( cmdOk, errMsg403NotAColeWallet, errMsg403WrongPass, errMsg404NoWallet )
...
spec = do
    describe "COLE_CLI_ADDRESSES" $ do
...
scenario_ADDRESS_CREATE_02 = it title $ \ctx -> runResourceT @IO $ do
    ...
    let wid = T.unpack (w ^. walletId)
    (c, out, err) <- createAddressViaCLI ctx [wid] (T.unpack fixturePassphrase)
    T.unpack err `shouldContain` errMsg403NotAColeWallet
    c `shouldBe` ExitFailure 1
    out `shouldBe` mempty
  where
...
scenario_ADDRESS_IMPORT_02 = it title $ \ctx -> runResourceT @IO $ do
    ...
    (Exit c, Stdout _out, Stderr err) <- importAddressViaCLI ctx [wid, addr]
    c `shouldBe` ExitFailure 1
    err `shouldContain` errMsg403NotAColeWallet
  where
