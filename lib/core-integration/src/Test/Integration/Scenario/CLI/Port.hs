    ...
    describe "PORT_04 - Fail nicely when port is out-of-bounds" $ do
        ...
        forM_ tests $ \(cmd, opt, port) -> let args = [cmd, opt, show port] in
            it (unwords args) $ \_ -> do
                (exit, Stdout (_ :: String), Stderr err) <- bccWalletCLI args
