...
spec = describe "COMMON_CLI_MISC" $ do
    it "CLI_VERSION - bcc-wallet shows version" $ do
        (Exit c, Stdout out) <- bccWalletCLI @_ @IO ["version"]
        let v = L.dropWhileEnd (== '\n') out
        v `shouldContain` (showVersionAsDate version <> " (git revision: " )
        c `shouldBe` ExitSuccess

    describe "CLI_HELP - bcc-wallet shows help on bad arg or param" $ do
        ...
        forM_ badArgs $ \args -> it args $ \_ -> do
            (Exit c, Stdout o, Stderr e) <- bccWalletCLI @_ @IO $ words args
            c `shouldBe` ExitFailure 1
            o `shouldBe` ""
            e `shouldContain` "Usage:"

    describe "CLI_HELP - bcc-wallet shows help with" $  do
        let test option = it option $ do
                (Exit c, Stdout o, Stderr e) <- bccWalletCLI @_ @IO [option]
                e `shouldBe` ""
