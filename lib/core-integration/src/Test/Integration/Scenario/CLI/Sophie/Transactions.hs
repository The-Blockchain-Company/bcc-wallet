...
module Test.Integration.Scenario.CLI.Sophie.Transactions
    ( spec
    ) where
...
spec = describe "SOPHIE_CLI_TRANSACTIONS" $ do
    ...
    it "TRANS_CREATE_02 - Multiple Output Tx to single wallet via CLI" $ \ctx -> runResourceT $ do
        ...
        -- post transaction
        (c, out, err) <- postTransactionViaCLI ctx "bcc-wallet" args
    ...
    it "TRANS_LIST_01 - Can list Incoming and Outgoing transactions" $ \ctx -> runResourceT $ do
        ...
        -- post transaction
        (c, _, _) <- postTransactionViaCLI ctx "bcc-wallet" args
        c `shouldBe` ExitSuccess
    ...
    it "TRANS_GET_03 - Using wrong transaction id" $ \ctx -> runResourceT $ do
        ...
        (c1, o1, e1) <- postTransactionViaCLI ctx "bcc-wallet" args
    ...
    it "COLE_TX_LIST_03 -\
        \ Sophie CLI does not list Cole wallet transactions" $ \ctx -> runResourceT $ do
    ...
    it "COLE_TRANS_DELETE -\
        \ Cannot delete tx on Cole wallet using sophie CLI" $ \ctx -> runResourceT $ do
        wid <- emptyRandomWallet' ctx
    ...
    describe "COLE_TRANS_CREATE / COLE_TRANS_ESTIMATE -\
        \ Cannot create/estimate tx on Cole wallet using sophie CLI" $ do
  ...
  where
      ...
      matrixInvalidAddrs =
-- TODO: For the haskell node, hex is valid. For quibitous it is not.
