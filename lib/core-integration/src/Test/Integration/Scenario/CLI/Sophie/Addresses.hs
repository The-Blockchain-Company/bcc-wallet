...
module Test.Integration.Scenario.CLI.Sophie.Addresses
    ( spec
    ) where
...
spec = describe "SOPHIE_CLI_ADDRESSES" $ do
    ...
    it "ADDRESS_LIST_01 - Can list addresses - non-default poolGap" $ \ctx -> runResourceT $ do
        ...
        walId <- emptyWalletWith' ctx
                    ("This is Wallet, OK?", "bcc-wallet", addrPoolGap)
        (Exit c, Stdout out, Stderr err) <- listAddressesViaCLI ctx [walId]
        err `shouldBe` "Ok.\n"
        c `shouldBe` ExitSuccess
        json <- expectValidJSON (Proxy @[ApiAddress n]) out
    ...
    it "ADDRESS_LIST_03 - Generates new address pool gap" $ \ctx -> runResourceT $ do
        ...
        wDest <- emptyWalletWith ctx ("Wallet", "bcc-wallet", initPoolGap)
        ...
        forM_ [0..initPoolGap - 1] $ \addrNum -> do
            ...
            (cTx, _, _) <- postTransactionViaCLI ctx "bcc-wallet" args
    ...
    it "COLE_ADDRESS_LIST - Cole wallet on Sophie CLI" $ \ctx -> runResourceT $ do
