...
module Test.Integration.Scenario.CLI.Sophie.Wallets
    ( spec
    , walletNames
    , walletNamesInvalid
    ) where
...
spec = describe "SOPHIE_CLI_WALLETS" $ do
    it "COLE_GET_03 - Sophie CLI does not show Cole wallet" $ \ctx -> runResourceT $ do
    ...
    it "COLE_LIST_03 - Sophie CLI does not list Cole wallet" $ \ctx -> runResourceT $ do
        coleWid <- emptyRandomWallet' ctx
        sophieWid <- emptyWallet' ctx
        ...
        wids `shouldContain` [sophieWid]
        wids `shouldNotContain` [coleWid]

    it "COLE_DELETE_03 - Sophie CLI does not delete Cole wallet" $ \ctx -> runResourceT $ do
        wid <- emptyRandomWallet' ctx
    ...
    it "COLE_WALLETS_UTXO -\
        \ Cannot show Cole wal utxo with sophie CLI" $ \ctx -> runResourceT $ do
        wid <- emptyRandomWallet' ctx
    ...
    it "COLE_WALLETS_UPDATE_PASS -\
        \ Cannot update Cole wal with sophie CLI" $ \ctx -> runResourceT $ do
        wid <- emptyRandomWallet' ctx
        let port = T.pack $ show $ ctx ^. typed @(Port "wallet")
        let args = T.unpack <$>
                [ "wallet", "update", "passphrase"
                , "--port", port, T.pack wid ]
        (Exit c, Stdout out, Stderr err) <- bccWalletCLI args
        out `shouldBe` ""
        c `shouldBe` ExitFailure 1
        err `shouldContain` errMsg404NoWallet (T.pack wid)

    it "COLE_WALLETS_UPDATE -\
        \ Cannot update name Cole wal with sophie CLI" $ \ctx -> runResourceT $ do
        wid <- emptyRandomWallet' ctx
        let port = T.pack $ show $ ctx ^. typed @(Port "wallet")
        let args = T.unpack <$>
                [ "wallet", "update", "name"
    ...
    it "WALLETS_CREATE_02 - Restored wallet preserves funds" $ \ctx -> runResourceT $ do
        ...
        (cp, op, ep) <- postTransactionViaCLI ctx "bcc-wallet" args
