...
{-------------------------------------------------------------------------------
               CLI option handling and bcc-node configuration
-------------------------------------------------------------------------------}

execBenchWithNode
    :: (RestoreBenchArgs -> cfg)
    -- ^ Get backend-specific network configuration from args
    -> (Trace IO Text -> cfg -> BccNodeConn -> IO ())
    -- ^ Action to run
    -> IO ExitCode
    ...
    case argUseAlreadyRunningNodeSocketPath args of
        ...
        Nothing -> do
            res <- withNetworkConfiguration args $ \nodeConfig ->
                withBccNode (trMessageText tr) nodeConfig $
                    action tr (networkConfig args)
            case res of
                Left exited -> do
                    sayErr $ "FAIL: bcc-node exited with status " <> toText exited
                    pure $ ExitFailure 1
                Right _ -> pure ExitSuccess

withNetworkConfiguration :: RestoreBenchArgs -> (BccNodeConfig -> IO a) -> IO a
withNetworkConfiguration args action = do
    ...
    withNodeDir $ \dir -> action BccNodeConfig
        { nodeDir          = dir
...
data RestoreBenchArgs = RestoreBenchArgs
    ...
    , argUseAlreadyRunningNodeSocketPath :: Maybe BccNodeConn
    , argQuiet :: Bool
    } deriving (Show, Eq)
...
restoreBenchArgsParser envNetwork envConfigsDir envNodeDatabaseDir envNodeSocket = RestoreBenchArgs
    ...
    <*> strOption
        ( long "bcc-node-configs"
          <> short 'c'
          <> metavar "DIR"
          <> envDefault "BCC_NODE_CONFIGS" envConfigsDir
    ...
    <*> optional (strOption
        ( long "node-db"
          ...
          <> help "Directory to put bcc-node state. Defaults to $NODE_DB, \
              \falls back to temporary directory"))
    <*> optional (option (eitherReader bccNodeConn)
        ( long "running-node"
          <> metavar "SOCKET"
          <> envDefault "BCC_NODE_SOCKET_PATH" envNodeSocket
          <> help "Path to the socket of an already running bcc-node. \
              \Also set by $BCC_NODE_SOCKET_PATH. If not set, bcc-node \
...
getRestoreBenchArgsParser = restoreBenchArgsParser
    <$> lookupEnv' "NETWORK"
    <*> lookupEnv' "BCC_NODE_CONFIGS"
    <*> lookupEnv' "NODE_DB"
    <*> parseEnv bccNodeConn "BCC_NODE_SOCKET"
  where
