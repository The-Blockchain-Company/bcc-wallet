...
--
-- - Module's documentation in `shardagnostic-network/typed-protocols/src/Network/TypedProtocols.hs`
-- - Data Diffusion and Peer Networking in Sophie (see: https://raw.githubusercontent.com/wiki/The-Blockchain-Company/bcc-wallet/data_diffusion_and_peer_networking_in_sophie.pdf)
--     - In particular sections 4.1, 4.2, 4.6 and 4.8
module Bcc.Wallet.Sophie.Network
    ( -- * Top-Level Interface
      pattern Cursor
    , withNetworkLayer
...
import Bcc.Wallet.Cole.Compatibility
    ( coleCodecConfig, protocolParametersFromUpdateState )
...
import Shardagnostic.Network.Client.Wallet
    ( ChainSyncCmd (..)
    , ChainSyncLog (..)
    , LSQ (..)
    , LocalStateQueryCmd (..)
...
import qualified Bcc.Wallet.Primitive.SyncProgress as SyncProgress
...
-- | Network layer cursor for Sophie. Mostly useless since the protocol itself is
-- stateful and the node's keep track of the associated connection's cursor.
data instance Cursor = Cursor
    (Async ())
    (Point (BccBlock StandardCrypto))
    (TQueue IO (ChainSyncCmd (BccBlock StandardCrypto) IO))
...
withNetworkLayer
    ...
    -> SyncTolerance
    -> (NetworkLayer IO (BccBlock StandardCrypto) -> IO a)
...
withNetworkLayerBase tr net np conn versionData tol action = do
  ...
  where
    ...
    connectNodeTipClient
        :: HasCallStack
        => RetryHandlers
        -> IO ( STM IO (Tip (BccBlock StandardCrypto))
              , TMVar IO (W.ProtocolParameters, W.SlottingParameters)
              , TMVar IO (BccInterpreter StandardCrypto)
              , TMVar IO AnyBccEra
              )
    ...
    connectLocalTxSubmissionClient
        ...
        -> IO ( TQueue IO (LocalTxSubmissionCmd
                  (Bcc.TxInMode BccMode)
                  (Bcc.TxValidationErrorInMode BccMode)
                  IO)
              )
    connectLocalTxSubmissionClient handlers = do
        q <- atomically newTQueue
        link =<< async (connectBccApiClient tr handlers connectInfo q)
        pure q
    ...
    connectDelegationRewardsClient
        ...
        => RetryHandlers
        -> IO (TQueue IO
                (LocalStateQueryCmd (BccBlock StandardCrypto) IO))
    ...
    _postTx localTxSubmissionQ tx = do
        ...
        let cmd = CmdSubmitTx $ unsealSophieTx tx
    ...
    _stakeDistribution queue coin = do
        ...
        let qry :: LSQ (BccBlock StandardCrypto) IO (Maybe W.StakePoolsSummary)
        ...
        -- The result will be Nothing if query occurs during the cole era
      ...
      where
        stakeDistr
            :: LSQ (BccBlock StandardCrypto) IO
                (Maybe (Map W.PoolId Percentage))
        stakeDistr = sophieBased
            (fromPoolDistr <$> LSQry Sophie.GetStakeDistribution)

        getNOpt :: LSQ (BccBlock StandardCrypto) IO (Maybe Int)
        getNOpt = onAnyEra
            (pure Nothing)
            (Just . optimumNumberOfPools <$> LSQry Sophie.GetCurrentPParams)
            (Just . optimumNumberOfPools <$> LSQry Sophie.GetCurrentPParams)
            (Just . optimumNumberOfPools <$> LSQry Sophie.GetCurrentPParams)
            (Just . fromIntegral . Aurum._nOpt <$> LSQry Sophie.GetCurrentPParams)

        queryNonMyopicMemberRewards
            :: LSQ (BccBlock StandardCrypto) IO
                    (Maybe (Map W.PoolId W.Coin))
        queryNonMyopicMemberRewards = sophieBased $
            (getRewardMap . fromNonMyopicMemberRewards)
                <$> LSQry (Sophie.GetNonMyopicMemberRewards stake)
          where
            stake :: Set (Either SL.Coin a)
            stake = Set.singleton $ Left $ toSophieCoin coin
...
type NetworkClient m = NodeToClientVersion -> ShardagnosticApplication
    'InitiatorMode
...
mkWalletClient
    ...
    -> CodecConfig (BccBlock StandardCrypto)
    -> W.GenesisParameters
        -- ^ Static blockchain parameters
    -> TQueue m (ChainSyncCmd (BccBlock StandardCrypto) m)
  ...
  where
    ...
    showP p = case (getPoint p) of
        ...
        At blk -> mconcat
            ...
            , ", "
            , pretty $ fromBccHash $ Point.blockPointHash blk
            , ")"
            ]
...
mkDelegationRewardsClient
    ...
    -> CodecConfig (BccBlock StandardCrypto)
    -> TQueue m (LocalStateQueryCmd (BccBlock StandardCrypto) m)
...
codecVersion
    :: NodeToClientVersion
    -> BlockNodeToClientVersion (BccBlock StandardCrypto)
codecVersion version = verMap ! version
    where verMap = supportedNodeToClientVersions (Proxy @(BccBlock StandardCrypto))

codecConfig :: W.SlottingParameters -> CodecConfig (BccBlock c)
codecConfig sp = BccCodecConfig
    (coleCodecConfig sp)
    SophieCodecConfig
    SophieCodecConfig
    SophieCodecConfig
...
serialisedCodecs
    ...
    => NodeToClientVersion
    -> CodecConfig (BccBlock StandardCrypto)
    -> DefaultCodecs (BccBlock StandardCrypto) m
...
type BccInterpreter sc = Interpreter (BccEras sc)
...
mkTipSyncClient
    ...
    -> (BccInterpreter StandardCrypto -> m ())
        -- ^ Notifier callback for when time interpreter is updated.
    -> (AnyBccEra -> m ())
        -- ^ Notifier callback for when the era is updated
    -> m (NetworkClient m, STM m (Tip (BccBlock StandardCrypto)))
mkTipSyncClient tr np onPParamsUpdate onInterpreterUpdate onEraUpdate = do
    (localStateQueryQ :: TQueue m (LocalStateQueryCmd (BccBlock StandardCrypto) m))
        <- atomically newTQueue

    tipVar <- newTVarIO (Just $ AnyBccEra ColeEra, TipGenesis)
    ...
    let queryParams = do
            eraBounds <- W.EraInfo
                <$> LSQry (QueryAnytimeCole GetEraStart)
                <*> LSQry (QueryAnytimeSophie GetEraStart)
                <*> LSQry (QueryAnytimeEvie GetEraStart)
                <*> LSQry (QueryAnytimeJen GetEraStart)
                <*> LSQry (QueryAnytimeAurum GetEraStart)

            sp <- coleOrSophieBased
                (pure $ W.slottingParameters np)
                ((slottingParametersFromGenesis . getCompactGenesis)
                    <$> LSQry Sophie.GetGenesisConfig)

            pp <- onAnyEra
                (protocolParametersFromUpdateState eraBounds
                    <$> LSQry Cole.GetUpdateInterfaceState)
                (fromSophiePParams eraBounds
                    <$> LSQry Sophie.GetCurrentPParams)
                (fromSophiePParams eraBounds
                    <$> LSQry Sophie.GetCurrentPParams)
                (fromSophiePParams eraBounds
                    <$> LSQry Sophie.GetCurrentPParams)
                (fromAurumPParams eraBounds
                    <$> LSQry Sophie.GetCurrentPParams)
            return (pp, sp)
    ...
    let client v = nodeToClientProtocols (const $ return $ NodeToClientProtocols
            { localChainSyncProtocol =
                ...
                InitiatorProtocolOnly $ MuxPeerRaw
                    ...
                    $ chainSyncClientPeer
                    $ chainSyncFollowTip toBccEra (curry (atomically . writeTVar tipVar))
...
connectBccApiClient
    ...
    -> IO ()
connectBccApiClient tr handlers info localTxSubmissionQ =
...
newRewardBalanceFetcher
    ...
    -> STM IO (Tip (BccBlock StandardCrypto))
    -- ^ STM action for observing the current tip
    -> TQueue IO (LocalStateQueryCmd (BccBlock StandardCrypto) IO)
...
fetchRewardAccounts tr queryRewardQ accounts = do
        ...
        return res
  where
    coleValue :: Map W.RewardAccount W.Coin
    coleValue = Map.fromList . map (, minBound) $ Set.toList accounts

    sophieQry
        :: (Crypto.HashAlgorithm (SL.ADDRHASH (EraCrypto sophieEra)))
        => LSQ
            (Sophie.SophieBlock sophieEra)
            IO
            (Map W.RewardAccount W.Coin, [NetworkLayerLog])
    sophieQry =
       fmap fromBalanceResult
        . LSQry
        . Sophie.GetFilteredDelegationsAndRewardAccounts
    ...
    fromBalanceResult (deleg, rewardAccounts) =
        ( Map.mapKeys fromStakeCredential $
            Map.map fromSophieCoin rewardAccounts
...
data NetworkLayerLog where
    ...
    MsgTxSubmission
        :: (TraceSendRecv
            (LocalTxSubmission (Bcc.TxInMode BccMode) (Bcc.TxValidationErrorInMode BccMode)))
        -> NetworkLayerLog
    MsgLocalStateQuery
        :: QueryClientName
        -> (TraceSendRecv
            (LocalStateQuery (BccBlock StandardCrypto) (Point (BccBlock StandardCrypto)) (Query (BccBlock StandardCrypto))))
        -> NetworkLayerLog
    MsgHandshakeTracer ::
    ...
    MsgLocalStateQueryEraMismatch :: MismatchEraInfo (BccEras StandardCrypto) -> NetworkLayerLog
    ...
    MsgInterpreter :: BccInterpreter StandardCrypto -> NetworkLayerLog
    -- TODO: Combine ^^ and vv
...
coleOrSophieBased
    :: LSQ Cole.ColeBlock m a
    -> (forall sophieEra. LSQ (Sophie.SophieBlock (sophieEra StandardCrypto)) m a)
    -> LSQ (BccBlock StandardCrypto) m a
coleOrSophieBased onCole onSophieBased = onAnyEra
    onCole
    onSophieBased
    onSophieBased
    onSophieBased
...
-- acquired point.
--
-- Where possible, the more convenient @sophieBased@ or @coleOrSophieBased@
-- should be used. This more raw helper was added to simplify dealing with
-- @PParams@ in aurum.
onAnyEra
    :: LSQ Cole.ColeBlock m a
    -> LSQ (Sophie.SophieBlock StandardSophie) m a
    -> LSQ (Sophie.SophieBlock StandardEvie) m a
    -> LSQ (Sophie.SophieBlock StandardJen) m a
    -> LSQ (Sophie.SophieBlock StandardAurum) m a
    -> LSQ (BccBlock StandardCrypto) m a
onAnyEra onCole onSophie onEvie onJen onAurum = currentEra >>= \case
    AnyBccEra ColeEra -> mapQuery QueryIfCurrentCole onCole
    AnyBccEra SophieEra -> mapQuery QueryIfCurrentSophie onSophie
    AnyBccEra EvieEra -> mapQuery QueryIfCurrentEvie onEvie
    AnyBccEra JenEra -> mapQuery QueryIfCurrentJen onJen
    AnyBccEra AurumEra -> mapQuery QueryIfCurrentAurum onAurum
  where
    mapQuery
        :: (forall r. BlockQuery block1 r
            -> BlockQuery block2
                ((Either (MismatchEraInfo (BccEras StandardCrypto))) r))
        -> LSQ block1 m a
        -> LSQ block2 m a
    ...
    unwrap = either (error "impossible: coleOrSophieBased query resulted in an \
        \era mismatch") id


-- | Return Nothings in Cole, or @Just result@ in Sophie.
sophieBased
    :: (forall sophieEra. LSQ (Sophie.SophieBlock (sophieEra StandardCrypto)) m a)
    -> LSQ (BccBlock StandardCrypto) m (Maybe a)
sophieBased onSophieBased = coleOrSophieBased
    (pure Nothing) -- on cole
    (Just <$> onSophieBased)
...
currentEra :: LSQ (BccBlock StandardCrypto) m AnyBccEra
currentEra = eraIndexToAnyBccEra <$> LSQry (QueryHardFork GetCurrentEra)

-- | Provides a mapping from 'EraIndex' to 'AnyBccEra'.
--
-- This mapping replaces a conversion between enumerations.
--
-- The following is used as a reference for the index mapping:
-- https://github.com/The-Blockchain-Company/bcc-node/blob/3531289c9f79eab7ac5d3272ce6e6821504fec4c/bcc-api/src/Bcc/Api/Eras.hs#L188
--
eraIndexToAnyBccEra :: EraIndex xs -> AnyBccEra
eraIndexToAnyBccEra index =
    case eraIndexToInt index of
        0 -> AnyBccEra ColeEra
        1 -> AnyBccEra SophieEra
        2 -> AnyBccEra EvieEra
        3 -> AnyBccEra JenEra
        4 -> AnyBccEra AurumEra
        _ -> error "eraIndexToAnyBccEra: unknown era"
