...
module Bcc.Wallet.Sophie.Pools
    ( StakePoolLayer (..)
    , newStakePoolLayer
    , monitorStakePools
    , monitorMetadata
...
combineLsqData StakePoolsSummary{nOpt, rewards, stake} =
  ...
  where
    ...
    -- TODO: This case seems possible on sophie_testnet, but why, and how
    -- should we treat it?
    --
...
monitorStakePools followTr (NetworkParameters gp sp _pp) nl DBLayer{..} =
  ...
  where
    ...
    getHeader :: BccBlock StandardCrypto -> BlockHeader
    getHeader = toBccBlockHeader gp

    forward
        :: IORef EpochNo
        -> NonEmpty (BccBlock StandardCrypto)
        -> BlockHeader
      ...
      where
        forAllBlocks = \case
            BlockCole _ -> do
                pure ()
            BlockSophie blk -> do
                forEachSophieBlock (fromSophieBlock gp blk) (getProducer blk)
            BlockEvie blk ->
                forEachSophieBlock (fromEvieBlock gp blk) (getProducer blk)
            BlockJen blk ->
                forEachSophieBlock (fromJenBlock gp blk) (getProducer blk)
            BlockAurum blk ->
                forEachSophieBlock (fromAurumBlock gp blk) (getProducer blk)

        forLastBlock = \case
            BlockCole blk ->
                putHeader (toColeBlockHeader gp blk)
            BlockSophie blk ->
                putHeader (toSophieBlockHeader getGenesisBlockHash blk)
            BlockEvie blk ->
                putHeader (toSophieBlockHeader getGenesisBlockHash blk)
            BlockJen blk ->
                putHeader (toSophieBlockHeader getGenesisBlockHash blk)
        ...
        forEachSophieBlock (blk, certificates) poolId = do
