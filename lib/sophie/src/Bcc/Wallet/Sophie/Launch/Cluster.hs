...
-- License: Apache-2.0
--
-- Provides functions to launch bcc-nodes in a cluster for /testing/.

module Bcc.Wallet.Sophie.Launch.Cluster
...
import qualified Bcc.Chain.UTxO as Legacy
import qualified Bcc.Wallet.Cole.Compatibility as Cole
...
import qualified Bcc.Wallet.Sophie.Compatibility as Sophie
...
-- | Returns the sophie test data path, which is usually relative to the
-- package sources, but can be overridden by the @SOPHIE_TEST_DATA@ environment
-- variable.
getSophieTestDataPath :: IO FilePath
getSophieTestDataPath = fromMaybe source <$> lookupEnvNonEmpty var
  where
    source = $(getTestData) </> "bcc-node-sophie"
    var = "SOPHIE_TEST_DATA"

logFileConfigFromEnv
    :: Maybe String
    -- ^ Optional extra subdir for TESTS_LOGDIR. E.g. @Just "aurum"@ and
    -- @Just "jen"@ to keep them separate.
    -> IO LogFileConfig
...
-- Allow configuring @bcc-node@ log level with the
-- @BCC_NODE_TRACING_MIN_SEVERITY@ environment variable.
nodeMinSeverityFromEnv :: IO Severity
nodeMinSeverityFromEnv =
    minSeverityFromEnv Info "BCC_NODE_TRACING_MIN_SEVERITY"

-- Allow configuring integration tests and wallet log level with
-- @BCC_WALLET_TRACING_MIN_SEVERITY@ environment variable.
walletMinSeverityFromEnv :: IO Severity
walletMinSeverityFromEnv =
    minSeverityFromEnv Warning "BCC_WALLET_TRACING_MIN_SEVERITY"
...
walletListenFromEnv = envFromText "BCC_WALLET_PORT" >>= \case
...
instance FromJSON PreserveInitialFundsOrdering where
    parseJSON = Aeson.withObject "ColeGenesis" $ \obj -> do
        initialFunds <- obj .: "nonAvvmBalances"

        pure $ PreserveInitialFundsOrdering $ concat $
            map (map mkOut . Map.toList) initialFunds
      where
        mkOut = Cole.fromTxOut . uncurry Legacy.TxOut .
            bimap unsafeMkAddress unsafeMkEntropic
        unsafeMkAddress =
            fromRight bomb . Cole.decodeAddressBase58
          where
            bomb = error "PreserveInitialFundsOrdering: address not valid base58"

        unsafeMkEntropic =
            fromRight bomb . Cole.mkEntropic . read
          where
            bomb = error "PreserveInitialFundsOrdering: invalid entropic value"

--------------------------------------------------------------------------------
-- For Integration
--------------------------------------------------------------------------------

-- | Make a 'ProcessConfig' for running @bcc-cli@. The program must be on
-- the @PATH@, as normal. Sets @BCC_NODE_SOCKET_PATH@ for the subprocess, if
-- a 'BccNodeConn' is provided.
cliConfigBase
    :: Tracer IO ClusterLog -- ^ for logging the command
    -> Maybe BccNodeConn -- ^ optional bcc-node socket path
    -> [String] -- ^ command-line arguments
    -> IO (ProcessConfig () () ())
cliConfigBase tr conn args = do
    traceWith tr (MsgCLI args)
    env <- getEnvironment
    let mkEnv c = ("BCC_NODE_SOCKET_PATH", nodeSocketFile c):env
    let cliEnv = maybe setEnvInherit (setEnv . mkEnv) conn
    pure $ cliEnv $ proc "bcc-cli" args

cliConfigNode
    :: Tracer IO ClusterLog -- ^ for logging the command
    -> BccNodeConn -- ^ bcc-node socket path
    -> [String] -- ^ command-line arguments
    -> IO (ProcessConfig () () ())
...
-- | A quick helper to interact with the 'bcc-cli'. Assumes the bcc-cli
...
-- | Runs a @bcc-cli@ command and retries for up to 30 seconds if the
-- command failed.
--
-- Assumes @bcc-cli@ is available in @PATH@.
cliRetry
    :: Tracer IO ClusterLog
...
cliRetry tr msg processConfig = do
    ...
    case st of
        ...
        ExitFailure _ -> throwIO $ ProcessHasExited
            ("bcc-cli failed: " <> BL8.unpack err) st
  where
    cmd = do
...
clusterToApiEra = \case
    ColeNoHardFork -> ApiCole
    SophieHardFork -> ApiSophie
    EvieHardFork -> ApiEvie
    JenHardFork -> ApiJen
    AurumHardFork -> ApiAurum
...
clusterEraFromEnv =
  ...
  where
    ...
    getEra env = case map toLower env of
        "cole" -> pure ColeNoHardFork
        "sophie" -> pure SophieHardFork
        "evie" -> pure EvieHardFork
        "jen" -> pure JenHardFork
        "aurum" -> pure AurumHardFork
...
-- BFT node.
--
-- The cluster is configured to automatically hard fork to Sophie at epoch 1
-- and then to Evie at epoch 2. Callback actions can be provided to run
...
withCluster tr dir LocalClusterConfig{..} onSetup onClusterStart =
    bracketTracer' tr "withCluster" $ do
        ...
        withBFTNode tr dir bftCfg $ \bftSocket block0 params -> do
            ...
            let pledgeOf 0 = 2*oneMillionBcc
                pledgeOf _ = oneMillionBcc
...
data NodeParams = NodeParams
    ...
    , nodeLogConfig :: LogFileConfig
      -- ^ The node will always log to "bcc-node.log" relative to the
...
withBFTNode tr baseDir params action =
    bracketTracer' tr "withBFTNode" $ do
        ...
        [bftCert, bftPrv, vrfPrv, kesPrv, opCert] <- forM
            [ "bft-leader" <> ".cole.cert"
            , "bft-leader" <> ".cole.skey"
            , "bft-leader" <> ".vrf.skey"
            , "bft-leader" <> ".kes.skey"
            , "bft-leader" <> ".opcert"
            ]
        ...
        withBccNodeProcess tr name cfg $ \socket ->
...
-- | Launches a @bcc-node@ with the given configuration which will not forge
...
withRelayNode tr baseDir params act =
    bracketTracer' tr "withRelayNode" $ do
        ...
        withBccNodeProcess tr name cfg act
  where
    name = "node"
    dir = baseDir </> name
    NodeParams systemStart hardForks (port, peers) logCfg = params

-- | Populates the configuration directory of a stake pool @bcc-node@.
--
...
setupStakePoolData
    ...
    -> IO (BccNodeConfig, FilePath, FilePath)
...
withStakePool tr baseDir idx params pledgeAmt poolConfig action =
    bracketTracer' tr "withStakePool" $ do
        ...
        withStaticServer dir $ \url -> do
            ...
            withBccNodeProcess tr name cfg $ \socket -> do
...
withBccNodeProcess
    ...
    -> (BccNodeConn -> IO a)
    -> IO a
withBccNodeProcess tr name cfg = withBccNode tr' cfg >=> throwErrs
  where
...
genConfig dir systemStart clusterEra logCfg = do
    ...
    Yaml.decodeFileThrow (source </> "node.config")
        >>= withAddedKey "SophieGenesisFile" sophieGenesisFile
        >>= withAddedKey "ColeGenesisFile" coleGenesisFile
        >>= withAddedKey "AurumGenesisFile" aurumGenesisFile
        >>= enableAurumIfNeeded
    ...
    ----
    -- Cole Genesis
    Yaml.decodeFileThrow @_ @Aeson.Value (source </> "cole-genesis.yaml")
        >>= withAddedKey "startTime" startTime
        >>= withObject transformInitialFunds
        >>= Aeson.encodeFile coleGenesisFile

    ----
    -- Sophie Genesis
    Yaml.decodeFileThrow @_ @Aeson.Value (source </> "sophie-genesis.yaml")
        >>= withObject (pure . updateSystemStart systemStart')
        >>= Aeson.encodeFile sophieGenesisFile

    Yaml.decodeFileThrow @_ @Aeson.Value (source </> "aurum-genesis.yaml")
        >>= Aeson.encodeFile aurumGenesisFile
    ...
    (coleGenesisData, coleGenesisHash) <- unsafeRunExceptT
        $ withExceptT show
        $ readGenesisData coleGenesisFile
    let (coleParams, _) = Cole.fromGenesisData (coleGenesisData, coleGenesisHash)
    let gp = genesisParameters coleParams
    let block0 = Cole.genesisBlockFromTxOuts gp initialFunds

    ----
    -- Parameters
    sophieGenesis <- Yaml.decodeFileThrow
        @_ @(SophieGenesis StandardSophie) sophieGenesisFile
    let networkMagic = sgNetworkMagic sophieGenesis
    let sophieParams = fst $ Sophie.fromGenesisData sophieGenesis []
    let versionData = NodeToClientVersionData $ NetworkMagic networkMagic

    pure
        ( dir </> "node.config"
        , block0
        , coleParams { protocolParameters = protocolParameters sophieParams }
        , versionData
        )
  where
    sophieGenesisFile :: FilePath
    sophieGenesisFile = dir </> "sophie-genesis.json"

    aurumGenesisFile :: FilePath
    aurumGenesisFile = dir </> "aurum-genesis.json"

    coleGenesisFile :: FilePath
    coleGenesisFile = dir </> "cole-genesis.json"
    ...
    withHardForks era = withObject (pure . HM.union (HM.fromList hardForks))
      where
        hardForks =
            ...
            | hardFork <- [SophieHardFork .. era] ]


    enableAurumIfNeeded =
        if clusterEra == AurumHardFork
...
writePolicySigningKey dir keyHash cborHex = do
    ...
    Aeson.encodeFile keyFile $ object
        [ "type" .= Aeson.String "PaymentSigningKeySophie_ed25519"
        , "description" .= Aeson.String "Payment Signing Key"
        , "cborHex" .= cborHex
        ]
    pure keyFile

-- | Dig in to a @bcc-cli@ TextView key file to get the hex-encoded key.
...
sendFaucetFundsTo tr conn dir targets = batch 80 targets $
    sendFaucet tr conn dir "bcc" . map coinBundle
  where
    coinBundle = fmap (\c -> (TokenBundle.fromCoin c, []))

-- | Create transactions to fund the given faucet addresses with Bcc and assets.
--
-- Beside the 'TokenBundle' of Bcc and assets, there is a list of
...
moveInstantaneousRewardsTo tr conn dir targets = do
    ...
    testData <- getSophieTestDataPath
  ...
  where
    ...
    mkVerificationKey (xpub, reward) = do
        ...
        let json = Aeson.object
                [ "type" .= Aeson.String "StakeVerificationKeySophie_ed25519"
...
submitTx :: Tracer IO ClusterLog -> BccNodeConn -> String -> FilePath -> IO ()
...
waitForSocket :: Tracer IO ClusterLog -> BccNodeConn -> IO ()
...
waitUntilRegistered :: Tracer IO ClusterLog -> BccNodeConn -> String -> FilePath -> IO ()
...
takeFaucet = do
    ...
    let signingKey = basename <> ".sophie.key"
    pure (txin, signingKey)

-- | List of faucets also referenced in the sophie 'genesis.yaml'
...
operatorsFixture =
    [ ( PoolId $ unsafeFromHex
      ...
      , Aeson.object
          ...
          , "homepage" .= Aeson.String "https://tbco.io"
          ]
      )
    , ( PoolId $ unsafeFromHex
...
faucetAmt :: Integer
faucetAmt = 1000 * oneMillionBcc

-- | Just one million Bcc, in Entropic.
oneMillionBcc :: Integer
oneMillionBcc = 1_000_000_000_000
...
instance ToText ClusterLog where
    toText = \case
        ...
        MsgCLI args -> T.pack $ unwords ("bcc-cli":args)
      where
