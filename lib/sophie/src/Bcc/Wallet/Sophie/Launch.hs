...
-- License: Apache-2.0
--
-- Command-line option passing for bcc-wallet sophie.
--

module Bcc.Wallet.Sophie.Launch
    ( -- * Network
...
import Bcc.Launcher.Node
    ( BccNodeConn, bccNodeConn, isWindows )
...
-- | Sophie hard fork network configuration has two genesis datas.
-- As a special case for mainnet, we hardcode the cole genesis data.
data NetworkConfiguration where
    ...
    TestnetConfig
        :: FilePath
        -- ^ Genesis data in JSON format, for cole era.
        -> NetworkConfiguration
...
-- | --node-socket=FILE
nodeSocketOption :: Parser BccNodeConn
nodeSocketOption = option (eitherReader (addHelp . bccNodeConn)) $ mempty
    <> long "node-socket"
  ...
  where
    helpText = mconcat
        ...
        , "Note:", pipeHelp ]
    pipeHelp = " Windows named pipes are of the form \\\\.\\pipe\\bcc-node"
    addHelp = first (if isWindows then (++ pipeHelp) else id)

-- | --mainnet --sophie-genesis=FILE
-- --testnet --cole-genesis=FILE --sophie-genesis=FILE
-- --staging --cole-genesis=FILE --sophie-genesis=FILE
networkConfigurationOption :: Parser NetworkConfiguration
networkConfigurationOption = mainnet <|> testnet <|> staging
  where
    mainnet = mainnetFlag
    testnet = TestnetConfig <$> genesisFileOption "cole" "testnet"
    staging = StagingConfig <$> genesisFileOption "cole" "staging"

    mainnetFlag = flag' MainnetConfig $ mempty
        <> long "mainnet"
        <> help "Use Bcc mainnet protocol"
...
parseGenesisData = \case
    MainnetConfig -> do
        ...
        pure
            ...
            , Cole.mainnetNetworkParameters
            , mainnetVersionData
            , Cole.emptyGenesis (genesisParameters Cole.mainnetNetworkParameters)
            )

    TestnetConfig coleGenesisFile -> do
        (genesisData, genesisHash) <-
            withExceptT show $ readGenesisData coleGenesisFile
        ...
        let pm = Cole.fromProtocolMagicId $ gdProtocolMagicId genesisData
        let (discriminant, vData) = someCustomDiscriminant mkSomeNetwork pm
        let (np, outs) = Cole.fromGenesisData (genesisData, genesisHash)
        let block0 = Cole.genesisBlockFromTxOuts (genesisParameters np) outs
    ...
    StagingConfig coleGenesisFile -> do
        (genesisData, genesisHash) <-
            withExceptT show $ readGenesisData coleGenesisFile
