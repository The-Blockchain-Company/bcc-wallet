...
module Bcc.Wallet.Sophie.CompatibilitySpec
    ( spec
    ) where
...
import qualified Bcc.Ledger.Sophie as SLAPI
...
spec = do
    describe "Conversions" $ do
        it "toPoint' . fromTip' == getTipPoint" $ property $ \gh tip -> do
            ...
            let toPoint' = toPoint gh :: W.BlockHeader -> Point (BccBlock StandardCrypto)
            toPoint' (fromTip' tip) === (getTipPoint tip)

        it "unsafeIntToWord" $
            property prop_unsafeIntToWord

    describe "Sophie StakeAddress" $ do
    ...
    describe "Sophie Addresses" $ do
        prop "(Mainnet) can be deserialised by sophie ledger spec" $ \k -> do
            let Address addr = paymentAddress @'Mainnet @SophieKey k
            case SL.deserialiseAddr @StandardCrypto addr of
                Just _ -> property True
                Nothing -> property False

        prop "Sophie addresses from base16, bech32 and base58" $ \k -> do
            let addr@(Address bytes) = paymentAddress @'Mainnet @SophieKey k
            conjoin
        ...
        prop "Sophie addresses from bech32 - testnet" $ \k ->
            let addr@(Address raw) = paymentAddress @('Testnet 0) @SophieKey k
            in decodeAddress @('Testnet 0) (bech32testnet raw) === Right addr
                   & counterexample (show $ bech32testnet raw)

        prop "Cole addresses from base16, bech32 and base58" $ \k -> do
            let addr@(Address bytes) = paymentAddress @'Mainnet @ColeKey k
            conjoin
                [ decodeAddress @'Mainnet (base16 bytes) === Right addr
                    & counterexample (show $ base16 bytes)
                , decodeAddress @'Mainnet (bech32 bytes) === Right addr
    ...
    describe "decentralizationLevelFromPParams" $ do
        ...
        let mkDecentralizationParam :: SL.UnitInterval -> SLAPI.PParams (SL.SophieEra StandardCrypto)
    ...
    describe "Bcc.Api.Value-TokenBundle conversion" $ do
        it "roundtrips" $ checkCoverage $ property $ \tb ->
            cover 20 (TokenBundle.getCoin tb /= Coin 0) "has bcc" $
            cover 2 (TokenBundle.getCoin tb == Coin 0) "has no bcc" $
            cover 10 (length (snd $ TokenBundle.toFlatList tb) > 3)
                "has some assets" $
            fromBccValue (toBccValue tb) === tb
    ...
    describe "InspectAddr" $ do
        -- Cases below are taken straight from bcc-addresses. We don't go in
    ...
    describe "golden tests for script hashes for different versions" $ do
        testScriptsAllLangs Bcc.SimpleScriptV1
        testScriptsAllLangs Bcc.SimpleScriptV2
        testScriptsTimelockLang

    describe "golden tests for script preimages for different versions" $ do
        testScriptPreimages Bcc.SimpleScriptV1
        testScriptPreimages Bcc.SimpleScriptV2
        testTimelockScriptImagesLang


    -- ADP-656 idea is here to decode every zerepoch example and try to Server.balanceTransaction
    ...
    describe "decode zerepoch jsons and coin select for different wallets" $ do
...
prop_assessTokenBundleSize_enlarge b1' b2' =
  ...
  where
    assess = assessTokenBundleSize
        $ tokenBundleSizeAssessor jenTokenBundleMaxSize
...
toPaymentHash :: Text -> Bcc.SimpleScript lang
toPaymentHash txt =
    case Bcc.deserialiseFromRawBytesHex (Bcc.AsHash Bcc.AsPaymentKey) (T.encodeUtf8 txt) of
        Just payKeyHash -> Bcc.RequireSignature payKeyHash
        Nothing -> error "Hash key not valid"
...
checkScriptHashes title adrestiaScript nodeScript = it title $
    ...
    Bcc.serialiseToRawBytes (Bcc.hashScript nodeScript)

checkScriptPreimage
    :: Bcc.SerialiseAsCBOR (Bcc.Script lang)
    => String
    -> Script KeyHash
    -> Bcc.Script lang
    -> SpecWith ()
checkScriptPreimage title adrestiaScript nodeScript = it title $
    ...
    BS.append "\00" (Bcc.serialiseToCBOR nodeScript)

scriptMatrix
    :: Bcc.SimpleScriptVersion lang
    -> [(String, Script KeyHash, Bcc.Script lang)]
scriptMatrix version =
    ...
    , ( show version <> " RequireAllOf"
      ...
      , toSimpleScript $
          Bcc.RequireAllOf [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2]
      )
    , ( show version <> " RequireAllOf"
      , RequireAllOf [toKeyHash hashKeyTxt1, toKeyHash hashKeyTxt2, toKeyHash hashKeyTxt3]
      , toSimpleScript $
          Bcc.RequireAllOf [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2, toPaymentHash hashKeyTxt3]
      )
    , ( show version <> " RequireAnyOf"
      , RequireAnyOf [toKeyHash hashKeyTxt1, toKeyHash hashKeyTxt2]
      , toSimpleScript $
          Bcc.RequireAnyOf [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2]
      )
    , ( show version <> " RequireAnyOf"
      , RequireAnyOf [toKeyHash hashKeyTxt1, toKeyHash hashKeyTxt2, toKeyHash hashKeyTxt3]
      , toSimpleScript $
          Bcc.RequireAnyOf [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2, toPaymentHash hashKeyTxt3]
      )
    , ( show version <> " RequireSomeOf"
      , RequireSomeOf 2 [toKeyHash hashKeyTxt1, toKeyHash hashKeyTxt2, toKeyHash hashKeyTxt3]
      , toSimpleScript $
          Bcc.RequireMOf 2 [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2, toPaymentHash hashKeyTxt3]
      )
    , ( show version <> " RequireSomeOf"
      , RequireSomeOf 2 [toKeyHash hashKeyTxt1, toKeyHash hashKeyTxt2, toKeyHash hashKeyTxt3, toKeyHash hashKeyTxt4]
      , toSimpleScript $
          Bcc.RequireMOf 2 [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2, toPaymentHash hashKeyTxt3, toPaymentHash hashKeyTxt4]
      )
    , ( show version <> " nested 1"
      ...
      , toSimpleScript $
          Bcc.RequireMOf 2 [ toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2
                               , Bcc.RequireAllOf [toPaymentHash hashKeyTxt3, toPaymentHash hashKeyTxt4]
                                       ]
      )
    , ( show version <> " nested 2"
      ...
      , toSimpleScript $
          Bcc.RequireAllOf [ toPaymentHash hashKeyTxt1
                               , Bcc.RequireAnyOf [toPaymentHash hashKeyTxt2, toPaymentHash hashKeyTxt3, toPaymentHash hashKeyTxt4]
                               ]
      )
    , ( show version <> " nested 3"
      ...
      , toSimpleScript $
          Bcc.RequireMOf 1 [ toPaymentHash hashKeyTxt1
                               , Bcc.RequireAllOf [ toPaymentHash hashKeyTxt2
                                                      , Bcc.RequireAnyOf [toPaymentHash hashKeyTxt3, toPaymentHash hashKeyTxt4]
                                                      ]
                               ]
      )
    ]
  where
    toSimpleScript = Bcc.SimpleScript version
...
testScriptsAllLangs
    :: forall lang . Bcc.SimpleScriptVersion lang
    -> Spec
...
testScriptPreimages
    :: forall lang . Bcc.IsScriptLanguage lang
    => Bcc.SimpleScriptVersion lang
    -> Spec
...
timelockScriptMatrix
    :: [(String, Script KeyHash, Bcc.Script Bcc.SimpleScriptV2)]
timelockScriptMatrix =
    [ ( "SimpleScriptV2 ActiveFromSlot"
      , RequireAllOf [toKeyHash hashKeyTxt1, ActiveFromSlot 120]
      , toSimpleScript $
          Bcc.RequireAllOf [toPaymentHash hashKeyTxt1, Bcc.RequireTimeAfter Bcc.TimeLocksInSimpleScriptV2 (SlotNo 120)]
      )
    , ( "SimpleScriptV2 ActiveUntilSlot"
      , RequireAllOf [toKeyHash hashKeyTxt1, ActiveUntilSlot 120]
      , toSimpleScript $
          Bcc.RequireAllOf [toPaymentHash hashKeyTxt1, Bcc.RequireTimeBefore Bcc.TimeLocksInSimpleScriptV2 (SlotNo 120)]
      )
    , ( "SimpleScriptV2 ActiveFromSlot and ActiveUntilSlot"
      ...
      , toSimpleScript $
          Bcc.RequireAllOf
          [ Bcc.RequireTimeAfter Bcc.TimeLocksInSimpleScriptV2 (SlotNo 120)
          , Bcc.RequireTimeBefore Bcc.TimeLocksInSimpleScriptV2 (SlotNo 150)
          , Bcc.RequireAnyOf [toPaymentHash hashKeyTxt1, toPaymentHash hashKeyTxt2 ]
          ]
      )
    ]
  where
    hashKeyTxt1 = "deeae4e895d8d57378125ed4fd540f9bf245d59f7936a504379cfc1e"
    hashKeyTxt2 = "60a3bf69aa748f9934b64357d9f1ca202f1a768aaf57263aedca8d5f"
    toSimpleScript = Bcc.SimpleScript Bcc.SimpleScriptV2
...
instance Arbitrary (Tip (BccBlock StandardCrypto)) where
    arbitrary = frequency
...
instance Arbitrary (SophieKey 'AddressK XPrv) where
    shrink _ = []
    arbitrary = do
        mnemonic <- arbitrary
        return $ Sophie.unsafeGenerateKeyFromSeed mnemonic mempty

instance Arbitrary (ColeKey 'AddressK XPrv) where
    shrink _ = []
    arbitrary = do
        mnemonic <- arbitrary
        acctIx <- toEnum <$> arbitrary
        addrIx <- toEnum <$> arbitrary
        return $ Cole.unsafeGenerateKeyFromSeed (acctIx, addrIx) mnemonic mempty
