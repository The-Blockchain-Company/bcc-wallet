...
parserInfo = info (helper <*> argsParser) $ mconcat
    ...
    , header $ "mock-token-metadata-server"
        <> " - for testing bcc-wallet native asset metadata"
    ]
  where
    docs = mconcat
        [ p [ "This serves only the `POST /metadata/query' endpoint of Bcc "
            , "metadata-server. Other endpoints are unimplemented because they "
            , "are not used by bcc-wallet."
            ]
        , p [ "The OpenAPI specification of the real metadata-server is here:" ]
        , code $ "https://github.com/The-Blockchain-Company/metadata-server"
        ...
        , code "https://github.com/The-Blockchain-Company/metadata-registry-testnet"
        ...
        , code "bcc-wallet serve --token-metadata-server URL ..."
