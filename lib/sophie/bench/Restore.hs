...
--     $ nix-build -A benchmarks.bcc-wallet.restore -o restore && ./restore/bin/restore testnet
-- @
--
-- or
-- @
--     $ ./.buildkite/bench-restore.sh sophie testnet
-- @
--
...
-- @
--      stack bench bcc-wallet:bench:restore
--          --ba 'mainnet -c $CONFIGURATION_DIR
--          --running-node $BCC_NODE_SOCKET_PATH
-- @
--
...
import Bcc.Wallet.Sophie.Launch
    ( BccNodeConn, NetworkConfiguration (..), parseGenesisData )
...
main :: IO ()
main = execBenchWithNode argsNetworkConfig bccRestoreBench >>= exitWith
...
argsNetworkConfig args = case argNetworkName args of
    "mainnet" ->
        MainnetConfig
    _ ->
        TestnetConfig (argsNetworkDir args </> "genesis-cole.json")

-- | Run all available benchmarks.
bccRestoreBench :: Trace IO Text -> NetworkConfiguration -> BccNodeConn -> IO ()
bccRestoreBench tr c socketFile = do
  ...
  where
    walletRnd
        :: Text
        -> (ColeKey 'RootK XPrv -> Int -> s)
        -> (WalletId, WalletName, s)
    walletRnd wname mkState =
        let
            seed = dummySeedFromName wname
            xprv = Cole.generateKeyFromSeed seed mempty
    ...
    walletSeq
        :: Text
        -> ((SophieKey 'RootK XPrv, Passphrase "encryption") -> AddressPoolGap -> s)
        -> (WalletId, WalletName, s)
    walletSeq wname mkState =
        let
            seed = dummySeedFromName wname
            xprv = Sophie.generateKeyFromSeed (seed, Nothing) mempty
            wid = WalletId $ digest $ publicKey xprv
    ...
    mkSeqAnyState'
        ...
        => Proxy p
        -> Proxy n
        -> (SophieKey 'RootK XPrv, Passphrase "encryption")
        -> AddressPoolGap
        -> SeqAnyState n SophieKey p
