...
import Bcc.Wallet.LatencyBenchShared
...
import Test.Integration.Faucet
    ( jenIntegrationTestAssets, sophieIntegrationTestFunds )
...
main = withUtf8Encoding $
    withLatencyLogging setupTracers $ \tracers capture ->
        withSophieServer tracers $ \ctx -> do
...
walletApiBench capture ctx = do
  ...
  where
    ...
    -- Creates n fixture wallets and send 1-bcc transactions to one of them
    ...
    nFixtureWalletWithUTxOs n utxoNumber = do
        ...
        eventually "Wallet balance is as expected" $ do
            rWal1 <- request @ApiWallet ctx
                (Link.getWallet @'Sophie wal1) Default Empty
            verify rWal1
                [ expectSuccess
                , expectField
        ...
        rStat <- request @ApiUtxoStatistics ctx
                (Link.getUTxOsStatistics @'Sophie wal1) Default Empty
    ...
    repeatPostTx wDest amtToSend batchSize amtExp = do
        ...
        replicateM_ batchSize
            (postTx (wSrc, Link.createTransactionOld @'Sophie, fixturePassphrase) wDest amtToSend)
        eventually "repeatPostTx: wallet balance is as expected" $ do
            rWal1 <- request @ApiWallet  ctx (Link.getWallet @'Sophie wDest) Default Empty
            verify rWal1
                [ expectSuccess
                , expectField
                    (#balance . #available . #getQuantity)
    ...
    runScenario scenario = runResourceT $ scenario >>= \(wal1, wal2, walMA, maWalletToMigrate) -> liftIO $ do
        t1 <- measureApiLogs capture
            (request @[ApiWallet] ctx (Link.listWallets @'Sophie) Default Empty)
        fmtResult "listWallets        " t1

        t2 <- measureApiLogs capture
            (request @ApiWallet  ctx (Link.getWallet @'Sophie wal1) Default Empty)
        fmtResult "getWallet          " t2

        t3 <- measureApiLogs capture
            (request @ApiUtxoStatistics ctx (Link.getUTxOsStatistics @'Sophie wal1) Default Empty)
        fmtResult "getUTxOsStatistics " t3

        t4 <- measureApiLogs capture
            (request @[ApiAddress n] ctx (Link.listAddresses @'Sophie wal1) Default Empty)
        fmtResult "listAddresses      " t4

        t5 <- measureApiLogs capture
            (request @[ApiTransaction n] ctx (Link.listTransactions @'Sophie wal1) Default Empty)
        fmtResult "listTransactions   " t5

        (_, txs) <- unsafeRequest @[ApiTransaction n] ctx (Link.listTransactions @'Sophie wal1) Empty
        let txid = (head txs) ^. #id
        t5a <- measureApiLogs capture
            (request @[ApiTransaction n] ctx (Link.getTransaction @'Sophie wal1 (ApiTxId txid)) Default Empty)
        fmtResult "getTransaction     " t5a

        (_, addrs) <- unsafeRequest @[ApiAddress n] ctx (Link.listAddresses @'Sophie wal2) Empty
        let amt = minUTxOValue era
        let destination = (addrs !! 1) ^. #id
        let payload = Json [json|{
                "payments": [{
        ...
        t6 <- measureApiLogs capture $ request @ApiFee ctx
            (Link.getTransactionFeeOld @'Sophie wal1) Default payload
        ...
        t7 <- measureApiLogs capture $ request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wal1) Default payloadTx
        ...
        t7a <- measureApiLogs capture $ request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie wal2) Default payloadTxTo5Addr
        ...
        t7b <- measureApiLogs capture $ request @(ApiTransaction n) ctx
            (Link.createTransactionOld @'Sophie walMA) Default payloadMA
        ...
        let endpointPlan = (Link.createMigrationPlan @'Sophie maWalletToMigrate)
        ...
        -- Perform a migration:
        let endpointMigrate = Link.migrateWallet @'Sophie maWalletToMigrate
...
withSophieServer
    ...
    -> (Context -> IO ())
    -> IO ()
withSophieServer tracers action = do
    ctx <- newEmptyMVar
  ...
  where
    ...
    setupFaucet dir (RunningNode conn _ _) = do
        ...
        let addresses = map (first encodeAddr) sophieIntegrationTestFunds
        let addressesMA = map (first encodeAddr) (jenIntegrationTestAssets (Coin 10_000_000))
