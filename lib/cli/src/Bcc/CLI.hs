...
import qualified Bcc.BM.Data.BackendKind as CM
import qualified Bcc.BM.Data.Observable as Obs
...
cli cmds = info (helper <*> subparser cmds) $ mempty
    <> progDesc "Bcc Wallet Command-Line Interface (CLI)"
    <> header (mconcat
...
cmdColeWalletCreate
    :: WalletClient ApiColeWallet
    -> Mod CommandFields (IO ())
cmdColeWalletCreate mkClient =
    command "create" $ info (helper <*> cmds) $ mempty
        <> progDesc "Create a new Cole wallet."
  where
    cmds = subparser $ mempty
        <> cmdColeWalletCreateFromMnemonic mkClient

data ColeWalletCreateFromMnemonicArgs = ColeWalletCreateFromMnemonicArgs
    { _port :: Port "Wallet"
    , _name :: WalletName
    , _style :: ColeWalletStyle
    }

cmdColeWalletCreateFromMnemonic
    :: WalletClient ApiColeWallet
    -> Mod CommandFields (IO ())
cmdColeWalletCreateFromMnemonic mkClient =
  ...
  where
    cmd = fmap exec $ ColeWalletCreateFromMnemonicArgs
        <$> portOption
        <*> walletNameArgument
        <*> walletStyleOption Icarus [Random,Icarus,Trezor,Ledger]
    exec (ColeWalletCreateFromMnemonicArgs wPort wName wStyle) = case wStyle of
        Random -> do
            wSeed <- do
            ...
            runClient wPort Aeson.encodePretty $ postWallet mkClient $
                RandomWalletFromMnemonic $ ColeWalletPostData
                    (ApiMnemonicT wSeed)
                    (ApiT wName)
                    (ApiT wPwd)
        ...
        Icarus -> do
            ...
            wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
            runClient wPort Aeson.encodePretty $ postWallet mkClient $
                SomeIcarusWallet $ ColeWalletPostData
                    (ApiMnemonicT wSeed)
                    (ApiT wName)
                    (ApiT wPwd)
        ...
        Trezor -> do
            ...
            wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
            runClient wPort Aeson.encodePretty $ postWallet mkClient $
                SomeTrezorWallet $ ColeWalletPostData
                    (ApiMnemonicT wSeed)
                    (ApiT wName)
                    (ApiT wPwd)
        ...
        Ledger -> do
            ...
            wPwd <- getPassphraseWithConfirm "Please enter a passphrase: "
            runClient wPort Aeson.encodePretty $ postWallet mkClient $
                SomeLedgerWallet $ ColeWalletPostData
                    (ApiMnemonicT wSeed)
                    (ApiT wName)
                    (ApiT wPwd)
...
data TransactionFeatures = NoSophieFeatures | SophieFeatures
    deriving (Show, Eq)
...
cmdTransaction
    ...
    -> WalletClient wallet
    -> Mod CommandFields (IO ())
cmdTransaction = cmdTransactionBase SophieFeatures
...
cmdTransactionBase
    ...
    -> WalletClient wallet
    -> Mod CommandFields (IO ())
cmdTransactionBase isSophie mkTxClient mkWalletClient =
    command "transaction" $ info (helper <*> cmds) $ mempty
        <> progDesc "About transactions"
  where
    cmds = subparser $ mempty
        <> cmdTransactionCreate isSophie mkTxClient mkWalletClient
        <> cmdTransactionFees isSophie mkTxClient mkWalletClient
...
whenSophie :: a -> Parser a -> TransactionFeatures -> Parser a
whenSophie j s = \case
    NoSophieFeatures -> pure j
    SophieFeatures -> s
...
cmdTransactionCreate isSophie mkTxClient mkWalletClient =
  ...
  where
    cmd = fmap exec $ TransactionCreateArgs
        ...
        <*> whenSophie (ApiTxMetadata Nothing) metadataOption isSophie
        <*> whenSophie Nothing timeToLiveOption isSophie
...
-- | --state-dir=DIR, default: ~/.bcc-wallet/$backend/$network
...
--
-- Note that we in the future might replace the type @ColeWalletStyle@ with
-- another type, to include Quibitous keys.
walletStyleOption
    :: ColeWalletStyle
        -- ^ Default style
    -> [ColeWalletStyle]
        -- ^ Accepted styles
    -> Parser ColeWalletStyle
...
metadataOption = option txMetadataReader $ mempty
    ...
    <> help ("Application-specific transaction metadata as a JSON object. "
             ...
             <> "bcc-wallet OpenAPI specification.")
...
getPrometheusURL = do
    prometheus_port <- lookupEnv "BCC_WALLET_PROMETHEUS_PORT"
    prometheus_host <- fromMaybe "localhost" <$> lookupEnv "BCC_WALLET_PROMETHEUS_HOST"
...
getEKGURL = do
    ekg_port <- lookupEnv "BCC_WALLET_EKG_PORT"
    ekg_host <- fromMaybe "localhost" <$> lookupEnv "BCC_WALLET_EKG_HOST"
...
initTracer loggerName outputs = do
    ...
    cfg <- do
        ...
        CM.setBackends c "bcc-wallet.metrics" (Just [CM.EKGViewBK])
        forM_ ekgHP $ \(h, p) -> do
    ...
    pure (sb, (cfg, tr))
  where
    -- https://github.com/The-Blockchain-Company/bcc-node/blob/f7d57e30c47028ba2aeb306a4f21b47bb41dec01/bcc-node/src/Bcc/Node/Configuration/Logging.hs#L224
...
getDataDir backendDir = do
    ...
    dataDir <- getXdgDirectory dir "bcc-wallet"
