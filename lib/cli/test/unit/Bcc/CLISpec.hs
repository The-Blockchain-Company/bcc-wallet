...
spec = do
    describe "Specification / Usage Overview" $ do
        ...
        let goldenDir = $(getTestData) </> "Bcc" </> "CLISpec"
    ...
    describe "Pool metadata fetching option" $ do
        ...
        mapM_
            ...
            , ( "https", "https://tbcodev.io", ok )
            , ( "not http(s)", "gopher://tbco.io", err )
...
test fn (GetLineTest prompt_ input_ output expected) =
    withSystemTempDirectory "bcc-wallet-cli" $ \dir -> do
        -- Setup
    ...
    case execParserPure defaultPrefs parser args of
        ...
        Failure failure -> do
            ...
            let settings = Settings
                    { goldenDirectory = dir
                    , postProcess = T.replace exe "bcc-wallet" . (<> "\n")
                    }
