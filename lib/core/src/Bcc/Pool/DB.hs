...
data DBLayer m = forall stm. (MonadFail stm, MonadIO stm) => DBLayer
    ...
    , putStakeDistribution
        :: EpochNo
        -> [(PoolId, Quantity "entropic" Word64)]
        -> stm ()
    ...
    , readStakeDistribution
        :: EpochNo
        -> stm [(PoolId, Quantity "entropic" Word64)]
...
-- inference rule", as described in "A Formal Specification of the Bcc
-- Ledger":
--
-- https://hydra.tbco.io/build/3202141/download/1/ledger-spec.pdf
--
determinePoolLifeCycleStatus
