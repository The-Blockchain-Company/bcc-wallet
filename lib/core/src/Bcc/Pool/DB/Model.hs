...
data PoolDatabase = PoolDatabase
    ...
    , distributions :: !(Map EpochNo [(PoolId, Quantity "entropic" Word64)])
...
mPutStakeDistribution
    :: EpochNo -> [(PoolId, Quantity "entropic" Word64)] -> ModelOp ()
mPutStakeDistribution epoch distribution =
    modify #distributions $ Map.insert epoch distribution

mReadStakeDistribution
    :: EpochNo -> ModelOp [(PoolId, Quantity "entropic" Word64)]
