...
-- "Bcc.Wallet.DB" and "Bcc.Wallet.Network" to realize its role as
...
--   values for this parameter are described in 'Bcc.Wallet.AddressDiscovery' sub-modules.
...
import Bcc.Wallet.Primitive.AddressDerivation.Sophie
    ( SophieKey, deriveAccountPrivateKeySophie )
...
import qualified Bcc.Wallet.Primitive.AddressDiscovery.Random as Rnd
import qualified Bcc.Wallet.Primitive.AddressDiscovery.Sequential as Seq
import qualified Bcc.Wallet.Primitive.AddressDiscovery.Shared as Shared
import qualified Bcc.Wallet.Primitive.CoinSelection.Balance as Balance
import qualified Bcc.Wallet.Primitive.Migration as Migration
...
-- lenses (see Bcc.Wallet#Capabilities). These components are extracted from the context
...
getWalletUtxoSnapshot ctx wid = do
    ...
    let bundles = availableUTxO @s pending wallet
            ...
            & F.toList
            & fmap (view #tokens)
    pure $ pairBundleWithMinBccQuantity pp <$> bundles
  where
    nl = ctx ^. networkLayer
    tl = ctx ^. transactionLayer @k

    pairBundleWithMinBccQuantity
        :: ProtocolParameters -> TokenBundle -> (TokenBundle, Coin)
    pairBundleWithMinBccQuantity pp bundle =
        (bundle, computeMinBccQuantity $ view #tokens bundle)
      where
        computeMinBccQuantity :: TokenMap -> Coin
        computeMinBccQuantity =
            view #txOutputMinimumBccQuantity (view #constraints tl pp)
...
readRewardAccount ctx wid = db & \DBLayer{..} -> do
    cp <- withExceptT ErrReadRewardAccountNoSuchWallet
        ...
        $ readCheckpoint wid
    case testEquality (typeRep @s) (typeRep @sophie) of
        Nothing ->
            throwE ErrReadRewardAccountNotASophieWallet
        Just Refl -> do
            let s = getState cp
...
calcMinimumCoinValues ctx outs = do
    pp <- currentProtocolParameters nl
    pure
        $ view #txOutputMinimumBccQuantity (constraints tl pp)
...
getAccountPublicKeyAtIndex ctx wid pwd ix purposeM = db & \DBLayer{..} -> do
    ...
    withRootKey @ctx @s @k ctx wid pwd ErrReadAccountPublicKeyRootKey
        $ \rootK scheme -> do
            let encPwd = preparePassphrase scheme pwd
            let xprv = deriveAccountPrivateKeySophie purpose encPwd (getRawKey rootK) acctIx
