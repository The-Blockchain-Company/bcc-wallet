...
data TransactionLayer k tx = TransactionLayer
    ...
    , calcScriptExecutionCost
        :: ProtocolParameters
            -- Current protocol parameters
        -> tx
            -- The constructed transaction that could contain zerepoch scripts
        -> Coin
...
data TransactionCtx = TransactionCtx
    ...
    , txZerepochScriptExecutionCost :: Coin
    -- ^ Total execution cost of zerepoch scripts, determined by their execution units
...
defaultTransactionCtx = TransactionCtx
    ...
    , txZerepochScriptExecutionCost = Coin 0
    }
...
data ErrMkTransaction
    ...
    | ErrMkTransactionInvalidEra AnyBccEra
