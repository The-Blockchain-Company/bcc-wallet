...
import Bcc.Wallet.DB.Sqlite.Types
...
import qualified Bcc.Wallet.Primitive.Model as W
...
import qualified Bcc.Wallet.Primitive.Types.UTxO as W
...
migrateManually tr proxy defaultFieldValues =
    ...
    , addHardforkEpochIfMissing
      ...
      -- FIXME
      -- Temporary migration to fix Klarity flight wallets. This should
      -- really be removed as soon as we have a fix for the bcc-sl:wallet
  ...
  where
    ...
    --     Cole, from mnemonic          | ✓      | ✓
    --     Cole, from xprv              | ✓      | ✓
    --     Sophie, from mnemonic        | ✓      | ✓
    --     Sophie, from account pub key | ø      | ø
    ...
    addSeqStateDerivationPrefixIfMissing conn
        ...
        | isSophieDatabase = do
            addColumn_ conn True (DBField SeqStateDerivationPrefix) sophiePrefix
      ...
      where
        ...
        icarusPrefix = T.pack $ show $ toText
            $ Seq.DerivationPrefix (Seq.purposeBIP44, Seq.coinTypeBcc, minBound)

        isSophieDatabase =
            keyTypeDescriptor proxy == keyTypeDescriptor (Proxy @SophieKey)
        sophiePrefix = T.pack $ show $ toText
            $ Seq.DerivationPrefix (Seq.purposeCIP1852, Seq.coinTypeBcc, minBound)
