...
-- License: Apache-2.0
--
-- A client used to query asset metadata from the Bcc metadata-server.
--
-- The OpenAPI specification is here:
-- <https://github.com/The-Blockchain-Company/metadata-server/blob/master/specifications/api/openapi.yaml>
--
-- An important consideration is that bcc-wallet should not trust the
...
module Bcc.Wallet.TokenMetadata
    (
...
{-------------------------------------------------------------------------------
                       Client for Bcc metadata-server
-------------------------------------------------------------------------------}
