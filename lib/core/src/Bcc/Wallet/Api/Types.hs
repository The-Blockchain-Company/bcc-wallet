...
--
-- <https://github.com/The-Blockchain-Company/bcc-wallet/blob/master/specifications/api/swagger.yaml Wallet API Specification>
...
module Bcc.Wallet.Api.Types
    ...
    -- * API Types (Cole)
    , ApiColeWallet (..)
    , ApiColeWalletBalance (..)
    , ColeWalletPostData (..)
    , SomeColeWalletPostData (..)
    , ColeWalletFromXPrvPostData (..)
...
import Codec.Binary.Bech32.TH
    ( humanReadablePart )
import "bcc-addresses" Codec.Binary.Encoding
...
import qualified Bcc.Wallet.Primitive.AddressDerivation as AD
import qualified Bcc.Wallet.Primitive.Types as W
import qualified Bcc.Wallet.Primitive.Types.RewardAccount as W
import qualified Bcc.Wallet.Primitive.Types.TokenBundle as W
import qualified Bcc.Wallet.Primitive.Types.TokenMap as TokenMap
import qualified Bcc.Wallet.Primitive.Types.TokenMap as W
...
instance FromText ColeWalletStyle where
    fromText = fromTextToBoundedEnum SnakeLowerCase

instance ToText ColeWalletStyle where
    toText = toTextFromBoundedEnum SnakeLowerCase

data SndFactor
    = SndFactor

type family StyleSymbol (style :: ColeWalletStyle) :: Symbol where
...
type instance AllowedMnemonics 'Sophie   = '[15,18,21,24]
type instance AllowedMnemonics 'SndFactor = '[9,12]

fmtAllowedWords :: ColeWalletStyle -> String
fmtAllowedWords =
...
data ApiCoinSelection (n :: NetworkDiscriminant) = ApiCoinSelection
    ...
    , deposits :: ![Quantity "entropic" Natural]
...
data ApiCoinSelectionChange (n :: NetworkDiscriminant) = ApiCoinSelectionChange
    ...
    , amount :: !(Quantity "entropic" Natural)
...
data ApiWalletBalance = ApiWalletBalance
    { available :: !(Quantity "entropic" Natural)
    , total :: !(Quantity "entropic" Natural)
    , reward :: !(Quantity "entropic" Natural)
    } deriving (Eq, Generic, Show)
      deriving anyclass NFData
...
data ApiWalletUtxoSnapshotEntry = ApiWalletUtxoSnapshotEntry
    { bcc :: !(Quantity "entropic" Natural)
    , bccMinimum :: !(Quantity "entropic" Natural)
    , assets :: !(ApiT W.TokenMap)
    }
    deriving (Eq, Generic, Show)
    deriving anyclass NFData
...
data ApiStakePool = ApiStakePool
    ...
    , cost :: !(Quantity "entropic" Natural)
    , margin :: !(Quantity "percent" Percentage)
    , pledge :: !(Quantity "entropic" Natural)
...
data ApiStakePoolMetrics = ApiStakePoolMetrics
    { nonMyopicMemberRewards :: !(Quantity "entropic" Natural)
...
data WalletPostData = WalletPostData
    ...
    , mnemonicSentence :: !(ApiMnemonicT (AllowedMnemonics 'Sophie))
    , mnemonicSecondFactor :: !(Maybe (ApiMnemonicT (AllowedMnemonics 'SndFactor)))
    , name :: !(ApiT WalletName)
    , passphrase :: !(ApiT (Passphrase "raw"))
    } deriving (Eq, Generic, Show)

data SomeColeWalletPostData
    = RandomWalletFromMnemonic (ColeWalletPostData (AllowedMnemonics 'Random))
    | RandomWalletFromXPrv ColeWalletFromXPrvPostData
    | SomeIcarusWallet (ColeWalletPostData (AllowedMnemonics 'Icarus))
    | SomeTrezorWallet (ColeWalletPostData (AllowedMnemonics 'Trezor))
    | SomeLedgerWallet (ColeWalletPostData (AllowedMnemonics 'Ledger))
    | SomeAccount AccountPostData
    deriving (Eq, Generic, Show)

data ColeWalletPostData mw = ColeWalletPostData
    { mnemonicSentence :: !(ApiMnemonicT mw)
    , name :: !(ApiT WalletName)
    , passphrase :: !(ApiT (Passphrase "raw"))
    } deriving (Eq, Generic, Show)

data ColeWalletFromXPrvPostData = ColeWalletFromXPrvPostData
    { name :: !(ApiT WalletName)
...
data ColeWalletPutPassphraseData = ColeWalletPutPassphraseData
...
data ApiFee = ApiFee
    { estimatedMin :: !(Quantity "entropic" Natural)
    , estimatedMax :: !(Quantity "entropic" Natural)
    , minimumCoins :: ![Quantity "entropic" Natural]
    , deposit :: !(Quantity "entropic" Natural)
    } deriving (Eq, Generic, Show)
...
data ApiNetworkParameters = ApiNetworkParameters
    ...
    , minimumUtxoValue :: !(Quantity "entropic" Natural)
...
data ApiEraInfo = ApiEraInfo
    { cole :: !(Maybe ApiEpochInfo)
    , sophie :: !(Maybe ApiEpochInfo)
    , evie :: !(Maybe ApiEpochInfo)
    , jen :: !(Maybe ApiEpochInfo)
    , aurum :: !(Maybe ApiEpochInfo)
    } deriving (Eq, Generic, Show)
...
toApiNetworkParameters (NetworkParameters gp sp pp) txConstraints toEpochInfo = do
    cole <- traverse toEpochInfo (pp ^. #eras . #cole)
    sophie <- traverse toEpochInfo (pp ^. #eras . #sophie)
    evie <- traverse toEpochInfo (pp ^. #eras . #evie)
    jen <- traverse toEpochInfo (pp ^. #eras . #jen)
    aurum <- traverse toEpochInfo (pp ^. #eras . #aurum)

    let apiEras = ApiEraInfo { cole, sophie, evie, jen, aurum }
    ...
    return $ ApiNetworkParameters
        ...
        , minimumUtxoValue = toApiCoin $ case (view #minimumUTxOvalue pp) of
            ...
            MinimumUTxOValueCostPerWord _perWord ->
                txOutputMinimumBccQuantity txConstraints TokenMap.empty
        , eras = apiEras
...
-- | The response bcc-wallet returns upon successful submission of a
...
coinToQuantity :: Integral n => Coin -> Quantity "entropic" n
coinToQuantity = Quantity . fromIntegral . unCoin

coinFromQuantity :: Integral n => Quantity "entropic" n -> Coin
...
data ApiWalletMigrationPlan (n :: NetworkDiscriminant) = ApiWalletMigrationPlan
    ...
    , totalFee :: Quantity "entropic" Natural
...
data ApiVerificationKeySophie = ApiVerificationKeySophie
...
data ApiColeWallet = ApiColeWallet
    { id :: !(ApiT WalletId)
    , balance :: !(ApiColeWalletBalance)
    , assets :: !ApiWalletAssetsBalance
...
data ApiOurStakeKey (n :: NetworkDiscriminant) = ApiOurStakeKey
    ...
    , _stake :: !(Quantity "entropic" Natural)
      -- ^ The total bcc this stake key controlls / is associated with. This
      -- also includes the reward balance.
    , _rewardBalance :: !(Quantity "entropic" Natural)
...
instance ToJSON ApiVerificationKeySophie where
    toJSON (ApiVerificationKeySophie (pub, role_) hashed) =
...
instance FromJSON ApiVerificationKeySophie where
    parseJSON value = do
        ...
        pure $ ApiVerificationKeySophie (payload,role) hashing
      where
...
instance ToJSON SomeColeWalletPostData where
    toJSON = \case
...
instance FromJSON SomeColeWalletPostData where
    parseJSON = withObject "SomeColeWallet" $ \obj -> do
...
instance MkSomeMnemonic mw => FromJSON (ColeWalletPostData mw) where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance ToJSON (ColeWalletPostData mw) where
    toJSON = genericToJSON defaultRecordTypeOptions
...
instance FromJSON ColeWalletFromXPrvPostData where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance ToJSON ColeWalletFromXPrvPostData where
    toJSON = genericToJSON defaultRecordTypeOptions
...
instance FromJSON ColeWalletPutPassphraseData where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance ToJSON ColeWalletPutPassphraseData where
    toJSON = genericToJSON defaultRecordTypeOptions
...
data ApiColeWalletBalance = ApiColeWalletBalance
    { available :: !(Quantity "entropic" Natural)
    , total :: !(Quantity "entropic" Natural)
    } deriving (Eq, Generic, Show)
      deriving anyclass NFData

instance FromJSON ApiColeWalletBalance where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance ToJSON ApiColeWalletBalance where
    toJSON = genericToJSON defaultRecordTypeOptions
...
instance DecodeAddress n => FromJSON (ApiTxOut n) where
    parseJSON = withObject "ApiTxOut" $ \o -> do
        ...
        let splitV (Just amt, Nothing) (acc1, acc2) = (amt:acc1, acc2)
            ...
            splitV _ _ = error "parseValue should either return bcc or token"
        ...
        case (datum, amts) of
            ...
            (_, _) -> fail "there should be one 'entropic' in 'value'"
     where
       parseValue = withObject "Value" $ \o ->
           case HM.toList o of
               [] -> fail "Value should not be empty"
               cs -> for cs $ \pair ->
                   parseBcc pair <|> parseTokens pair
       parseBcc (numTxt, num) =
           if numTxt == "entropic" then do
               q <- parseJSON num
               pure (Just q, Nothing)
           else
               fail "expected 'entropic' key"
       parseTokens (numTxt, obj) =
           if numTxt == "entropic" then
...
instance EncodeAddress n => ToJSON (ApiTxOut n) where
    toJSON (ApiTxOut addr data' (Quantity amt) (ApiT assets')) = case data' of
      ...
      where
        objShared =
            [ "address" .= toJSON addr
            , "value" .= object (("entropic" .= toJSON amt) : tokens)
            ]
...
instance FromJSON (ApiT W.TokenBundle) where
    ...
    parseJSON = withObject "Value " $ \v ->
      ...
      where
        validateCoin :: Quantity "entropic" Word64 -> Aeson.Parser Coin
...
instance FromJSON ApiColeWallet where
    parseJSON = genericParseJSON defaultRecordTypeOptions
instance ToJSON ApiColeWallet where
    toJSON = genericToJSON defaultRecordTypeOptions
...
-- | Data required when submitting a mint/burn transaction. Bcc implements
...
-- bcc-wallet API at the moment. Because bcc-wallet manages the keys of
