...
import Bcc.Api
    ( AnyBccEra (..), BccEra (..), SerialiseAsCBOR (..) )
...
import Bcc.Wallet.Api.Server.Tls
...
import Bcc.Wallet.Primitive.AddressDerivation.Cole
    ( ColeKey, mkColeKeyFromMasterKey )
...
import Bcc.Wallet.Primitive.Delegation.UTxO
    ( stakeKeyCoinDistr )
import Bcc.Wallet.Primitive.Migration
    ( MigrationPlan (..) )
import Bcc.Wallet.Primitive.Model
    ( Wallet
    , availableBalance
    , availableUTxO
    , currentTip
...
import qualified Bcc.Wallet as W
import qualified Bcc.Wallet.Api.Types as Api
import qualified Bcc.Wallet.Network as NW
...
import qualified Bcc.Wallet.Primitive.Types.UTxO as UTxO
import qualified Bcc.Wallet.Registry as Registry
...
postWallet ctx generateKey liftKey (WalletOrAccountPostData body) = case body of
    Left  body' ->
        postSophieWallet ctx generateKey body'
    Right body' ->
        postAccountWallet ctx mkSophieWallet liftKey
...
postSophieWallet
    ...
    -> WalletPostData
    -> Handler ApiWallet
postSophieWallet ctx generateKey body = do
    ...
    fst <$> getWallet ctx (mkSophieWallet @_ @s @k) (ApiT wid)
  where
...
mkSophieWallet ctx wid cp meta pending progress = do
    ...
    -- In the Sophie era of Cole;Sophie;Evie toApiWalletDelegation using
    ...
    -- 1. We expect the next few eras to have the same epoch length as Sophie
...
postRandomWallet
    ...
    => ctx
    -> ColeWalletPostData '[12,15,18,21,24]
    -> Handler ApiColeWallet
postRandomWallet ctx body = do
  ...
  where
    wName = getApiT (body ^. #name)
    pwd   = preparePassphrase EncryptWithPBKDF2 $ getApiT (body ^. #passphrase)
    rootXPrv = Cole.generateKeyFromSeed seed pwd
...
postRandomWalletFromXPrv ctx body = do
    s <- liftIO $ mkRndState coleKey <$> getStdRandom random
    void $ liftHandler $ createWalletWorker @_ @s @k ctx wid
        (\wrk -> W.createWallet @(WorkerCtx ctx) @_ @s @k wrk wid wName s)
        idleWorker
    withWorkerCtx ctx wid liftE liftE $ \wrk -> liftHandler $
        W.attachPrivateKeyFromPwdHash wrk wid (coleKey, pwd)
    fst <$> getWallet ctx mkLegacyWallet (ApiT wid)
  where
    wName = getApiT (body ^. #name)
    pwd   = getApiT (body ^. #passphraseHash)
    masterKey = getApiT (body ^. #encryptedRootPrivateKey)
    coleKey = mkColeKeyFromMasterKey masterKey
    wid = WalletId $ digest $ publicKey coleKey
...
-- Legacy wallets like 'Cole Random' and 'Icarus Sequential' are handled
...
withLegacyLayer
    :: forall cole icarus n a.
        ( cole ~ ApiLayer (RndState n) ColeKey
    ...
    -> (icarus, Handler a)
    -> Handler a
withLegacyLayer (ApiT wid) (cole, withCole) (icarus, withIcarus) =
...
withLegacyLayer'
    ...
    => ApiT WalletId
    -> (cole, Handler a, ErrWalletNotResponding -> Handler a)
    -> (icarus, Handler a, ErrWalletNotResponding -> Handler a)
    -> Handler a
withLegacyLayer' (ApiT wid)
  (cole, withCole, deadCole)
  (icarus, withIcarus, deadIcarus)
    = tryCole (const $ tryIcarus liftE deadIcarus) deadCole
  where
    ...
    tryCole onMissing onNotResponding = withWorkerCtx @_
        @(RndState n)
        @ColeKey
        cole
        wid
...
putColeWalletPassphrase
    ...
    -> ColeWalletPutPassphraseData
    -> Handler NoContent
putColeWalletPassphrase ctx (ApiT wid) body = do
    let (ColeWalletPutPassphraseData oldM (ApiT new)) = body
...
getWalletUtxoSnapshot ctx (ApiT wid) = do
  ...
  where
    ...
    mkApiWalletUtxoSnapshotEntry (bundle, minCoin) = ApiWalletUtxoSnapshotEntry
        { bcc = coinToQuantity $ view #coin bundle
        , bccMinimum = coinToQuantity minCoin
...
-- no rewards => bcc in distr == utxo balance
...
mkApiWalletMigrationPlan s addresses rewardWithdrawal plan =
  ...
  where
    ...
    mkApiWalletMigrationBalance b = ApiWalletMigrationBalance
        { bcc = coinToQuantity $ view #coin b
...
getNetworkInformation st nl = liftIO $ do
  ...
  where
    ...
    toApiEra :: AnyBccEra -> ApiEra
    toApiEra (AnyBccEra ColeEra) = ApiCole
    toApiEra (AnyBccEra SophieEra) = ApiSophie
    toApiEra (AnyBccEra EvieEra) = ApiEvie
    toApiEra (AnyBccEra JenEra) = ApiJen
    toApiEra (AnyBccEra AurumEra) = ApiAurum
...
mkRewardAccountBuilder
    :: forall ctx s k (n :: NetworkDiscriminant) sophie.
        ( ctx ~ ApiLayer s k
        , sophie ~ SeqState n SophieKey
        , HardDerivation k
        , Bounded (Index (AddressIndexDerivationType k) 'AddressK)
        , WalletKey k
        , Typeable s
...
mkRewardAccountBuilder ctx wid withdrawal = do
    ...
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
        case (testEquality (typeRep @s) (typeRep @sophie), withdrawal) of
            (Nothing, Just{}) ->
                liftHandler $ throwE ErrReadRewardAccountNotASophieWallet
            ...
            (Just Refl, Just (ExternalWithdrawal (ApiMnemonicT mw))) -> do
                let (xprv, acct, path) = W.someRewardAccount @SophieKey mw
                wdrl <- liftHandler (W.queryRewardBalance @_ wrk acct)
...
instance IsServerError ErrBalanceTx where
    toServerError = \case
        ...
        ErrBalanceTxUpdateError ErrColeTxNotSupported ->
            apiError err403 CreatedInvalidTransaction
                "Balancing Cole transactions is not supported."
...
instance IsServerError ErrListTransactions where
    toServerError = \case
        ...
        ErrListTransactionsMinWithdrawalWrong ->
            ...
            "The minimum withdrawal value must be at least 1 Entropic."
    ...
    toServerError = \case
        ErrCreateMigrationPlanEmpty ->
            apiError err403 NothingToMigrate $ mconcat
                ...
                , "amount of bcc in your wallet is insufficient to pay for "
                , "any of the funds to be migrated. Try adding some bcc to "
            ...
            case selectionError of
                ...
                Balance.InsufficientMinCoinValues xs ->
                    apiError err403 UtxoTooSmall $ mconcat
                        [ "Some outputs have bcc values that are too small. "
                        , "There's a minimum bcc value specified by the "
                        , "protocol that each output must satisfy. I'll handle "
                        , "that minimum value myself when you do not explicitly "
                        , "specify an bcc value for an output. Otherwise, you "
                        , "must specify enough bcc. Here are the problematic "
                ...
                Balance.UnableToConstructChange e ->
                    apiError err403 CannotCoverFee $ T.unwords
                        ...
                        , "is not enough bcc available to pay for the fee and"
                        , "also pay for the minimum bcc quantities of all"
                        , "change outputs. I need approximately"
                        , pretty (shortfall e)
                        , "bcc to proceed. Try increasing your wallet balance"
