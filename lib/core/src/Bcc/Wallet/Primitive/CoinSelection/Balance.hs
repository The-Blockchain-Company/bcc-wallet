...
--
module Bcc.Wallet.Primitive.CoinSelection.Balance
    (
...
import Bcc.Wallet.Primitive.Types.UTxOIndex
...
import qualified Bcc.Wallet.Primitive.Types.Tx as Tx
...
-- | Calculates the bcc selection delta.
--
-- See 'SelectionDelta'.
--
selectionDeltaCoin
...
selectionHasValidSurplus constraints selection =
  ...
  where
    ...
    surplusIsValid = (&&)
        <$> surplusHasNoNonBccAssets
        <*> surplusNotBelowMinimumCost

    -- None of the non-bcc assets can have a surplus.
    surplusHasNoNonBccAssets :: TokenBundle -> Bool
    surplusHasNoNonBccAssets surplus =
...
-- | Calculates the bcc selection surplus, assuming there is a surplus.
--
...
data UnableToConstructChangeError = UnableToConstructChangeError
    { requiredCost
        ...
        -- considered valid. This does not include min Bcc values.
    , shortfall
        :: !Coin
...
-- | Transforms a set of outputs (provided by users) into valid Bcc outputs.
--
-- Every output in Bcc needs to have a minimum quantity of bcc, in order to
-- prevent attacks that flood the network with worthless UTxOs.
--
-- However, we do not require users to specify the minimum bcc quantity
...
-- quantity manually for each non-bcc output. That quantity is the minimum
...
--      a single minimal bcc-only output to act as a change generation target,
--      such that:
--
--          computeUTxOBalanceSufficiencyInfo params ==
...
performSelectionEmpty performSelectionFn constraints params =
  ...
  where
    ...
    -- outputs must have at least the minimum bcc quantity. Therefore, the
    -- dummy output must also satisfy this condition.
    --
    -- However, we must also ensure that the value is non-zero, since:
    --
    --   1. Under some cost models, the 'computeMinimumBccQuantity' function
    --      has a constant value of zero.
    --
    --   2. The change generation algorithm requires that the total bcc balance
    --      of all outputs is non-zero.
    --
    minCoin :: Coin
    minCoin = max
        (Coin 1)
        (view #computeMinimumBccQuantity constraints TokenMap.empty)
...
performSelectionNonEmpty constraints params
    ...
    -- Are the minimum bcc quantities of the outputs too small?
  ...
  where
        ...
        mkInsufficientMinCoinValueError o
            ...
            | otherwise =
                Just $ InsufficientMinCoinValueError
                    { expectedMinCoinValue, outputWithInsufficientBcc = o }
          where
            expectedMinCoinValue = computeMinimumBccQuantity
    ...
    -- until it succeeds.
    --
    -- Between each call, it selects an extra bcc-only input to inject
    -- additional bcc to construct change outputs.
    --
    -- Eventually it returns just a final selection, or 'Nothing' if no more
    -- bcc-only inputs are available.
    --
    ...
    makeChangeRepeatedly s = case mChangeGenerated of
        ...
        Right change ->
            ...
            -- change outputs is fewer than optimal, because the supply of bcc
            -- was insufficient. Try again with more bcc to see if it leads to
            -- an improvement:
            selectOneEntry s >>= \case
                Just s' ->
                    makeChangeRepeatedly s'
                Nothing ->
                    -- There is no more bcc available. Terminate with a
                    -- less-than-optimal number of change outputs.
                    pure $ Right $ mkSelectionResult change

        Left changeErr ->
            -- We've failed to make any change outputs, because the supply of
            -- bcc was insufficient. Try again with more bcc.
            selectOneEntry s >>= \case
                Just s' ->
                    makeChangeRepeatedly s'
                Nothing ->
                    -- There is no more bcc available, and we were unable to
      ...
      where
        ...
        mChangeGenerated = makeChange MakeChangeCriteria
            { minCoinFor = computeMinimumBccQuantity
            , bundleSizeAssessor = TokenBundleSizeAssessor assessTokenBundleSize
            , requiredCost
            , extraCoinSource
            , extraCoinSink
...
runSelection params =
  ...
  where
    ...
    -- necessarily has a non-zero bcc amount. By running the other selectors
...
-- | Specializes 'selectMatchingQuantity' to bcc.
--
selectCoinQuantity
    :: MonadRandom m
    => IsUTxOSelection utxoSelection
    ...
    -> m (Maybe UTxOSelectionNonEmpty)
selectCoinQuantity =
    selectMatchingQuantity (WithBccOnly :| [Any])
...
data MakeChangeCriteria minCoinFor bundleSizeAssessor = MakeChangeCriteria
    { minCoinFor :: minCoinFor
      -- ^ A function that computes the minimum required bcc quantity for a
      -- particular output.
    , bundleSizeAssessor :: bundleSizeAssessor
        -- ^ A function to assess the size of a token bundle.
    , requiredCost :: Coin
      -- ^ The minimal (and optimal) delta between the total bcc balance
      -- of all input bundles and the total bcc balance of all output and
...
--
-- Returns 'Nothing' if the specified inputs do not provide enough bcc to
-- satisfy the minimum delta and minimum bcc quantities of the change bundles
-- generated.
--
...
--
--    2.  The total bcc balance of all outputs is zero.
--
-- Pre-condition (1) should be satisfied by any result produced by the
-- 'runSelection' function.
--
-- Pre-condition (2) should be satisfied by assigning a minimum bcc quantity
      ...
      where
        ...
        splitOversizedMaps =
          ...
          where
            ...
            -- bcc quantity will be, since bcc quantities are assigned at a
            -- later stage (in 'assignCoinsToChangeMaps').
            --
            -- Therefore, we err on the side of caution, and assess the size
            -- of a change map combined with the maximum possible bcc quantity.
            --
            ...
            -- be within the limit when combined with its final bcc quantity.
            --
...
--
--    - attempts to assign a minimum bcc quantity to every change map, but
--      iteratively drops empty change maps from the start of the list if the
--      amount of bcc is insufficient to cover them all.
--
--    - continues dropping empty change maps from the start of the list until
--      it is possible to assign a minimum bcc value to all remaining entries.
--
--    - returns a list that is identical in length to the input list if (and
--      only if) it was possible to assign a minimum bcc quantity to all change
--      maps.
--
--    - returns a list that is shorter than the input list if it was only
--      possible to assign a minimum bcc quantity to a suffix of the given
--      list.
--
--    - fails if (and only if) there was not enough bcc available to assign the
--      minimum bcc quantity to all non-empty change maps.
--
assignCoinsToChangeMaps
    :: HasCallStack
    => Coin
    -- ^ The total quantity of bcc available, including any extra source of bcc.
    -> (TokenMap -> Coin)
    -- ^ A function to calculate the minimum required bcc quantity for any
    -- token map.
    ...
    -- enough bcc available to assign a minimum bcc quantity to all non-empty
    -- token maps.
assignCoinsToChangeMaps bccAvailable minCoinFor pairsAtStart
    | not changeMapsCorrectlyOrdered =
        changeMapsNotCorrectlyOrderedError
    | otherwise =
        loop bccRequiredAtStart pairsAtStart
  where
    loop !bccRequired !pairsNonEmpty = case pairsNonEmpty of

        pair :| pairs | bccAvailable >= bccRequired ->
            -- We have enough bcc available to pay for the minimum required
            ...
            -- Calculate the amount of bcc that remains after assigning the
            -- minimum amount to each map. This should be safe, as we have
            -- already determined that we have enough bcc available:
            let bccRemaining = bccAvailable `Coin.distance` bccRequired in
            -- Partition any remaining bcc according to the weighted
            ...
            let bundlesForOutputCoins = TokenBundle.fromCoin <$>
                    makeChangeForCoin outputCoinsRemaining bccRemaining in
            -- Finally, combine the minimal coin asset bundles with the
            -- bundles obtained by partitioning the remaining bcc amount:
            Right $ NE.toList $ NE.zipWith (<>)
                bundlesForAssetsWithMinimumCoins
                bundlesForOutputCoins

        (m, _) :| (p : ps) | TokenMap.isEmpty m && bccAvailable < bccRequired ->
            -- We don't have enough bcc available to pay for the minimum
            -- required amount of every asset map, but we do have an empty
            -- asset map that is safe to drop. This will reduce the amount of
            -- bcc required by a small amount:
            let bccRequired' = bccRequired `Coin.distance` minCoinFor m in
            loop bccRequired' (p :| ps)

        (m, _) :| [] | TokenMap.isEmpty m && bccAvailable < bccRequired ->
            -- We didn't have any non-bcc assets at all in our change, and we
            -- also don't have enough bcc available to pay even for a single
            -- change output. We just burn the available bcc amount (which
            -- will be small), returning no change.
            Right []

        _ ->
            -- We don't have enough bcc available, and there are no empty token
            -- maps available to drop. We have to give up at this point.
            Left (bccRequired `Coin.difference` bccAvailable)

    bccRequiredAtStart = F.fold $ minCoinFor . fst <$> pairsAtStart
...
-- | Assigns the minimum required bcc quantity to a token map.
--
...
-- | Constructs a list of bcc change outputs based on the given distribution.
--
...
makeChangeForCoin
    ...
    => NonEmpty Coin
        ...
        -- bcc quantity should be distributed.
    -> Coin
        -- ^ A surplus bcc quantity to be distributed.
    -> NonEmpty Coin
