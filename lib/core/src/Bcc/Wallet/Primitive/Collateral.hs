...
module Bcc.Wallet.Primitive.Collateral
    (
    -- * Data types
      AddressType(..)
    , Credential(..)
...
-- In the realm of bcc-ledger-specs, we recognize the following types of
-- addresses:
--   (see https://hydra.tbco.io/build/6752483/download/1/ledger-spec.pdf):
--
...
data AddressType
    ...
    | BootstrapAddress
    -- ^ A Bootstrap (a.k.a. Cole) address
    deriving (Eq, Show)
...
-- a collateral input, returns @Just@ along with the total BCC value of the
-- UTxO. Otherwise returns @Nothing@ if it is not a suitable collateral value.
asCollateral
    :: TxOut
    -- ^ TxOut from a UTxO entry
    -> Maybe Coin
    -- ^ The total BCC value of that UTxO if it is suitable for collateral,
    -- otherwise Nothing.
asCollateral txOut
