...
module Bcc.Wallet.Primitive.Slotting
    ( -- ** Queries
      Qry
    , currentEpoch
    , epochOf
...
import qualified Shardagnostic.Consensus.BlockchainTime.WallClock.Types as Bcc
import qualified Shardagnostic.Consensus.HardFork.History.Qry as HF
import qualified Shardagnostic.Consensus.HardFork.History.Summary as HF
...
-- @interpretQuery@.
--
-- == Differences to the underlying consensus 'Shardagnostic.Consensus.HardFork.History.Qry.Qry'
--
-- @HF.Qry@ can only be interpreted in a
-- single era. If you have
--
-- @
--     q1 = epochOf someSlotInCole
--     q2 = epochOf someSlotInSophie
-- @
--
...
toSlotId slot = do
    ...
    return $ SlotId
        (EpochNo $ fromIntegral $ Bcc.unEpochNo e)
...
timeOfEpoch epoch = do
    ...
    el <- EraContainedQry $ qryFromExpr $ EEpochSize $ ELit $ toBccEpochNo epoch
    sl <- EraContainedQry $ qryFromExpr $ ESlotLength $ ELit ref

    let convert = fromRational . toRational
    let el' = convert $ Bcc.unEpochSize el
    let sl' = Bcc.getSlotLength sl

    let timeInEpoch = el' * sl'

    return (refTime, timeInEpoch `addUTCTime` refTime)
  where
    toBccEpochNo (EpochNo e) = Bcc.EpochNo $ fromIntegral e
...
firstSlotInEpoch = EraContainedQry . epochToSlot' . convertEpochNo
  where
    convertEpochNo (EpochNo e) = Bcc.EpochNo $ fromIntegral e
...
-- | Same as 'Bcc.toRelativeTime', but has error handling for times before
...
toRelativeTime (StartTime start) utc
    | utc < start = Nothing
    | otherwise = Just $ Bcc.toRelativeTime (SystemStart start) utc
...
fromRelativeTime t = do
    start <- getStartTime
    pure (Bcc.fromRelativeTime (coerce start) t)
...
mkSingleEraInterpreter start sp = TimeInterpreter
  ...
  where
    ...
    sz = Bcc.EpochSize $ fromIntegral $ unEpochLength $ sp ^. #getEpochLength
    len = Bcc.mkSlotLength $ unSlotLength $ sp ^. #getSlotLength
...
-- great. Also ADP-575.
--
-- From the underlying shardagnostic-consensus function:
--
