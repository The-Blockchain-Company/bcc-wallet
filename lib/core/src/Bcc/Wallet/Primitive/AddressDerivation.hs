...
module Bcc.Wallet.Primitive.AddressDerivation
    (
    -- * HD Derivation
      Depth (..)
    , Index (..)
...
--
-- bcc-wallet implements two similar HD schemes on top of BIP-0039 that are:
--
-- - BIP-0044 (for so-called Icarus wallets)
-- - CIP-1815 (for so-called Sophie and Quibitous wallets)
--
-- Both scheme works by considering 5 levels of derivation from an initial root
-- key (see also 'Depth' from Bcc.Wallet.Primitive.AddressDerivation). A
...
--
-- However, historically, addresses in Bcc used to be generated across the
...
--
-- - @Mainnet@: is a shortcut for quickly pointing to mainnet. On Cole and
--              Sophie, it assumes no discrimination. It has a known magic and
...
-- various key types:
--
-- 1. For 'ColeKey', it can only be the address itself!
-- 2. For 'SophieKey', then the "payment" fingerprint refers to the payment key
