...
-- scheme as used by the legacy Bcc wallets.

module Bcc.Wallet.Primitive.AddressDiscovery.Random
    (
    -- ** State
      RndState (..)
    , RndStateLike
...
import Bcc.Wallet.Primitive.AddressDerivation.Cole
    ( ColeKey (..), deriveAccountPrivateKey, deriveAddressPrivateKey )
...
-- decoded as a Cole HD random address, and where the wallet key can be used
...
instance IsOwned (RndState n) ColeKey where
...
-- seed.
mkRndState :: ColeKey 'RootK XPrv -> Int -> RndState n
...
instance PaymentAddress n ColeKey => GenChange (RndState n) where
    type ArgGenChange (RndState n) = (ColeKey 'RootK XPrv, Passphrase "encryption")
...
deriveRndStateAddress
    :: forall n. (PaymentAddress n ColeKey)
    => ColeKey 'RootK XPrv
    -> Passphrase "encryption"
    -> DerivationPath
    -> Address
...
instance KnownNat p => IsOwned (RndAnyState n p) ColeKey where
    isOwned _ _ _ = Nothing

instance PaymentAddress n ColeKey => GenChange (RndAnyState n p) where
