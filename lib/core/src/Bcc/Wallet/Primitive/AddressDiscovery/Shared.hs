...
module Bcc.Wallet.Primitive.AddressDiscovery.Shared
    (
    -- ** State
      SharedState (..)
    , SharedStateFields (..)
...
import qualified Bcc.Address as CA
...
mkSharedState accIx pending = updateSharedState state id
  where
    state = SharedState
        { derivationPrefix = DerivationPrefix (purposeCIP1854, coinTypeBcc, accIx)
