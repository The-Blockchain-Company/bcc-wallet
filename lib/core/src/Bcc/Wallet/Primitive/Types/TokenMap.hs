...
-- | Provides the 'TokenMap' type, which represents a map of named non-bcc
--   token quantities scoped by token policy.
--
-- The 'TokenMap' type does not provide a way to store bcc quantities. If you
-- also need to store bcc quantities, use the 'TokenBundle' type.
--
-- This module is meant to be imported qualified. For example:
--
-- >>> import qualified Bcc.Wallet.Primitive.Types.TokenMap as TM
--
module Bcc.Wallet.Primitive.Types.TokenMap
    (
    -- * Types
