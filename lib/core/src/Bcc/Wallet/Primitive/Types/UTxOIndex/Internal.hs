...
--
module Bcc.Wallet.Primitive.Types.UTxOIndex.Internal
    (
    ----------------------------------------------------------------------------
    -- Public Interface
    ----------------------------------------------------------------------------
...
data UTxOIndex = UTxOIndex
    { assetsAll
        ...
        -- An index of all entries that contain at least one non-bcc asset.
    , assetsSingleton
        :: !(Map AssetId (NonEmptySet TxIn))
        -- An index of all entries that contain exactly one non-bcc asset.
    , coins
        :: !(Set TxIn)
        -- An index of all entries that contain no non-bcc assets.
    , balance
        :: !TokenBundle
...
data SelectionFilter
    ...
    | WithBccOnly
        -- ^ Select any UTxO entry that only has bcc and no other assets.
    | WithAsset AssetId
        -- ^ Select any UTxO entry that has a non-zero quantity of the specified
        -- asset.
    | WithAssetOnly AssetId
        -- ^ Select any UTxO entry that has a non-zero quantity of the specified
        -- asset, but no other non-bcc assets.
    deriving (Eq, Show)
...
selectRandom u selectionFilter =
  ...
  where
    ...
    selectionSet = case selectionFilter of
        ...
        WithBccOnly -> entriesWithBccOnly u
...
-- | Returns the set of keys for entries that have no assets other than bcc.
--
entriesWithBccOnly :: UTxOIndex -> Set TxIn
entriesWithBccOnly = coins
...
-- | Returns the set of keys for entries that have no non-bcc assets other than
--   the given asset.
--
