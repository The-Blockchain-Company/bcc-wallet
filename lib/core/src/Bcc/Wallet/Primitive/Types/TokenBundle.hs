...
-- | Provides the 'TokenBundle' type, which combines a 'Coin' (entropic) value
...
--
module Bcc.Wallet.Primitive.Types.TokenBundle
    (
    -- * Types
      TokenBundle (..)
    , AssetId (..)
...
-- | Combines a 'Coin' (entropic) value with a map of named token quantities,
...
-- | Creates a singleton token bundle from an bcc 'Coin' value.
--
fromCoin :: Coin -> TokenBundle
fromCoin c = TokenBundle c mempty

-- | Coerces a token bundle to an bcc 'Coin' value.
--
...
-- | Gets the current bcc 'Coin' value from a token bundle.
--
...
-- | Sets the current bcc 'Coin' value for a token bundle.
--
...
--
-- The bcc 'Coin' quantity is equipartitioned across the resulting bundles.
--
-- The quantities of each non-bcc asset are unchanged.
--
equipartitionAssets
    :: TokenBundle
