...
--
module Bcc.Wallet.Primitive.Types.Address
    ( Address (..)
    , AddressState (..)
    ) where
...
-- | Representation of Bcc addresses.
--
-- Addresses are basically a human-friendly representation of public keys.
-- Historically in Bcc, there exist different sort of addresses, and new
-- ones are to come. So far, we can distinguish between three types of
-- address:
--
-- - Cole Random addresses, which holds a payload with derivation path details
-- - Cole Sequential addresses, also known as Icarus'style addresses
-- - Sophie base addresses, see also [implementation-decisions/address](https://github.com/The-Blockchain-Company/implementation-decisions/blob/master/text/0001-address.md)
--
-- For more details, see also [About Address Derivation](https://github.com/The-Blockchain-Company/bcc-wallet/wiki/About-Address-Derivation)
--
-- Sophie base addresses can be divided into two types:
--
