...
--
module Bcc.Wallet.Primitive.Types.Tx
    (
    -- * Types
      Tx (..)
    , TxIn (..)
...
-- | Primitive @Tx@-type.
--
-- Currently tailored for quibitous in that inputs are @(TxIn, Coin)@
...
data Tx = Tx
    ...
    , fee
        ...
        -- explicitly in Sophie, but not in Cole although in Cole they can
    ...
    , metadata
        ...
        --
        -- See Appendix E of
        -- <https://hydra.tbco.io/job/Bcc/bcc-ledger-specs/delegationDesignSpec/latest/download-by-type/doc-pdf/delegation_design_spec Sophie Ledger: Delegation/Incentives Design Spec>.
    ...
    , scriptValidity
        ...
        -- constructing the block. May be 'Nothing' for pre-Aurum and pending
        -- transactions.
...
--
-- 'Coin' values correspond to the bcc asset.
--
...
data UnsignedTx input output change withdrawal = UnsignedTx
    ...
    , unsignedInputs
        :: NonEmpty input
        -- Inputs are *necessarily* non-empty because Bcc requires at least
...
instance Show SealedTx where
    -- InAnyBccEra is missing a Show instance, so define one inline.
    showsPrec d (SealedTx v tx' bs) = showParen (d > 10) $
        ...
        showsPrec 11 v
      where
        showsTx :: InAnyBccEra Bcc.Tx -> ShowS
        showsTx (InAnyBccEra era tx) =
            showString "InAnyBccEra" .
            showChar ' ' .
            showsPrec 11 era .
            showChar ' ' .
            showsPrec 11 tx
...
instance Buildable SealedTx where
    build (SealedTx v tx' bs) = if v then buildTx tx' else hexF bs
      where
        buildTx :: InAnyBccEra Bcc.Tx -> Builder
        buildTx (InAnyBccEra _ tx) = build $ pShowNoColor tx
...
sameEra :: InAnyBccEra a -> InAnyBccEra a -> Bool
sameEra (InAnyBccEra e1 _) (InAnyBccEra e2 _) =
    case testEquality e1 e2 of
        Just Refl -> True
        Nothing -> False

instance NFData SealedTx where
    rnf (SealedTx v (InAnyBccEra _ tx) bs) = tx' `deepseq` bs `deepseq` ()
      where
        -- Showing the transaction should be enough to fully evaluate it.
        tx' = if v then show tx else ""

getSealedTxBody :: SealedTx -> InAnyBccEra Bcc.TxBody
getSealedTxBody (SealedTx _ (InAnyBccEra era tx) _) =
    InAnyBccEra era (Bcc.getTxBody tx)

getSealedTxWitnesses :: SealedTx -> [InAnyBccEra Bcc.KeyWitness]
getSealedTxWitnesses (SealedTx _ (InAnyBccEra era tx) _) =
    [InAnyBccEra era w | w <- Bcc.getTxWitnesses tx]

-- | Construct a 'SealedTx' from a "Bcc.Api" transaction.
sealedTxFromBcc :: InAnyBccEra Bcc.Tx -> SealedTx
sealedTxFromBcc tx = SealedTx True tx (bccTxToBytes tx)
  where
    bccTxToBytes :: InAnyBccEra Bcc.Tx -> ByteString
    bccTxToBytes (InAnyBccEra _era tx') = Bcc.serialiseToCBOR tx'

-- | Construct a 'SealedTx' from a "Bcc.Api" transaction.
sealedTxFromBcc' :: Bcc.IsBccEra era => Bcc.Tx era -> SealedTx
sealedTxFromBcc' = sealedTxFromBcc . InAnyBccEra Bcc.bccEra

-- | Construct a 'SealedTx' from a 'Bcc.Api.TxBody'.
sealedTxFromBccBody :: Bcc.IsBccEra era => Bcc.TxBody era -> SealedTx
sealedTxFromBccBody = sealedTxFromBcc . InAnyBccEra Bcc.bccEra . mk
  where
    mk body = Bcc.Tx body []

-- | Deserialise a Bcc transaction. The transaction can be in the format of
-- any era. This function will try the most recent era first, then
-- previous eras until 'ColeEra'.
bccTxFromBytes
    :: AnyBccEra -- ^ Most recent era
    -> ByteString -- ^ Serialised transaction
    -> Either DecoderError (InAnyBccEra Bcc.Tx)
bccTxFromBytes maxEra bs = asum $ map snd $ filter (withinEra maxEra . fst)
    [ deserialise AurumEra  Bcc.AsAurumEra
    , deserialise JenEra    Bcc.AsJenEra
    , deserialise EvieEra Bcc.AsEvieEra
    , deserialise SophieEra Bcc.AsSophieEra
    , deserialise ColeEra   Bcc.AsColeEra
    ]
  where
    deserialise
        ...
        => BccEra era
        -> Bcc.AsType era
        -> (AnyBccEra, Either DecoderError (InAnyBccEra Bcc.Tx))
    deserialise era asEra =
        ( anyBccEra era
        , InAnyBccEra era <$> deserialiseFromCBOR (Bcc.AsTx asEra) bs
        )
    ...
    asum xs = case partitionEithers xs of
        (_, (a:_)) -> Right a
        ((e:_), []) -> Left e
        ([], []) -> internalError "bccTxFromBytes: impossible"

-- | @a `withinEra` b@ is 'True' iff @b@ is the same era as @a@, or an earlier
-- one.
withinEra :: AnyBccEra -> AnyBccEra -> Bool
withinEra = (>=) `on` numberEra
  where
    numberEra :: AnyBccEra -> Int
    numberEra (AnyBccEra e) = case e of
        ColeEra   -> 1
        SophieEra -> 2
        EvieEra -> 3
        JenEra    -> 4
...
sealedTxFromBytes = sealedTxFromBytes' (anyBccEra AurumEra)
...
-- serialised Bcc transaction could not have this header, because they
...
getSerialisedTxParts (SealedTx _ (InAnyBccEra _ tx) _) = SerialisedTxParts
    { serialisedTxBody = serialiseToCBOR $ Bcc.getTxBody tx
    , serialisedTxWitnesses = serialiseToCBOR <$> Bcc.getTxWitnesses tx
    }
...
data TransactionInfo = TransactionInfo
    ...
    -- block. May be 'Nothing' for pre-Aurum and pending transactions.
    } deriving (Generic, Show, Eq)

instance NFData TransactionInfo

-- | Indicates whether the script associated with a transaction has passed or
-- failed validation. Pre-Aurum era, scripts were not supported.
...
data TxConstraints = TxConstraints
    ...
    , txOutputMinimumBccQuantity :: TokenMap -> Coin
      -- ^ The variable minimum bcc quantity of a transaction output.
...
txOutputCoinMinimum constraints = txOutputMinimumBccQuantity constraints mempty
