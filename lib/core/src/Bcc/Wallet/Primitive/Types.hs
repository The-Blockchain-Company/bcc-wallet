...
-- Haskell translation of the [Formal Specification for a Bcc Wallet](https://github.com/The-Blockchain-Company/bcc-wallet/blob/master/specifications/wallet/formal-specification-for-a-bcc-wallet.pdf)
--
-- It doesn't contain any particular business-logic code, but defines a few
-- primitive operations on Wallet core types as well.

module Bcc.Wallet.Primitive.Types
    (
    -- * Block
      Block(..)
    , BlockHeader(..)
...
--
-- Whereas @Wallet s@ in 'Bcc.Wallet.Primitive' can be updated using
...
data PassphraseScheme
    ...
    | EncryptWithPBKDF2
        -- ^ Encryption scheme used since bcc-wallet
...
--
-- The metadata information is not used directly by bcc-wallet, but rather
...
--
-- FIXME 'Double' is an old artifact from the Cole era on bcc-sl. It must
-- go.
data FeePolicy = LinearFee
    (Quantity "entropic" Double)
    (Quantity "entropic/byte" Double)
    deriving (Eq, Show, Generic)
...
data SlottingParameters = SlottingParameters
    ...
    , getSecurityParameter :: Quantity "block" Word32
        ...
        --
        -- Determines the value of 'stabilityWindowCole' and
        -- 'stabilityWindowSophie'.
    } deriving (Generic, Show, Eq)

instance NFData SlottingParameters

-- | In Cole, this stability window is equal to 2k slots, where _k_ is the
--  'getSecurityParameter'
stabilityWindowCole :: SlottingParameters -> Quantity "block" Word64
stabilityWindowCole sp = Quantity (2 * k)
  where
    k = fromIntegral $ getQuantity $ getSecurityParameter sp

-- | In Sophie, this stability window is equal to _3k/f_ slots where _k_ is the
-- 'getSecurityParameter' and _f_ is the 'ActiveSlotCoefficient'.
stabilityWindowSophie :: SlottingParameters -> Quantity "block" Word64
stabilityWindowSophie sp = Quantity len
  where
    len = ceiling (3 * k / f)
    k = fromIntegral $ getQuantity $ getSecurityParameter sp
...
-- |
--
-- It is expected that there is an order, @cole, sophie, evie, jen@, by
...
instance Buildable (EraInfo EpochNo) where
    build (EraInfo cole sophie evie jen aurum) = blockListF' "-" id
        [ "cole" <> boundF cole
        , "sophie" <> boundF sophie
        , "evie" <> boundF evie
        , "jen" <> boundF jen
        , "aurum" <> boundF aurum
        ]
...
data MinimumUTxOValue
    -- | In Sophie, tx outputs could only be created if they were larger than
    -- this `MinimumUTxOValue`.
    = MinimumUTxOValue Coin

    -- | With Aurum, `MinimumUTxOValue` is replaced by an bcc-cost per word of
    -- the output. Note that the aurum ledger assumes fixed sizes for address
...
data ProtocolParameters = ProtocolParameters
    ...
    , executionUnitPrices
        ...
        -- ^ The prices for 'ExecutionUnits' as a fraction of a 'Entropic' and
...
-- | The maximum size of a serialized `TokenBundle` (`_maxValSize` in the Aurum
-- ledger)
...
-- | Hard-coded protocol magic for the Cole MainNet
...
--  @PoolCert@ type in bcc-ledger-specs.
