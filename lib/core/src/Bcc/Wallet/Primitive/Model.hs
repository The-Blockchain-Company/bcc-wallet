...
-- License: Apache-2.0
--
-- Here we find the "business logic" to manage a Bcc wallet. This is a
-- direct implementation of the model from the [Formal Specification for a Bcc Wallet](https://github.com/The-Blockchain-Company/bcc-wallet/blob/master/specifications/wallet/formal-specification-for-a-bcc-wallet.pdf)
...
module Bcc.Wallet.Primitive.Model
    (
    -- * Type
      Wallet
...
import qualified Bcc.Wallet.Primitive.Types.TokenBundle as TB
...
prefilterBlock b u0 = runState $ do
  ...
  where
    ...
    applyTx (!txs, !prevUTxO) tx = do
        ...
        -- a Cole transaction. In which case fees can actually be calculated as
        ...
        let actualFee direction = case (tx ^. #fee, direction) of
                (Just x, Outgoing) -> -- Sophie and beyond.
                    Just x

                (Nothing, Outgoing) -> -- Cole
                    let
        ...
        return $ if hasKnownOutput && not hasKnownInput then
            ...
            let
                bccSpent = TB.getCoin spent
                bccReceived = TB.getCoin received
                dir = if bccSpent > bccReceived then Outgoing else Incoming
                amount = distance bccSpent bccReceived
            in
