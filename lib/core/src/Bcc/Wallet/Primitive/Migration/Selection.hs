...
--
module Bcc.Wallet.Primitive.Migration.Selection
    (
    -- * Types
      Selection (..)
    , SelectionError (..)
...
--
data SelectionError
    = SelectionBccInsufficient
    -- ^ Indicates that the desired selection would not have enough bcc to pay
...
--
-- Returns 'SelectionBccInsufficient' if the desired selection would not have
-- enough bcc to pay for the fee.
--
...
create constraints reward inputs =
    balance constraints $ Selection
        ...
        , inputIds = fst <$> inputs
        , outputs = assignMinimumBccQuantity constraints <$>
            F.foldl'
...
--
-- The bcc quantities of the outputs are maximized in order to minimize the fee
-- excess.
--
-- Pre-condition: outputs have minimal bcc quantities.
--
-- Guarantees the following property for a returned selection 's':
--
-- >>> verify s == SelectionCorrect
...
balance constraints unbalancedSelection = do
    ...
    unbalancedFee <- first (const SelectionBccInsufficient) $
    ...
    unbalancedFeeExcess <- maybeToEither SelectionBccInsufficient $
...
assignMinimumBccQuantity :: TxConstraints -> TokenMap -> TokenBundle
assignMinimumBccQuantity constraints m =
    TokenBundle c m
  where
    c = txOutputMinimumBccQuantity constraints m
...
addValueToOutputs constraints outputsOriginal outputUnchecked =
  ...
  where
    ...
    safeMerge a b
      ...
      where
        isSafe = (&&)
            (txOutputHasValidSizeIfBccMaximized constraints value)
...
--   possible bcc quantity.
--
-- Using this function to check all outputs provided to 'balance' will ensure
-- that it has complete freedom to adjust the bcc quantities of those outputs,
-- without exceeding the output size limit.
--
txOutputHasValidSizeIfBccMaximized :: TxConstraints -> TokenMap -> Bool
txOutputHasValidSizeIfBccMaximized constraints output =
...
-- | Minimizes the given fee excess by adding bcc to the given output bundles.
--
-- This function:
--
--  - guarantees to leave all non-bcc quantities unchanged.
--
--  - guarantees to not change the length of the list.
--
--  - guarantees that each resulting output bundle will have an bcc quantity
--    that is greater than or equal to its original bcc quantity.
--
--  - guarantees that the resulting fee excess will be less than or equal to
--    the original fee excess.
--
--  - does not check that the given bcc quantities are above the minimum
--    required for each output, and therefore only guarantees that the
--    resulting bcc quantities will be above the minimum required if the
...
-- ideal case, will increase the bcc quantity of just one output bundle.
--
-- Increasing the bcc quantity of an output may increase the overall cost of
-- that output, as increasing an bcc quantity may increase the length of the
-- binary representation used to encode that quantity.
--
-- By maximizing the bcc increase of a single output, and minimizing the bcc
...
-- | Minimizes the given fee excess by adding bcc to the given output.
--
-- This function:
--
--  - guarantees to leave all non-bcc quantities unchanged.
--
--  - increases the bcc quantity of the given output until it is no longer
...
--
--  - guarantees that the resulting output bundle will have an bcc quantity
--    that is greater than or equal to its original bcc quantity.
--
--  - guarantees that the resulting fee excess will be less than or equal to
--    the original fee excess.
...
computeCurrentFee Selection {inputBalance, outputs, rewardWithdrawal}
    | bccBalanceIn >= bccBalanceOut =
        Right bccDifference
    | otherwise =
        Left (NegativeCoin bccDifference)
  where
    bccBalanceIn =
    ...
    bccDifference =
        Coin.distance bccBalanceIn bccBalanceOut
...
data SelectionCorrectnessError
    ...
    | SelectionOutputBelowMinimumBccQuantity
      SelectionOutputBelowMinimumBccQuantityError
...
verify constraints selection =
  ...
  where
    ...
    verifyAll = do
        ...
        checkOutputMinimumBccQuantities constraints selection
            `failWith` SelectionOutputBelowMinimumBccQuantity
...
--------------------------------------------------------------------------------
-- Selection correctness: minimum bcc quantities
--------------------------------------------------------------------------------

data SelectionOutputBelowMinimumBccQuantityError =
    SelectionOutputBelowMinimumBccQuantityError
        { outputBundle :: TokenBundle
          -- ^ The output that is below the expected minimum bcc quantity.
        , expectedMinimumBccQuantity :: Coin
          -- ^ The expected minimum bcc quantity.
        }
    deriving (Eq, Show)

checkOutputMinimumBccQuantities
    :: TxConstraints
    -> Selection input
    -> Maybe SelectionOutputBelowMinimumBccQuantityError
checkOutputMinimumBccQuantities constraints selection =
  ...
  where
    ...
    checkOutput outputBundle
        | TokenBundle.getCoin outputBundle >= expectedMinimumBccQuantity =
            Nothing
        | otherwise =
            Just SelectionOutputBelowMinimumBccQuantityError
                { outputBundle
                , expectedMinimumBccQuantity
                }
      where
        expectedMinimumBccQuantity =
            txOutputMinimumBccQuantity constraints (view #tokens outputBundle)
