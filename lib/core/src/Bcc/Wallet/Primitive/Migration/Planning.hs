...
--
module Bcc.Wallet.Primitive.Migration.Planning
    (
    -- * Migration planning
      createPlan
    , MigrationPlan (..)
...
-- is not enough bcc to pay for a "freerider" entry.
--
    ...
    extendWithFreerider (!utxo, !selection) =
        case extendWith Freerider constraints (utxo, selection) of
            Right (utxo', selection') ->
                extendWithFreerider (utxo', selection')
            Left ExtendSelectionBccInsufficient ->
...
data ExtendSelectionError
    = ExtendSelectionBccInsufficient
