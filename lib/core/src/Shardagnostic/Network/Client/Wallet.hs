...
-- Cole and Sophie.
module Shardagnostic.Network.Client.Wallet
    (
      -- * ChainSyncFollowTip
      chainSyncFollowTip
...
import qualified Shardagnostic.Network.Protocol.ChainSync.ClientPipelined as P
...
chainSyncFollowTip toBccEra onTipUpdate =
  ...
  where
    ...
    clientStNext True = ClientStNext
            ...
            , recvMsgRollForward  = doUpdate . Just . toBccEra
            }
      where
        doUpdate
            :: Maybe era
