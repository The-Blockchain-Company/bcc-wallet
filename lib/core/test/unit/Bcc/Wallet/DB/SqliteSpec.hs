...
module Bcc.Wallet.DB.SqliteSpec
    ( spec
    ) where
...
import Bcc.Wallet.DB.Arbitrary
...
import Bcc.Wallet.DB.StateMachine
...
import qualified Bcc.Wallet.DB.Sqlite.TH as DB
import qualified Bcc.Wallet.Primitive.AddressDerivation.Sophie as Seq
...
stateMachineSpecSeq = stateMachineSpec @SophieKey @(SeqState 'Mainnet SophieKey)
stateMachineSpecRnd = stateMachineSpec @ColeKey @(RndState 'Mainnet)
...
propertiesSpecSeq = around withSophieDBLayer $ describe "Properties"
...
loggingSpec :: Spec
loggingSpec = withLoggingDB @(SeqState 'Mainnet SophieKey) $ do
...
withLoggingDB
    ...
    => SpecWith (IO [DBLog], DBLayer IO s SophieKey)
    -> Spec
...
type TestDBSeq = DBLayer IO (SeqState 'Mainnet SophieKey) SophieKey
...
fileModeSpec =  do
    describe "Check db opening/closing" $ do
        it "Opening and closing of db works" $ do
            replicateM_ 25 $ do
                db <- temporaryDBFile
                withSophieFileDBLayer @(SeqState 'Mainnet SophieKey) db
                    (\_ -> pure ())
    ...
    before temporaryDBFile $
        ...
        it "create and list wallet works" $ \f -> do
            withSophieFileDBLayer f $ \DBLayer{..} -> do
                atomically $ unsafeRunExceptT $
        ...
        it "create and get meta works" $ \f -> do
            meta <- withSophieFileDBLayer f $ \DBLayer{..} -> do
                now <- getCurrentTime
        ...
        it "create and get private key" $ \f -> do
            (k, h) <- withSophieFileDBLayer f $ \db@DBLayer{..} -> do
                atomically $ unsafeRunExceptT $
                    initializeWallet testWid testCp testMetadata mempty gp
        ...
        describe "Golden rollback scenarios" $ do
            ...
            it "Should remove collateral inputs from the UTxO set if \
                \validation fails" $
                \f -> withSophieFileDBLayer f $ \db@DBLayer{..} -> do
            ...
            it "(Regression test #1575) - TxMetas and checkpoints should \
              ...
              withSophieFileDBLayer f $ \db@DBLayer{..} -> do
    ...
    describe "random operation chunks property" $ do
        it "realize a random batch of operations upon one db open"
            (property $ prop_randomOpChunks @(SeqState 'Mainnet SophieKey))
...
prop_randomOpChunks (KeyValPairs pairs) =
  ...
  where
    prop = do
        ...
        withSophieFileDBLayer filepath $ \dbF -> do
            cleanDB dbF
            withSophieDBLayer $ \dbM -> do
                cleanDB dbM
...
testOpeningCleaning
    :: (Show s, Eq s)
    => FilePath
    -> (DBLayer IO (SeqState 'Mainnet SophieKey) SophieKey -> IO s)
    -> s
    -> s
    -> Expectation
testOpeningCleaning filepath call expectedAfterOpen expectedAfterClean = do
    withSophieFileDBLayer filepath $ \db -> do
...
withTestDBFile
    :: (DBLayer IO (SeqState 'Mainnet SophieKey) SophieKey -> IO ())
    -> (FilePath -> IO a)
    -> IO a
...
temporaryDBFile = emptySystemTempFile "bcc-wallet-SqliteFileMode"
...
withSophieDBLayer :: PersistState s => (DBLayer IO s SophieKey -> IO a) -> IO a
withSophieDBLayer = withDBLayerInMemory nullTracer dummyTimeInterpreter

withSophieFileDBLayer
    :: PersistState s
    => FilePath
    -> (DBLayer IO s SophieKey -> IO a)
    -> IO a
withSophieFileDBLayer fp = withDBLayer
...
attachPrivateKey
    :: DBLayer IO s SophieKey
    -> WalletId
    -> ExceptT ErrNoSuchWallet IO (SophieKey 'RootK XPrv, Hash "encryption")
...
manualMigrationsSpec = describe "Manual migrations" $ do
    ...
    it "'migrate' db with no 'derivation_prefix' for seq state (Sophie)" $
        testMigrationSeqStateDerivationPrefix @SophieKey
            "sophieDerivationPrefix-v2020-10-07.sqlite"
            ( purposeCIP1852
            , coinTypeBcc
            , minBound
            )
    ...
    it "'migrate' db with old text serialization for 'Role'" $
        testMigrationRole @SophieKey
            "sophieRole-v2020-10-13.sqlite"

    it "'migrate' db with partially applied checkpoint migration" $
        testMigrationRole @SophieKey
            "sophieRole-corrupted-v2020-10-13.sqlite"

    it "'migrate' db with unused protocol parameters in checkpoints" $
        testMigrationCleanupCheckpoints @SophieKey
            "sophieDerivationPrefix-v2020-10-07.sqlite"
            (GenesisParameters
    ...
    it "'migrate' db to add fees to transactions" $
        testMigrationTxMetaFee @SophieKey
...
testMigrationPassphraseScheme
    :: forall s k. (k ~ SophieKey, s ~ SeqState 'Mainnet k)
    => IO ()
...
testCp :: Wallet (SeqState 'Mainnet SophieKey)
testCp = snd $ initWallet block0 initDummyState
  where
    initDummyState :: SeqState 'Mainnet SophieKey
...
testCpSeq :: Wallet (SeqState 'Mainnet SophieKey)
testCpSeq = snd $ initWallet block0 initDummyStateSeq

initDummyStateSeq :: SeqState 'Mainnet SophieKey
