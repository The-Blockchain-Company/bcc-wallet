...
--   https://tbco.io/blog/an-in-depth-look-at-quickcheck-state-machine/
--
...
module Bcc.Wallet.DB.StateMachine
    ( prop_sequential
    , prop_parallel
    , validateGenerators
...
instance MockPrivKey (SophieKey 'RootK) where
...
instance MockPrivKey (ColeKey 'RootK) where
    fromMockPrivKey s = (k, Hash (B8.pack s))
      where (k, _) = unsafeDeserializeXPrv (zeroes <> ":", mempty)
...
instance ToExpr (SeqState 'Mainnet SophieKey) where
    toExpr = defaultExprViaShow
