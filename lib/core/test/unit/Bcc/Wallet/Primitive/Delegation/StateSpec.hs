...
module Bcc.Wallet.Primitive.Delegation.StateSpec where
...
import Bcc.Wallet.Primitive.Delegation.State
    ( Cert (..)
    , DelegationState (..)
    , Key0Status (..)
    , PointerUTxO (..)
...
--
-- FIXME: We should do /some/ testing with @SophieKey@ though.
...
data Cmd
    ...
    | CmdAdversarialReg RewardAccount
      -- ^ Someone could pay 2 bcc to (re-)register your stake key. Your wallet
