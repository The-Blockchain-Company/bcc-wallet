...
module Bcc.Wallet.Primitive.AddressDerivationSpec
    ( spec
    ) where
...
spec = do
    ...
    parallel $ describe "Keys storing and retrieving roundtrips" $ do
        it "XPrv SophieKey"
            (property $ prop_roundtripXPrv @SophieKey)
        it "XPrv IcarusKey"
            (property $ prop_roundtripXPrv @IcarusKey)
        it "XPrv ColeKey"
            (property $ prop_roundtripXPrv @ColeKey)
        it "XPub SophieKey"
            (property $ prop_roundtripXPub @SophieKey)
        it "XPub IcarusKey"
            (property $ prop_roundtripXPub @IcarusKey)

    parallel $ describe "golden test legacy passphrase encryption" $ do
        it "compare new implementation with bcc-sl - short password" $ do
        ...
        it "compare new implementation with bcc-sl - normal password" $ do
            ...
            checkPassphrase EncryptWithScrypt pwd hash `shouldBe` Right ()
        it "compare new implementation with bcc-sl - empty password" $ do
            ...
            checkPassphrase EncryptWithScrypt pwd hash `shouldBe` Right ()
        it "compare new implementation with bcc-sl - bcc-wallet password" $ do
            let pwd  = Passphrase @"raw" $ BA.convert $ T.encodeUtf8 "bcc-wallet"
            let hash = Hash $ unsafeFromHex
...
instance Arbitrary (SophieKey 'RootK XPrv) where
    shrink _ = []
    arbitrary = genRootKeysSeqWithPass =<< genPassphrase (0, 16)

instance Arbitrary (SophieKey 'AccountK XPub) where
    shrink _ = []
    arbitrary = publicKey <$> (genRootKeysSeqWithPass =<< genPassphrase (0, 16))

instance Arbitrary (SophieKey 'RootK XPub) where
    shrink _ = []
...
genRootKeysSeqWithPass
    :: Passphrase "encryption"
    -> Gen (SophieKey depth XPrv)
genRootKeysSeqWithPass encryptionPass = do
    s <- SomeMnemonic <$> genMnemonic @15
    g <- Just . SomeMnemonic <$> genMnemonic @12
    return $ Sophie.unsafeGenerateKeyFromSeed (s, g) encryptionPass

genRootKeysRndWithPass
    :: Passphrase "encryption"
    -> Gen (ColeKey 'RootK XPrv)
genRootKeysRndWithPass encryptionPass = Cole.generateKeyFromSeed
