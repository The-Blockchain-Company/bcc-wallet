...
module Bcc.Wallet.Primitive.CoinSelection.BalanceSpec
    ( spec
    ) where
...
import Bcc.Wallet.Primitive.CoinSelection.Gen
...
import Bcc.Wallet.Primitive.Types.UTxOSelection.Gen
...
spec :: Spec
spec = describe "Bcc.Wallet.Primitive.CoinSelection.BalanceSpec" $
...
prop_prepareOutputsWith_twice
    :: MockComputeMinimumBccQuantity
    -> [TxOut]
    -> Property
prop_prepareOutputsWith_twice minCoinValueDef outs =
    once === twice
  where
    minCoinValueFor = unMockComputeMinimumBccQuantity minCoinValueDef
  ...
  where
    ...
    onInsufficientMinCoinValues es = do
      ...
      where
        ...
        actualMinCoinValue
            = txOutCoin . outputWithInsufficientBcc
    ...
    onUnableToConstructChange e = do
        ...
        let constraints' = SelectionConstraints
                ...
                , computeMinimumBccQuantity = computeMinimumBccQuantityZero
...
prop_coinSelectionLens_givesPriorityToCoins (Blind (Small u)) =
    entryCount > 0 ==> monadicIO $ do
        hasCoin <- isJust <$> run (UTxOIndex.selectRandom u WithBccOnly)
...
-- has been selected.
--
-- See: https://The-Blockchain-Company.atlassian.net/browse/ADP-890
--
...
genMockSelectionConstraints = MockSelectionConstraints
    ...
    <*> genMockComputeMinimumBccQuantity
...
shrinkMockSelectionConstraints =
    shrinkMapBy tupleToMock mockToTuple $ liftShrink4
        ...
        shrinkMockComputeMinimumBccQuantity
...
unMockSelectionConstraints m = SelectionConstraints
    ...
    , computeMinimumBccQuantity =
        unMockComputeMinimumBccQuantity $ view #computeMinimumBccQuantity m
    , computeMinimumCost =
...
--------------------------------------------------------------------------------
-- Computing minimum bcc quantities
--------------------------------------------------------------------------------

data MockComputeMinimumBccQuantity
    = MockComputeMinimumBccQuantityZero
    | MockComputeMinimumBccQuantityLinear
    deriving (Eq, Show, Bounded, Enum)

genMockComputeMinimumBccQuantity :: Gen MockComputeMinimumBccQuantity
genMockComputeMinimumBccQuantity = arbitraryBoundedEnum

shrinkMockComputeMinimumBccQuantity
    :: MockComputeMinimumBccQuantity -> [MockComputeMinimumBccQuantity]
shrinkMockComputeMinimumBccQuantity = \case
    MockComputeMinimumBccQuantityZero ->
        []
    MockComputeMinimumBccQuantityLinear ->
        [MockComputeMinimumBccQuantityZero]

unMockComputeMinimumBccQuantity
    :: MockComputeMinimumBccQuantity -> (TokenMap -> Coin)
unMockComputeMinimumBccQuantity = \case
    MockComputeMinimumBccQuantityZero ->
        computeMinimumBccQuantityZero
    MockComputeMinimumBccQuantityLinear ->
        computeMinimumBccQuantityLinear

-- | Returns a constant minimum bcc quantity of zero.
--
computeMinimumBccQuantityZero :: TokenMap -> Coin
computeMinimumBccQuantityZero = const (Coin 0)

-- | A dummy function for calculating the minimum bcc quantity to pay for a
--   token map.
--
...
--
computeMinimumBccQuantityLinear :: TokenMap -> Coin
computeMinimumBccQuantityLinear m =
...
type MakeChangeData =
    MakeChangeCriteria MockComputeMinimumBccQuantity MockAssessTokenBundleSize
...
makeChangeWith p = makeChange p
    { minCoinFor = unMockComputeMinimumBccQuantity $ minCoinFor p
...
prop_makeChange_length p =
  ...
  where
    ...
    zeroCostMakeChangeScenario = p
        { minCoinFor = computeMinimumBccQuantityZero
        , requiredCost = Coin 0
        }
...
prop_makeChange_success_minValueRespected p =
  ...
  where
    ...
    minCoinValueFor = unMockComputeMinimumBccQuantity (minCoinFor p)
...
-- The 'makeChange' function will fail if there is not enough bcc to assign
-- to all the generated change outputs. Indeed, each output must include a
-- minimum quantity of bcc.
--
-- See also `prop_makeChange` as a top-level property driver.
...
prop_makeChange_fail_minValueTooBig p =
    let makeChangeData = p
            { requiredCost = Coin 0
            , minCoinFor = MockComputeMinimumBccQuantityZero
            }
    in
        ...
        Right change ->
          ...
          where
            ...
            deltaCoin = TokenBundle.getCoin $ TokenBundle.unsafeSubtract
                ...
                totalOutputValue
            minCoinValueFor =
                unMockComputeMinimumBccQuantity (minCoinFor p)
...
unit_makeChange =
  ...
  where
    bundleSizeAssessor =
        mkTokenBundleSizeAssessor MockAssessTokenBundleSizeUnlimited
    matrix =
        -- Simple, only bcc, should construct a single change output with 1 bcc.
        [ ( computeMinimumBccQuantityZero
          , Coin 0
          , Coin 0
          , b 2 [] :| []
...
unit_assignCoinsToChangeMaps =
  ...
  where
    matrix =
        -- Simple case with a single Bcc-only output
        [ ( Coin 1
          , computeMinimumBccQuantityLinear
          , m 42 [] :| []
          , Right [b 1 []]
        ...
        -- Single Bcc-only output, but not enough left to create a change
        , ( Coin 1
          , (`addCoin` Coin 1) . computeMinimumBccQuantityLinear
          , m 42 [] :| []
          , Right []
          )
        ...
        -- Multiple Bcc-only change, not enough Bcc left to create them all
        , ( Coin 2
          , computeMinimumBccQuantityLinear
          , NE.fromList
            [ m 1337 []
        ...
        -- Hybrid Bcc & MA, not enough to cover both => Bcc change is dropped
        , ( Coin 2
          , computeMinimumBccQuantityLinear
          , NE.fromList
            [ m 42 []
...
instance Arbitrary MockComputeMinimumBccQuantity where
    arbitrary = genMockComputeMinimumBccQuantity
    shrink = shrinkMockComputeMinimumBccQuantity
