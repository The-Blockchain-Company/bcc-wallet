...
module Bcc.Wallet.Primitive.Types.UTxOIndexSpec
    ( spec
    ) where
...
import qualified Bcc.Wallet.Primitive.Types.UTxOIndex.Internal as UTxOIndex
...
spec :: Spec
spec =
    describe "Bcc.Wallet.Primitive.Types.UTxOIndexSpec" $ do
    ...
    parallel $ describe "Index Selection" $ do
        ...
        it "prop_selectRandom_one_withBccOnly" $
            property prop_selectRandom_one_withBccOnly
        ...
        it "prop_selectRandom_all_withBccOnly" $
            property prop_selectRandom_all_withBccOnly
...
prop_SelectionFilter_coverage selectionFilter = checkCoverage $ property
    $ cover 20 (category == MatchAny)
        "Any"
    $ cover 20 (category == MatchWithBccOnly)
        "WithBccOnly"
...
-- | Attempt to select a random entry with only bcc.
--
prop_selectRandom_one_withBccOnly :: UTxOIndex -> Property
prop_selectRandom_one_withBccOnly u = checkCoverage $ monadicIO $ do
    result <- run $ UTxOIndex.selectRandom u WithBccOnly
    monitor $ cover 50 (isJust result)
        "selected an entry"
    case result of
        Nothing ->
            assert utxoHasNoBccOnlyEntries
        Just ((i, o), u') -> do
            assert $ UTxOIndex.delete i u == u'
            assert $ UTxOIndex.insert i o u' == u
            assert $ UTxOIndex.member i u
            ...
            assert $ u /= u'
  where
    utxoHasNoBccOnlyEntries =
        Map.null $ Map.filter txOutIsBccOnly $ unUTxO $ UTxOIndex.toUTxO u
...
-- | Attempt to select all entries with only bcc from the index.
--
prop_selectRandom_all_withBccOnly :: UTxOIndex -> Property
prop_selectRandom_all_withBccOnly u = checkCoverage $ monadicIO $ do
    (selectedEntries, u') <- run $ selectAll WithBccOnly u
    monitor $ cover 70 (not (null selectedEntries))
        "selected at least one entry"
    assert $ L.all (\(_, o) -> not (txOutIsBccOnly o)) (UTxOIndex.toList u')
    assert $ L.all (\(_, o) -> txOutIsBccOnly o) selectedEntries
...
-- | Returns 'True' if (and only if) the given transaction output has a non-zero
--   quantity of the given asset and no other non-bcc assets.
--
...
--   assets other than bcc.
--
txOutIsBccOnly :: TxOut -> Bool
txOutIsBccOnly = TokenBundle.isCoin . view #tokens
