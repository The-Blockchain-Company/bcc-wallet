...
module Bcc.Wallet.Primitive.Types.TokenBundleSpec
    ( spec
    ) where
...
spec =
    ...
    describe "Behaviour" $
        it "toCoin only returns when token bundle has only BCC" $
            property prop_toCoin_onlyBcc
...
prop_toCoin_onlyBcc :: TokenBundle -> Property
prop_toCoin_onlyBcc bundle =
    let
    ...
    in
        ...
        cover 30 (TokenBundle.isCoin bundle)
            "Token bundle has no non-bcc assets" $
