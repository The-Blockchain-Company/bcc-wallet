...
module Bcc.Wallet.Primitive.AddressDerivation.MintBurnSpec
    ( spec
    ) where
...
import Bcc.Wallet.Primitive.AddressDerivation.MintBurn
...
spec = do
    parallel $ describe "Mint/Burn Policy key Address Derivation Properties" $ do
        ...
        it "Deriving a policy key with bcc-address returns same result as bcc-wallet" $ do
            ...
            unit_comparePolicyKeys goldenTestMnemonic (Index 0x80000010)
                ...
                -- Child key 1855H/1815H/16H generated by bcc-address CLI
                -- from test mnemonic
            unit_comparePolicyKeyHashes goldenTestMnemonic (Index 0x80000010)
                ...
                -- bcc-address CLI from test mnemonic
...
prop_keyHashMatchesXPrv pwd masterkey policyIx =
    hashVerificationKey
      ...
      === keyHash
  where
    rndKey :: SophieKey 'PolicyK XPrv
    keyHash :: KeyHash
    (rndKey, keyHash) = derivePolicyKeyAndHash pwd masterkey policyIx

    getPublicKey
        :: SophieKey 'PolicyK XPrv
        -> SophieKey 'ScriptK XPub
    getPublicKey =
        publicKey . (liftRawKey :: XPrv -> SophieKey 'ScriptK XPrv) . getRawKey
...
prop_keyDerivationRelation pwd masterkey policyIx =
  ...
  where
    ...
    keyAndHash :: (SophieKey 'PolicyK XPrv, KeyHash)
...
unit_comparePolicyKeys mnemonic index goldenPolicyKeyBech32 =
    let
        walletRootKey :: XPrv
        walletRootKey =
            Sophie.generateKeyFromSeed (SomeMnemonic mnemonic, Nothing) mempty
            & getRawKey
...
unit_comparePolicyKeyHashes mnemonic index goldenPolicyKeyHashHex =
    let
        ...
        walletPolicyData :: (SophieKey 'PolicyK XPrv, KeyHash)
        walletPolicyData =
