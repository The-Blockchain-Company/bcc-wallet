...
module Bcc.Wallet.Primitive.Migration.SelectionSpec
    where
...
import qualified Bcc.Wallet.Primitive.Migration.Selection as Selection
...
spec :: Spec
spec = describe "Bcc.Wallet.Primitive.Migration.SelectionSpec" $
...
prop_create_inner mockConstraints inputs reward =
    ...
    cover 1 (resultHasInsufficientBcc result)
        "Failure due to insufficient bcc" $
    cover 1 (resultIsFull result)
        "Failure due to oversized selection" $
    report mockConstraints
        "mockConstraints" $
    case result of
        Left SelectionBccInsufficient ->
            property True
...
resultHasInsufficientBcc :: MockSelectionResult -> Bool
resultHasInsufficientBcc = matchLeft $ \case
    SelectionBccInsufficient -> True
    _ -> False
    ...
    genExtraInput = (,)
        ...
        <*> oneof
            [ genTokenBundleMixed mockConstraints
              ...
              -- deliberately include some large bundles whose bcc
...
prop_extend_inner mockConstraints selectionOriginal input =
    ...
    cover 10 (resultHasZeroFeeExcess result)
        "Success with zero fee excess" $
    cover 0.1 (resultHasInsufficientBcc result)
        "Failure due to insufficient bcc" $
  ...
  where
    makeTests
        = verify
            ...
            "Value is preserved"
        . verify
            (all (txOutputHasValidSizeWithMaxBcc constraints) result)
            "All outputs have valid sizes (if bcc maximized)"
        . verify
...
txOutputHasValidSizeWithMaxBcc :: TxConstraints -> TokenMap -> Bool
txOutputHasValidSizeWithMaxBcc constraints b =
...
prop_minimizeFee_inner mockConstraints feeExcessBefore outputsBefore =
  ...
  where
    ...
    feeExcessReductionExpected =
        totalOutputCostIncrease <> totalOutputBccIncrease

    totalOutputBccAfter =
        F.foldMap (view #coin) outputsAfter
    totalOutputBccBefore =
        F.foldMap (view #coin) outputsBefore
    totalOutputBccIncrease =
        Coin.distance totalOutputBccAfter totalOutputBccBefore
  ...
  where
    makeTests
        ...
        . verify
            ...
            "all coins cost less than the maximum bcc quantity"
        . verify
  ...
  where
    makeTests
        ...
        . verify
            ...
            "all coins have a smaller size than the maximum bcc quantity"
        . verify
...
data MockTxConstraints = MockTxConstraints
    ...
    , mockTxOutputMaximumTokenQuantity
        :: MockTxOutputMaximumTokenQuantity
    , mockTxOutputMinimumBccQuantity
        :: MockTxOutputMinimumBccQuantity
    , mockTxMaximumSize
        :: MockTxMaximumSize
    }
...
genMockTxConstraints = do
    ...
    mockTxOutputMinimumBccQuantity <- genMockTxOutputMinimumBccQuantity
...
unMockTxConstraints MockTxConstraints {..} = TxConstraints
    ...
    , txOutputMinimumBccQuantity =
        unMockTxOutputMinimumBccQuantity mockTxOutputMinimumBccQuantity
...
--------------------------------------------------------------------------------
-- Mock minimum bcc quantities
--------------------------------------------------------------------------------

data MockTxOutputMinimumBccQuantity = MockTxOutputMinimumBccQuantity
    { perOutput :: Coin
    , perOutputAsset :: Coin
    }
    deriving (Eq, Show)

unMockTxOutputMinimumBccQuantity
    :: MockTxOutputMinimumBccQuantity
    -> (TokenMap -> Coin)
unMockTxOutputMinimumBccQuantity mock m =
    let assetCount = TokenMap.size m in
    perOutput mock
        <> mtimesDefault assetCount (perOutputAsset mock)

genMockTxOutputMinimumBccQuantity :: Gen MockTxOutputMinimumBccQuantity
genMockTxOutputMinimumBccQuantity = MockTxOutputMinimumBccQuantity
...
genCoinAboveMinimumBccQuantity :: MockTxConstraints -> Gen Coin
genCoinAboveMinimumBccQuantity mockConstraints =
    genCoinRange
...
genCoinBelowMinimumBccQuantity :: MockTxConstraints -> Gen Coin
genCoinBelowMinimumBccQuantity mockConstraints =
    genCoinRange
        (Coin 1)
...
genTokenBundleMixed mockConstraints =
  ...
  where
    ...
    distribution =
        [ (10, genCoinBelowMinimumBccQuantity <&> fmap TokenBundle.fromCoin)
        , (40, genCoinAboveMinimumBccQuantity <&> fmap TokenBundle.fromCoin)
        , (40, genTokenBundleWithMinimumBccQuantity)
        , (10, genTokenBundleAboveMinimumBccQuantity)
        ]

genTokenBundleWithMinimumBccQuantity :: MockTxConstraints -> Gen TokenBundle
genTokenBundleWithMinimumBccQuantity mockConstraints = do
    m <- genTokenMap mockConstraints
    let minBcc = txOutputMinimumBccQuantity constraints m
    pure $ TokenBundle minBcc m
  where
    constraints = unMockTxConstraints mockConstraints

genTokenBundleAboveMinimumBccQuantity :: MockTxConstraints -> Gen TokenBundle
genTokenBundleAboveMinimumBccQuantity mockConstraints = do
    m <- genTokenMap mockConstraints
    let minBcc = txOutputMinimumBccQuantity constraints m
    c <- genCoinRange (minBcc <> Coin 1) (minBcc `scaleCoin` 1000)
