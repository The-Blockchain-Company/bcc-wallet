...
module Bcc.Wallet.Primitive.Migration.PlanningSpec
    where

import Prelude

import Bcc.Wallet.Primitive.Migration.Planning
    ( CategorizedUTxO (..)
    , MigrationPlan (..)
    , UTxOEntryCategory (..)
...
spec :: Spec
spec = describe "Bcc.Wallet.Primitive.Migration.PlanningSpec" $
...
prop_createPlan_inner mockConstraints inputs reward =
  ...
  where
    makeTests
        ...
        . verify
            (totalInputBcc >= totalOutputBcc)
            "bcc is consumed and not created"
        . verify
            (totalInputTokenBalance == totalOutputTokenBalance)
            "balance of non-bcc tokens is preserved"
        . verify
    ...
    makeReports
        ...
        . report totalInputBcc
            "total input bcc"
        . report totalOutputBcc
            "total output bcc"
    ...
    totalFeeExpected
        | not (null (selections result)) =
            Coin.distance totalInputBcc totalOutputBcc
        | otherwise =
            Coin 0

    totalInputBcc :: Coin
    totalInputBcc = mconcat
