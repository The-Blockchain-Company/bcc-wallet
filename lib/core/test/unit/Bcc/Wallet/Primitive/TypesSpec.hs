...
module Bcc.Wallet.Primitive.TypesSpec
    ( spec
    ) where
...
import Bcc.Wallet.Primitive.Slotting.Legacy
    ( SlotParameters (..)
    , epochStartTime
    , flatSlot
    , fromFlatSlot
...
import Bcc.Wallet.Primitive.Types.Hash
    ( Hash (..) )
import Bcc.Wallet.Primitive.Types.HashSpec
    ()
...
spec :: Spec
spec = describe "Bcc.Wallet.Primitive.Types" $ do
    ...
    parallel $ describe "Buildable" $ do
        it "WalletId" $ do
            ...
            let xprv = generateKeyFromSeed
                    (mw, Nothing) mempty :: SophieKey 'RootK XPrv
        ...
        it "UTxOStatistics" $ do
            ...
            let bcc l = l * 1000*1000
            let utxo = UTxO $ Map.fromList
                    [ (txin "a", txout 1)
                    , (txin "b", txout 4)
                    , (txin "c", txout $ bcc 44)
                    , (txin "d", txout $ bcc 17)
                    , (txin "e", txout $ bcc 2000)
                    , (txin "f", txout $ bcc 9000)
                    , (txin "g", txout $ bcc 3000)
                    ]
            let stats = computeUtxoStatistics log10 utxo
            pretty stats `shouldBe` mconcat @String
                [ "= Total value of 14061000005 entropic across 7 UTxOs\n"
                , ""
                , " ... 10                2\n"
                , " ... 100               0\n"
                , " ... 1000              0\n"
