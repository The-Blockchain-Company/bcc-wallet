...
module Bcc.Wallet.Primitive.CollateralSpec where

import Prelude

import Bcc.Wallet.Primitive.Collateral
    ( AddressType (..)
    , Credential (..)
...
import qualified Bcc.Ledger.Address as L
import qualified Bcc.Ledger.Credential as L
import qualified Bcc.Ledger.Crypto as CC
import qualified Bcc.Ledger.Hashes as L
...
import qualified Test.Sophie.Spec.Ledger.Serialisation.EraIndepGenerators as L
import qualified Test.Sophie.Spec.Ledger.Serialisation.Generators.Genesis as L
    ( genRewardAcnt )
...
-- | Test that for any Cole address, we classify it as a Cole (a.k.a.
-- bootstrap) address.
prop_addressType_cole :: Property
prop_addressType_cole =
    forAll genColeAddr $ \coleAddr -> do
        let (Address addrBytes) = asAddress coleAddr
...
-- | Test that for any sophie keyhash address, we classify it as a sophie
...
prop_addressType_sophieKeyHash :: Property
prop_addressType_sophieKeyHash =
    forAll genSophieKeyHashAddr $ \sophieKeyHashAddr -> do
        let
            (Address addrBytes) = asAddress sophieKeyHashAddr
            addrType = B.runGet getAddressType (BL.fromStrict addrBytes)
        coverTable "Address types"
...
-- | Test that for any sophie scripthash address, we classify it as a sophie
-- scripthash address (although not necessarily the correct one, as it's a bit
-- difficult to assert that the exact type is correct, we do at least test that
-- each type is chosen sometimes using the coverage check).
prop_addressType_sophieScriptHash :: Property
prop_addressType_sophieScriptHash =
    forAll genSophieScriptHashAddr $ \sophieScriptHashAddr -> do
        let
            (Address addrBytes) = asAddress sophieScriptHashAddr
            addrType = B.runGet getAddressType (BL.fromStrict addrBytes)
        coverTable "Address types"
...
-- generated with "bcc-addresses":

unit_addressType_coleGolden :: Expectation
unit_addressType_coleGolden =
    B.runGet getAddressType coleAddrGolden `shouldBe` BootstrapAddress

unit_addressType_sophieEnterprisePaymentGolden :: Expectation
unit_addressType_sophieEnterprisePaymentGolden =
    B.runGet getAddressType sophieEnterprisePaymentAddrGolden
...
-- every type of address is evenly covered (i.e. "genColeAddr" only generates
-- one type of address so is given lower frequency than "genSophieAddr", which
...
genAnyAddress = frequency
    [ (10, asAddress <$> genSophieAddr)
    , (1, asAddress <$> genColeAddr)
...
-- | Attempt to simplify an address. Only sophie addresses can be simplified
...
unit_addressSuitableForCollateral_coleGolden :: Expectation
unit_addressSuitableForCollateral_coleGolden =
    let
        addr = Address . BL.toStrict $ coleAddrGolden
    in
        addressSuitableForCollateral addr `shouldBe` True

unit_addressSuitableForCollateral_sophieEnterprisePaymentGolden :: Expectation
unit_addressSuitableForCollateral_sophieEnterprisePaymentGolden =
    let
        addr = Address . BL.toStrict $ sophieEnterprisePaymentAddrGolden
    in
        addressSuitableForCollateral addr `shouldBe` True
...
spec = do
    parallel $ do
        describe "address types" $ do
            ...
            it "classifies cole address type correctly" $
                property prop_addressType_cole
            it "classifies stake address type correctly" $
                property prop_addressType_stake
            it "classifies sophie key hash type correctly" $
                property prop_addressType_sophieKeyHash
            it "classifies sophie script hash type correctly" $
                property prop_addressType_sophieScriptHash
            it "golden" $ do
                unit_addressType_coleGolden
                unit_addressType_sophieEnterprisePaymentGolden
                unit_addressType_stakeAddrGolden
...
-- cat recovery-phrase.txt | bcc-address key from-recovery-phrase Cole > root.prv
-- cat root.prv | bcc-address key child 14H/42H | tee addr.prv | bcc-address key public --with-chain-code | bcc-address address bootstrap --root $(cat root.prv | bcc-address key public --with-chain-code) --network-tag testnet 14H/42H
coleAddrGolden :: BL.ByteString
coleAddrGolden = BL.fromStrict . fromJust . decodeBase58 bitcoinAlphabet $ 
...
-- cat root.prv | bcc-address key child 1852H/1815H/0H/2/0 > stake.prv
-- cat stake.prv | bcc-address key public --with-chain-code | bcc-address address stake --network-tag testnet
...
-- cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie > root.prv
-- cat root.prv | bcc-address key child 1852H/1815H/0H/0/0 > addr.prv
-- cat addr.prv | bcc-address key public --with-chain-code | bcc-address address payment --network-tag 0 | bcc-address address pointer 42 14 0
...
-- cat root.prv | bcc-address key child 1852H/1815H/0H/2/0 > stake.prv
-- cat root.prv | bcc-address key child 1852H/1815H/0H/0/0 > addr.prv
-- cat addr.prv | bcc-address key public --with-chain-code | bcc-address address payment --network-tag testnet | bcc-address address delegation $(cat stake.prv | bcc-address key public --with-chain-code)
...
-- cat recovery-phrase.txt | bcc-address key from-recovery-phrase Sophie > root.prv
-- cat root.prv | bcc-address key child 1852H/1815H/0H/0/0 > addr.prv
-- cat addr.prv | bcc-address key public --with-chain-code | bcc-address address payment --network-tag testnet
sophieEnterprisePaymentAddrGolden :: BL.ByteString
sophieEnterprisePaymentAddrGolden = unsafeBech32Decode
...
-- "Test.Sophie.Spec.Ledger.Serialisation.EraIndepGenerators". Do not try to
...
genSophieAddr :: Gen (L.Addr CC.StandardCrypto)
genSophieAddr =
    L.Addr <$> arbitrary <*> arbitrary <*> arbitrary

genSophieScriptHashAddr :: Gen (L.Addr CC.StandardCrypto)
genSophieScriptHashAddr = L.Addr
    <$> arbitrary
    <*> (L.ScriptHashObj . L.ScriptHash <$> L.genHash)
    <*> arbitrary

genSophieKeyHashAddr :: Gen (L.Addr CC.StandardCrypto)
genSophieKeyHashAddr = L.Addr
    <$> arbitrary
    <*> (L.KeyHashObj <$> arbitrary)
    <*> arbitrary

genColeAddr :: Gen (L.Addr CC.StandardCrypto)
genColeAddr =
    L.AddrBootstrap . L.BootstrapAddress <$> hedgehog Cole.genAddress
