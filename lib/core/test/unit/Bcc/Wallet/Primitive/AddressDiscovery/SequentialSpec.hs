...
module Bcc.Wallet.Primitive.AddressDiscovery.SequentialSpec
    ( spec
    ) where
...
spec = do
    ...
    parallel $ describe "AddressPool (Sophie)" $ do
        forM_ styles $ \s@(Style proxyS) -> do
            parallel $ describe ("SophieKey " <> show s) $ do
                it "'lookupAddressPool' extends the pool by a maximum of 'gap'"
                    (checkCoverage (prop_poolGrowWithinGap @_ @SophieKey proxyS))
                it "'addresses' preserves the address order"
                    (checkCoverage (prop_roundtripMkAddressPool @_ @SophieKey proxyS))
                it "An AddressPool always contains at least 'gap pool' addresses"
                    (property (prop_poolAtLeastGapAddresses @_ @SophieKey proxyS))
                it "Our addresses are eventually discovered"
                    (property (prop_poolEventuallyDiscoverOurs @_ @SophieKey proxyS))
...
prop_genChangeGapFromRootXPrv g =
  ...
  where
    ...
    key = Sophie.unsafeGenerateKeyFromSeed (mw, Nothing) mempty
...
prop_genChangeGapFromAccountXPub g =
  ...
  where
    mw = someDummyMnemonic (Proxy @12)
    rootXPrv = Sophie.unsafeGenerateKeyFromSeed (mw, Nothing) mempty
...
prop_changeNoLock
    :: (SeqState 'Mainnet SophieKey, Int)
    -> Property
...
prop_lookupDiscovered
    :: (SeqState 'Mainnet SophieKey, Address)
    -> Property
...
prop_compareKnownUnknown
    :: (SeqState 'Mainnet SophieKey, ShowFmt Address, ShowFmt Address)
    -> Property
...
prop_changeIsOnlyKnownAfterGeneration
    :: ( AddressPool 'UtxoInternal SophieKey
       , AddressPool 'UtxoExternal SophieKey
       )
    -> Property
prop_changeIsOnlyKnownAfterGeneration (intPool, extPool) =
    let
        s0 :: SeqState 'Mainnet SophieKey
...
instance AddressPoolTest SophieKey where
    ourAccount = publicKey $
        Sophie.unsafeGenerateKeyFromSeed (mw, Nothing) mempty
      where
...
changeAddresses
    :: [Address]
    -> SeqState 'Mainnet SophieKey
    -> ([Address], SeqState 'Mainnet SophieKey)
...
instance Arbitrary Address where
    shrink _ = []
    arbitrary = frequency
        [ (8, elements $ take 25 (ourAddresses (Proxy @SophieKey) UtxoExternal))
        , (8, elements $ take 25 (ourAddresses (Proxy @SophieKey) UtxoInternal))
        , (8, elements $ take 25 (ourAddresses (Proxy @SophieKey) MutableAccount))
...
instance Arbitrary (SeqState 'Mainnet SophieKey) where
