...
module Bcc.Wallet.Primitive.AddressDiscovery.RandomSpec
    ( spec
    ) where
...
goldenSpecMainnet =
    parallel $ describe "Golden tests for Cole Addresses w/ random scheme (Mainnet)" $ do
...
goldenSpecTestnet =
    parallel $ describe "Golden tests forCole Addresses w/ random scheme (Testnet)" $ do
...
rndStateFromMnem :: SomeMnemonic -> (ColeKey 'RootK XPrv, RndState 'Mainnet)
