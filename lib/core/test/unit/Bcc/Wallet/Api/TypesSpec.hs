...
module Bcc.Wallet.Api.TypesSpec (spec) where
...
spec = parallel $ do
    ...
    describe "JSON golden roundtrip" $ do
            ...
            jsonRoundtripAndGolden $ Proxy @ApiColeWallet
            jsonRoundtripAndGolden $ Proxy @ApiColeWalletBalance
            ...
            jsonRoundtripAndGolden $ Proxy @ApiVerificationKeySophie
            ...
            jsonRoundtripAndGolden $ Proxy @SomeColeWalletPostData
            jsonRoundtripAndGolden $ Proxy @ColeWalletFromXPrvPostData
            ...
            jsonRoundtripAndGolden $ Proxy @ColeWalletPutPassphraseData
    ...
    describe "verify JSON parsing failures too" $ do
        ...
        it "AddressAmount (too small)" $ do
            ...
            Aeson.parseEither parseJSON [aesonQQ|
                { "address": "<addr>"
                , "amount": {"unit":"entropic","quantity":-14}
                }
    ...
    parallel $ describe "pointless tests to trigger coverage for record accessors" $ do
        ...
        it "ApiWallet" $ property $ \x ->
            ...
            in
                x' === x .&&. show x' === show x
        it "ApiColeWallet" $ property $ \x ->
            let
                x' = ApiColeWallet
                    { id = id (x :: ApiColeWallet)
                    , balance = balance (x :: ApiColeWallet)
                    , assets = assets (x :: ApiColeWallet)
                    , name = name (x :: ApiColeWallet)
                    , passphrase = passphrase (x :: ApiColeWallet)
                    , state = state (x :: ApiColeWallet)
                    , tip = tip (x :: ApiColeWallet)
                    , discovery = discovery (x :: ApiColeWallet)
                    }
            in
                x' === x .&&. show x' === show x
        ...
        it "WalletPutPassphraseData" $ property $ \x ->
            ...
            in
                x' === x .&&. show x' === show x
        it "ColeWalletPutPassphraseData" $ property $ \x ->
            let
                x' = ColeWalletPutPassphraseData
                    { oldPassphrase = oldPassphrase (x :: ColeWalletPutPassphraseData)
                    , newPassphrase = newPassphrase (x :: ColeWalletPutPassphraseData)
                    }
            in
                x' === x .&&. show x' === show x
...
instance Arbitrary (Quantity "entropic" Natural) where
    shrink (Quantity 0) = []
    shrink _ = [Quantity 0]
...
instance Arbitrary ApiColeWallet where
    arbitrary = genericArbitrary
    shrink = genericShrink
...
instance Arbitrary ApiColeWalletBalance where
    arbitrary = genericArbitrary
    shrink = genericShrink
...
instance Arbitrary ColeWalletFromXPrvPostData where
    arbitrary = do
        n <- arbitrary
        rootXPrv <- ApiT . unsafeXPrv . BS.pack <$> vector 128
        bytesNumber <- choose (64,100)
        h <- ApiT . Hash . B8.pack <$> replicateM bytesNumber arbitrary
        pure $ ColeWalletFromXPrvPostData n rootXPrv h

instance Arbitrary SomeColeWalletPostData where
    arbitrary = genericArbitrary
    shrink = genericShrink

instance Arbitrary (ColeWalletPostData '[12]) where
    arbitrary = genericArbitrary
    shrink = genericShrink

instance Arbitrary (ColeWalletPostData '[15]) where
    arbitrary = genericArbitrary
    shrink = genericShrink

instance Arbitrary (ColeWalletPostData '[12,15,18,21,24]) where
    arbitrary = genericArbitrary
    shrink = genericShrink
...
instance Arbitrary ColeWalletPutPassphraseData where
    arbitrary = genericArbitrary
    shrink = genericShrink
...
instance Arbitrary ApiVerificationKeySophie where
    arbitrary = do
        hashing <- elements [WithHashing, WithoutHashing]
        ApiVerificationKeySophie <$> genKeyRole (len hashing) <*> pure hashing
      where
        genKeyRole n = (,)
...
instance ToSchema ApiVerificationKeySophie where
    declareNamedSchema _ = declareSchemaForDefinition "ApiVerificationKeySophie"
...
instance Arbitrary ApiWalletUtxoSnapshot where
    arbitrary = do
      ...
      where
        ...
        genEntry = do
            bccValue1 <- genCoinPositive
            bccValue2 <- genCoinPositive
            -- The actual bcc quantity of an output's token bundle must be
            -- greater than or equal to the minimum permissible bcc quantity:
            let bcc = Api.coinToQuantity $ max bccValue1 bccValue2
            let bccMinimum = Api.coinToQuantity $ min bccValue1 bccValue2
...
instance Arbitrary ApiAddressInspect where
    arbitrary = do
        style <- elements [ "Cole", "Icarus", "Sophie" ]
...
instance ToSchema ApiColeWallet where
    declareNamedSchema _ = declareSchemaForDefinition "ApiColeWallet"
...
instance Typeable n => ToSchema (ApiWalletMigrationPostData n "lenient") where
    declareNamedSchema _ =
        declareSchemaForDefinition "ApiColeWalletMigrationPostData"

instance Typeable n => ToSchema (ApiWalletMigrationPostData n "raw") where
    declareNamedSchema _ =
        declareSchemaForDefinition "ApiSophieWalletMigrationPostData"
...
instance ToSchema (ColeWalletPostData '[12]) where
    declareNamedSchema _ = declareSchemaForDefinition "ApiColeWalletRandomPostData"

instance ToSchema (ColeWalletPostData '[15]) where
    declareNamedSchema _ = declareSchemaForDefinition "ApiColeWalletIcarusPostData"

instance ToSchema (ColeWalletPostData '[12,15,18,21,24]) where
    -- NOTE ApiColeWalletLedgerPostData works too. Only the description differs.
    declareNamedSchema _ = declareSchemaForDefinition "ApiColeWalletTrezorPostData"

instance ToSchema ColeWalletFromXPrvPostData where
    declareNamedSchema _ = declareSchemaForDefinition "ApiColeWalletRandomXPrvPostData"

instance ToSchema SomeColeWalletPostData where
    declareNamedSchema _ = declareSchemaForDefinition "SomeColeWalletPostData"
...
instance ToSchema ColeWalletPutPassphraseData where
    declareNamedSchema _ =
        declareSchemaForDefinition "ApiColeWalletPutPassphraseData"
