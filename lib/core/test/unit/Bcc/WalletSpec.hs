...
import Bcc.Wallet.Primitive.CoinSelection
    ( SelectionError (..) )
import Bcc.Wallet.Primitive.Migration.SelectionSpec
...
import Bcc.Wallet.Primitive.Types.Address
    ( Address (..) )
import Bcc.Wallet.Primitive.Types.Address.Gen
    ( genAddress )
...
spec :: Spec
spec = parallel $ describe "Bcc.WalletSpec" $ do
...
walletUpdatePassphrase
    :: (WalletId, WalletName, DummyState)
    -> Passphrase "raw"
    -> Maybe (SophieKey 'RootK XPrv, Passphrase "encryption")
    -> Property
...
data TxRetryTestCtx = TxRetryTestCtx
    { ctxDbLayer :: DBLayer TxRetryTestM DummyState SophieKey
...
prop_localTxSubmission tc = monadicIO $ do
    ...
    res <- run $ runTest st $ \ctx@(TxRetryTestCtx DBLayer{..} _ _ wid) -> do
        ...
        runLocalTxSubmissionPool @_ @DummyState @SophieKey cfg ctx wid
...
data WalletLayerFixture m = WalletLayerFixture
    { _fixtureDBLayer :: DBLayer m DummyState SophieKey
    , _fixtureWalletLayer :: WalletLayer m DummyState SophieKey
...
dummyTransactionLayer :: TransactionLayer SophieKey SealedTx
...
instance IsOwned DummyState SophieKey where
...
instance {-# OVERLAPS #-} Arbitrary (SophieKey 'RootK XPrv, Passphrase "encryption")
  where
    shrink _ = []
    arbitrary = do
        pwd <- arbitrary
