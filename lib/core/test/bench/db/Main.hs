...
import Bcc.Wallet.DB.Sqlite.TH
    ( migrateAll )
...
import Bcc.Wallet.Primitive.AddressDerivation.Sophie
    ( SophieKey (..), generateKeyFromSeed, unsafeGenerateKeyFromSeed )
...
bgroupWriteUTxO db = bgroup "UTxO (Write)"
  ...
  where
    bUTxO n s a = bench lbl $ withCleanDB db walletFixture $
      ...
      where lbl | a == 0    = n|+" CP (bcc-only) x "+|s|+" UTxO"
...
mkExtPool :: Int -> Int -> AddressPool  'UtxoExternal SophieKey
...
mkIntPool :: Int -> Int -> AddressPool 'UtxoInternal SophieKey
...
bgroupWriteRndState :: DBLayerBenchCole -> Benchmark
...
mkRndAddresses numAddrs i =
    Map.fromList addrs
  where
    addrs = [ force ((toEnum i, toEnum j), mkColeAddress i j) | j <- [1..numAddrs] ]
...
--
-- In Cole, the transaction max size is currently of 8kb.
--
...
--
-- This means a transaction in Cole can't have more than (worst case):
--
-- - ~45 inputs
-- - ~155 outputs
--
...
--
-- This means a transaction in Quibitous can't have more than (worst case):
--
-- - ~78 inputs
-- - ~200 outputs
--
...
walletFixtureCole :: DBLayerBenchCole -> IO ()
walletFixtureCole db@DBLayer{..} = do
    cleanDB db
    atomically $ unsafeRunExceptT $ initializeWallet
        testWid
        testCpCole
...
type DBLayerBench = DBLayer IO (SeqState 'Mainnet SophieKey) SophieKey
type DBLayerBenchCole = DBLayer IO (RndState 'Mainnet) ColeKey
type WalletBench = Wallet (SeqState 'Mainnet SophieKey)
type WalletBenchCole = Wallet (RndState 'Mainnet)
...
testCpCole :: WalletBenchCole
testCpCole = snd $ initWallet block0 initDummyRndState

{-# NOINLINE initDummySeqState #-}
initDummySeqState :: SeqState 'Mainnet SophieKey
initDummySeqState =
...
initDummyRndState =
    mkRndState rootK 42
  where
    rootK = Cole.generateKeyFromSeed mnemonic mempty
    mnemonic = unsafePerformIO $
...
mkAddress i j =
    delegationAddress @'Mainnet
        (SophieKey $ unsafeXPub $ B8.pack $ take 64 $ randoms $ mkStdGen seed)
        rewardAccount
  where
