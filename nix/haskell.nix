...
# Enable Haskell Program Coverage for bcc-wallet libraries and test suites.
...
let
  ...
  projectPackages = {
    bcc-wallet-cli = "lib/cli";
    bcc-wallet-core-integration = "lib/core-integration";
    bcc-wallet-core = "lib/core";
    bcc-wallet-launcher = "lib/launcher";
    bcc-numeric = "lib/numeric";
    bcc-wallet = "lib/sophie";
    strict-non-empty-containers = "lib/strict-non-empty-containers";
    bcc-wallet-test-utils = "lib/test-utils";
  ...
  pkg-set = haskell.mkStackPkgSet {
    ...
    modules = [
      ...
      # Provide configuration and dependencies to bcc-wallet components
      ({ config, ...}: let
        bccNodeExes = with config.hsPkgs;
          [ bcc-node.components.exes.bcc-node
            bcc-cli.components.exes.bcc-cli ];
      in {
        packages.bcc-wallet-core.components.tests = {
        ...
        packages.bcc-wallet.components.tests = {
          ...
          # because shardagnostic-network doesn't fully work under Wine.
          integration.testWrapper = lib.mkIf pkgs.stdenv.hostPlatform.isWindows ["echo"];

          unit.preCheck = noCacheOnBorsCookie +
            lib.optionalString stdenv.isDarwin ''
              # bcc-node socket path becomes too long otherwise
              export TMPDIR=/tmp
            '';
          ...
          integration.preCheck = noCacheCookie + ''
            ...
            export BCC_NODE_TRACING_MIN_SEVERITY=notice
          ...
          integration.postCheck = ''
            ...
            if [ -n "$TESTS_LOGDIR" && -f $out/nix-support/failed ]; then
              logfile=$out/bcc-wallet-integration-logs.tar.gz
          ...
          # provide bcc-node & bcc-cli to tests
          unit.build-tools = bccNodeExes;
          integration.build-tools = bccNodeExes;
        ...
        packages.bcc-wallet.components.benchmarks.latency =
          lib.optionalAttrs (!stdenv.hostPlatform.isWindows) {
            build-tools = [ pkgs.buildPackages.makeWrapper ];
            postInstall = ''
              wrapProgram $out/bin/* \
                --run "cd $src/lib/sophie" \
                --prefix PATH : ${lib.makeBinPath bccNodeExes}
            '';
          };

        # Add bcc-node to the PATH of the byroon restore benchmark.
        # bcc-node will want to write logs to a subdirectory of the working directory.
        # We don't `cd $src` because of that.
        packages.bcc-wallet.components.benchmarks.restore =
          lib.optionalAttrs (!stdenv.hostPlatform.isWindows) {
            build-tools = [ pkgs.buildPackages.makeWrapper ];
            postInstall = ''
              wrapProgram $out/bin/restore \
                --set BCC_NODE_CONFIGS ${pkgs.bcc-node-deployments} \
                --prefix PATH : ${lib.makeBinPath bccNodeExes}
            '';
          };


        packages.bcc-wallet.components.exes.local-cluster = let
          testData = src + /lib/sophie/test/data/bcc-node-sophie;
        in
        ...
        packages.bcc-wallet.components.exes.bcc-wallet.postInstall = optparseCompletionPostInstall + libSodiumPostInstall;
        packages.bcc-wallet-core.components.tests.unit.postInstall = libSodiumPostInstall;
        packages.bcc-wallet-cli.components.tests.unit.postInstall = libSodiumPostInstall;
        packages.bcc-wallet-launcher.components.tests.unit.postInstall = libSodiumPostInstall;
      })

      ({ config, ...}: let
        setGitRevPostInstall = ''
          ${buildPackages.tbco-nix-utils}/bin/set-git-rev "${config.packages.bcc-node.src.rev}" $out/bin/* || true
        '';
      in {
        # Add shell completions for tools.
        packages.bcc-cli.components.exes.bcc-cli.postInstall = optparseCompletionPostInstall + libSodiumPostInstall + setGitRevPostInstall;
        packages.bcc-node.components.exes.bcc-node.postInstall = optparseCompletionPostInstall + libSodiumPostInstall + setGitRevPostInstall;
        packages.bcc-addresses-cli.components.exes.bcc-address.postInstall = optparseCompletionPostInstall;
        packages.bech32.components.exes.bech32.postInstall = optparseCompletionPostInstall;
      })

      # Provide the git revision for bcc-addresses
      ({ config, ... }:
      {
        packages.bcc-addresses-cli.components.library.preBuild = ''
          export GITREV=${config.hsPkgs.bcc-addresses-cli.src.rev}
        '';
      })
      ...
      # source tree.
      {
        packages.bcc-wallet-core.components.tests.unit.preBuild = ''
      ...
      (lib.optionalAttrs profiling {
        ...
        packages.bcc-wallet.components.exes.bcc-wallet.enableExecutableProfiling = true;
        packages.bcc-wallet.components.benchmarks.restore.enableExecutableProfiling = true;
      })
      ...
      in {
        ...
        packages.bcc-wallet.components.benchmarks.restore = fullyStaticOptions;
        packages.bcc-wallet.components.exes.bcc-wallet = fullyStaticOptions;
        packages.bcc-wallet.components.tests.integration = fullyStaticOptions;
        packages.bcc-wallet.components.tests.unit = fullyStaticOptions;
        packages.bcc-wallet-cli.components.tests.unit = fullyStaticOptions;
        packages.bcc-wallet-core.components.benchmarks.db = fullyStaticOptions;
        packages.bcc-wallet-core.components.tests.unit = fullyStaticOptions;
        packages.bcc-wallet-launcher.components.tests.unit = fullyStaticOptions;
      ...
      {
        packages.bcc-addresses.cabal-generator = lib.mkForce null;
        packages.bcc-addresses-cli.cabal-generator = lib.mkForce null;
      }
