...
let
  ...
  # nix-prefetch-url --unpack https://github.com/The-Blockchain-Company/bcc-wallet/archive/v2021-06-11.zip
  releases = [
    { rev = "v2021-01-12";
  ...
  migrationTest = (importRelease null).haskellPackages.bcc-wallet.components.exes.migration-test;
    ...
    testRelease = rel: let
      ...
      in pkgs.writeScript "launch-migration-test-${releaseName rel}.sh" ''
        ...
        export PATH=${makeBinPath [
          targetRelease.bcc-wallet
          targetRelease.bcc-node
          migrationTest
          pkgs.bash
          pkgs.coreutils
          pkgs.python3
        ]}
        # fixme: ADP-549 port to sophie
        export genesisDataDir=${latestRelease.src}/lib/quibitous/test/data/quibitous
        export configFile=${targetRelease.src}/lib/quibitous/test/data/quibitous/config.yaml

        exec ${./launch-migration-test.sh} "$@"
      '' // { inherit (latestRelease) bcc-wallet; };
  ...
  in
    ...
    '' + concatMapStringsSep "\n" (test: ''
        ...
        ln -s ${test.test.bcc-wallet}/bin/* $out/${test.name}
  ...
  mkTestsWindows = let
    ...
    mkTestRunner = rel: rec {
      ...
      inherit (walletPackages) bcc-wallet src;
      allowFail = rel.allowFail or false;
      runner = let
  ...
  in
    ...
    '' + concatMapStringsSep "\n" (test: ''
      ...
      cp ${test.bcc-wallet}/bin/* $out/${test.name}
      cp ${test.runner} $out/${test.name}/${test.runner.name}
      # fixme: ADP-549 port to sophie
      cp ${latest.src}/lib/quibitous/test/data/quibitous/{block0.bin,config.yaml,secret.yaml} $out/${test.name}/data
