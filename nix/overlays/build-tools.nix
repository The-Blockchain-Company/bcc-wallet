...
pkgs: super: let
  ...
  # https://github.com/The-Blockchain-Company/nix-tools/issues/97
...
in {
  ...
  # These overrides are picked up by cabalWrapped in tbco-nix
  cabal = pkgs.haskell-build-tools.cabal-install;
  cabal-install = pkgs.haskell-build-tools.cabal-install;

  # Override this package set from tbco-nix with local materializations.
  ...
  in pkgs.symlinkJoin {
    inherit name;
    paths = pkgs.lib.attrValues project.tbco-nix-utils.components.exes;
    passthru = {
      inherit project;
      ...
      regenerateMaterialized = pkgs.writeShellScriptBin "regenerate-materialized-nix"
        (pkgs.lib.optionalString enableMaterialization ''
          echo 'Updating materialized nix for ${name}'
          ${mkMaterialize project.tbco-nix-utils}
        '');
    };
  };
}
