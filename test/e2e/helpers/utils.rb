...
module Helpers
  module Utils
    ...
    def bcc_address_get_acc_xpub(mnemonics, derivation_path, hex = true, wallet_type = "Shared", chain_code = "--with-chain-code")
      cmd(%(echo #{mnemonics.join(' ')} \
         | bcc-address key from-recovery-phrase #{wallet_type} \
         | bcc-address key child #{derivation_path} \
         | bcc-address key public #{chain_code} #{" | bech32" if hex})).gsub("\n", '')
    end
    ...
    def get_latest_binary_url
      ...
      "https://hydra.tbco.io/job/Bcc/bcc-wallet/bcc-wallet-#{os}64/latest/download-by-type/file/binary-dist"
    end

    def get_latest_configs_base_url
      "https://hydra.tbco.io/job/Bcc/tbco-nix/bcc-deployment/latest/download/1"
    end
  end
end
