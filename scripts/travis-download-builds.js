// This script downloads builds of bcc-wallet from Hydra.
...
async function findEvalByCommit(api, project, jobset, rev, page)  {
  ...
  const eval = _.find(r.data.evals, e => e.jobsetevalinputs["bcc-wallet"].revision === rev);
...
function findBccWalletEval(api, rev) {
  return findEvalByCommit(apiapi, "Bcc", "bcc-wallet", rev);
}
...
async function download(outDir, downloadSpec, jobs, options = {}) {
  const hydraUrl = "https://hydra.tbco.io/";
