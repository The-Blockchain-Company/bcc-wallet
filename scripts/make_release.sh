...
#
# Usage:
# cd bcc-wallet # (only works from here!)
...
OLD_BCC_NODE_TAG=$BCC_NODE_TAG
read -r -e -p "Bcc node tag: " -i "$BCC_NODE_TAG" BCC_NODE_TAG
sed -i -e "s/^BCC_NODE_TAG=\"$OLD_BCC_NODE_TAG\"/BCC_NODE_TAG=\"$BCC_NODE_TAG\"/g" "$SCRIPT"
...
line_to_insert=$(echo "$master_line" | sed -e "s/\`master\` branch/\[$GIT_TAG\](https:\/\/github.com\/The-Blockchain-Company\/bcc-wallet\/releases\/tag\/$GIT_TAG)/")
sed -i -e "s/^GIT_TAG=\"$GIT_TAG\"/GIT_TAG=\"$new_tag\"/g" "$SCRIPT"
...
REPO="The-Blockchain-Company/bcc-wallet"
...
sed -e "s/{{GIT_TAG}}/$GIT_TAG/g"                   \
    -e "s/{{BCC_NODE_TAG}}/$BCC_NODE_TAG/g" \
