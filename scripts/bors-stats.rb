...
def fetch_system_from_build_link(link)
    if link.start_with? "https://hydra.tbco.io" then
...
def fetch_comments(target = 100, before = nil, force_refetch = false)
  ...
  query = <<~END
    query { repository(name: "bcc-wallet", owner: "The-Blockchain-Company") {
      ...
      .map do |x|
        ...
        if not succ then
          buildLinks.each do |l|
            if l.start_with? "https://hydra.tbco.io" then
              tags += ["hydra"]
...
#
#   {2083=>[{"number"=>2083, "url"=>"https://github.com/The-Blockchain-Company/bcc-wallet/issues/2083", "title"=>"Windows integration" }
...
def fetch_comments_with_options(options)
  ...
  if options["auto-annotate"] then
    comments.each do |c|
      # 1) API Specifications, SOPHIE_TRANSACTIONS, TRANS_CREATE_01x - Single Output Transaction
      # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  comments[:filtered].each do |c|
    c.links.each do |l|
      if l.start_with? "https://buildkite" then
        ...
        build_url = "https://api.buildkite.com/v2/organizations/The-Blockchain-Company/pipelines/bcc-wallet/builds/#{build_id}"
        ...
        # href="https://hydra.tbco.io/build/6163056/nixlog/1/raw"
        # To find the link to the raw logs of the child build that actually failed.
        matches = html.scan(/(https:\/\/hydra\.tbco\.io\/build\/(\d+)\/nixlog\/\d\/raw)/)
        if not (matches == []) then
